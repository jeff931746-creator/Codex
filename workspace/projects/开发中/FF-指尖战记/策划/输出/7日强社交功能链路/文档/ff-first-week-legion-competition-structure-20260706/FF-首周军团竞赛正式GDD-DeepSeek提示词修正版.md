...
军团竞赛只接收已由所属玩法确认的功能结果，不直接判定上游玩法胜负、奖励归属或协助是否成立。各来源接入时按以下功能契约判断：

| 行为来源 | 结果提供方 | 上游确认的结果意图 | 军团竞赛接收的结果类型 | 未开放、未就绪或延迟确认时的玩家反馈 |
|---|---|---|---|---|
| 军团目标、军团任务、军团试炼 | 军团目标或任务系统 | 成员完成可服务本阶段竞赛的军团行为 | 目标完成、任务完成、试炼完成等已确认结果 | 来源未开放时不展示为可计入行动；完成后未确认时展示处理中，确认前不增加积分或贡献 |
| 世界狩猎、公共 Boss | 世界狩猎或公共 Boss 玩法 | 玩家产生有效参与、击败或奖励归属 | 有效参与、有效击败、有效奖励等已确认结果 | 玩法未开放时不进入追赶提示；奖励或击败归属未确认时展示等待结算，确认失败则说明本次未计入 |
| 寻血或高风险争夺 | 寻血或对应争夺玩法 | 玩家产生有效争夺、防守、追回或收益结果 | 有效争夺、有效防守、有效追回、有效收益等已确认结果 | 玩法未开放时不展示为竞赛来源；结果延迟时展示确认中，确认无效时只保留所属玩法自身反馈，不进入军团竞赛 |
| 同军团成员协助 | 协助发起玩法或协助系统 | 同军团成员完成有效响应，且协助被所属玩法接受 | 有效协助、有效响应、请求成员贡献等按配置开启的已确认结果 | 协助过期、失败、跨军团或重复响应时提示协助未计入；请求成员贡献未开启时只展示响应成员贡献 |
...
- 来源配置关闭时，该来源不展示为可计入行动；若玩家仍在所属玩法中完成行为，只获得所属玩法自身结果，不新增本阶段军团积分或成员贡献。
...
来源、结果类型、基础分、结果系数和权重都按本阶段配置生效。配置未覆盖的结果类型，不进入军团积分和成员贡献；页面按未计入结果展示原因。
...
- 并列排名不额外生成新的奖励规则；奖励档位、并列处理和展示文案均以本阶段奖励配置为准。
...
未领取奖励的最终处理方式由项目通用奖励规则决定。本文只要求在奖励说明中提前告知处理结果，并在结果保留结束前持续展示可领取状态或不可领取原因。
...
|---|---|---|---|---|---|
| 阶段开启时间 | 4.1 阶段开启与资格判定 | 阶段实例 | 按首周活动排期配置 | 具体日期、时间点 | 控制阶段从未开启进入进行中 |
| 阶段结束时间 | 4.1 阶段开启与资格判定 | 阶段实例 | 按首周活动排期配置 | 具体日期、时间点 | 到达后停止接收新结果 |
| 结算时间 | 4.5 结算与奖励领取 | 阶段实例 | 阶段结束后进入结算 | 具体时间点或结束后延迟时间 | 控制排名、奖励资格和领取状态生成 |
| 结果保留时间 | 4.6 结果保留与后续预告 | 阶段实例 | 结算后保留一段时间 | 时长配置 | 保留期间支持结果查看和奖励领取 |
| 有效军团条件 | 4.1 阶段开启与资格判定 | 阶段实例 | 结果生效时玩家属于有效军团 | 军团状态、成员状态、加入时间限制 | 用于判断行为是否可进入军团结果 |
| 中途加入计入规则 | 4.1 阶段开启与资格判定 | 阶段实例 | 加入后生效的行为可计入，加入前不追溯 | 可关闭中途加入计入，或增加加入后等待时间 | 需与军团系统成员状态一致 |
| 无军团展示规则 | 4.1 阶段开启与资格判定 | 阶段实例 | 可预览入口，不计入结果 | 关闭预览、展示入团引导、展示创建军团引导 | 影响无军团玩家入口反馈 |
...
| 来源积分权重 | 4.3 军团积分与贡献记录 | 阶段实例 | 待配置 | 每个来源独立配置 | 用于决定不同来源增加的军团积分 |
| 行为基础分 | 4.3 军团积分与贡献记录 | 阶段实例 | 待配置 | 按行为类型配置 | 区分目标完成、收益获得、争夺成功、协助成功的基础价值 |
| 结果系数 | 4.3 军团积分与贡献记录 | 阶段实例 | 待配置 | 按结果强度配置 | 普通完成、关键目标完成、追回成功、防守成功等使用不同系数 |
| 日上限 | 4.3 军团积分与贡献记录 | 阶段实例 | 待配置 | 按来源、成员或军团配置 | 限制单日可计入次数或积分 |
| 阶段上限 | 4.3 军团积分与贡献记录 | 阶段实例 | 待配置 | 按来源、成员或军团配置 | 限制整个阶段可计入次数或积分 |
...
| 成员贡献展示来源 | 4.3 军团积分与贡献记录 | 阶段实例 | 只展示对应真实有效行为的贡献来源 | 来源展示开关、展示排序、展示数量 | 贡献展示不得脱离实际行为结果 |
| 请求成员贡献开关 | 4.3 军团积分与贡献记录 | 阶段实例 | 响应成员默认获得贡献；请求成员是否获得贡献按开关配置 | 开启 / 关闭 | 开启时必须单独展示为请求成员贡献 |
| 排行刷新间隔 | 4.4 排行与追赶反馈 | 阶段实例 | 待配置 | 秒级、分钟级或手动刷新 | 页面需展示刷新状态和更新时间 |
| 排行展示范围 | 4.4 排行与追赶反馈 | 阶段实例 | 展示本军团及相关排名范围 | 全榜、前 N 名、本军团附近范围 | 影响排行页信息量 |
| 同分排序规则 | 4.4 排行与追赶反馈 | 阶段实例 | 更早达到当前积分的军团排前；仍无法区分则并列 | 可按项目统一排行规则覆盖 | 并列时按配置奖励档位发放奖励 |
| 追赶提示目标 | 4.4 排行与追赶反馈 | 阶段实例 | 指向当前可参与且未达上限的行为来源 | 按来源、奖励档位、相邻军团配置 | 不得承诺必然反超 |
| 奖励档位 | 4.5 结算与奖励领取 | 阶段实例 | 待配置 | 排名区间、并列处理、参与奖励 | 控制不同排名的奖励范围 |
| 奖励内容 | 4.5 结算与奖励领取 | 阶段实例 | 待配置 | 道具、货币、权益或其他奖励内容 | 需与项目奖励库确认 |
| 领取资格 | 4.5 结算与奖励领取 | 阶段实例 | 结算时属于军团且本阶段有有效贡献 | 可增加参与奖励或最低贡献要求 | 无贡献成员默认无阶段奖励 |
| 未领取奖励处理 | 4.5 结算与奖励领取 | 阶段实例 | 按项目通用未领取奖励规则处理 | 邮件、失效、补领入口等项目规则 | 必须在奖励说明中展示 |
| 延迟生效结果处理 | 4.5 结算与奖励领取 | 阶段实例 | 结算锁定后才生效的结果不进入本阶段 | 可按项目统一结算规则覆盖 | 避免结算后排名变化 |
...
|---|---|---|---|
| 各行为来源的积分值、权重、日上限和阶段上限 | 4.3 军团积分与贡献记录 | 当前资料未提供具体数值 | 需要数值表或活动配置确认 |
| 奖励档位、奖励内容和奖励数量 | 4.5 结算与奖励领取 | 当前资料未提供奖励表 | 需要奖励库和活动投放方案确认 |
| 阶段开启时间、结束时间、结算时间和结果保留时长 | 4.1 阶段开启与资格判定 | 需要首周活动排期确认 | 需要活动排期确认 |
| 排行刷新间隔和排行展示范围 | 4.4 排行与追赶反馈 | 需要 UE 信息量、服务器刷新策略和活动节奏确认 | 需要 UE 和服务器策略确认 |
| 请求成员是否默认获得协助贡献 | 4.3 军团积分与贡献记录 | 当前只确认可配置 | 需要协助玩法资料或项目口径确认默认开关 |
| 后续预告具体指向 GVG、沙盘、战役或威望中的哪一个 | 4.6 结果保留与后续预告 | 当前只确认后续共同目标预告 | 需要后续玩法排期确认 |
| 项目通用未领取奖励规则 | 4.5 结算与奖励领取 | 奖励过期、邮件补发或补领入口需与项目通用规则一致 | 需要项目通用规则确认 |
