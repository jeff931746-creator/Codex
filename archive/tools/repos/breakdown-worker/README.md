...
用大模型批量产出游戏机制拆解初稿，Claude 审校后落到 `机制库/`。LLM 调用统一使用 route。
...
# 2. 出初稿
python3 run.py --game "某游戏" --route SiliconFlow_GLM
...
- `--route DeepSeek_Official_Pro`：切到 DeepSeek 官方 API
- `--route Google_Gemini_Pro`：切到 Gemini
...
set +a
python3 run.py --game "某游戏" --route DeepSeek_Official_Pro
```
...
5. 按 `--route` 调对应 API，要求响应用 `===FILE: xxx===` 分隔多个文件
6. 拆分并写到 `outputs/<游戏名>/`
