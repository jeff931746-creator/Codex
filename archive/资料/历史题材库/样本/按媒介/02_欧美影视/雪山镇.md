---
# === 身份 ===
work_id: WORK-TVW-01027
work_name: 雪山镇
work_name_alt: []
media_type: TVW
year_first: 2002
year_range: [2002, 2004]
season_count: null

# === 三要素(v0 已有,迁移直翻)===
environment: 现代美国小镇生活
cultural_paradigm: 家庭伦理+小镇社区文化
narrative_core: 父子温情与成长

# === SDT 心理需求(TBD,等 LLM 阶段补)===
n_main: N-Rel
n_others: [N-Auto]
rel_object: virtual
t_can_carry: [T3, T1]
core_emotion_note: ""
core_conflict: 父亲与儿子在失去母亲后重建家庭纽带与小镇生活

# === 三层标签(主检索路径,见 TAGS.md)===
theme_primary: 家庭
theme_secondary: [亲情, 田园, 都市, 治愈]
visual_tags: [小镇雪景, 温馨晚餐, 社区活动, 田园清新]


# === 评分(v0 已有)===
acquisition_score: 2

# === 素材与玩法 ===
material_hooks: [小镇雪景, 父子日常, 温馨晚餐, 社区活动]
existing_games: []
recommended_playstyles: []
risk_notes: [受众窄, 付费浅]

# === 元数据 ===
source: deepseek-v4-flash + v0-migrate + tags
source_run_id: 2026-05-18-tags-enrich
evidence_level: B
review_status: auto
last_updated: 2026-05-18
# v0_duplicate_count: 4
---

## 迁移备注

本样本由 v0 markdown 表格迁移而来,LLM 阶段尚未补 SDT 三需求 / T 任务态 / core_conflict / material_hooks 重判。
迁移完成后 evidence_level = C,LLM 增量打标后升至 B。
