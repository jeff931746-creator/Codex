---
# === 身份 ===
work_id: WORK-TVW-00524
work_name: 点球成金
work_name_alt: []
media_type: TVW
year_first: 2011
year_range: [2011, 2011]
season_count: null

# === 三要素(v0 已有,迁移直翻)===
environment: 现代美国职业棒球界
cultural_paradigm: 数据统计分析+棒球竞技
narrative_core: 逆袭革新/管理博弈

# === SDT 心理需求(TBD,等 LLM 阶段补)===
n_main: N-Comp
n_others: []
rel_object: null
t_can_carry: [T2, T5]
core_emotion_note: ""
core_conflict: 数据派经理挑战传统棒球体制，用有限资源打造冠军队伍

# === 三层标签(主检索路径,见 TAGS.md)===
theme_primary: 体育竞技
theme_secondary: [棒球, 职场, 成长, 逆袭]
visual_tags: [棒球场, 更衣室, 办公室, 数据面板]


# === 评分(v0 已有)===
acquisition_score: 3

# === 素材与玩法 ===
material_hooks: [数据面板分析, 球员交易谈判, 逆袭胜利瞬间, 更衣室冲突]
existing_games: []
recommended_playstyles: []
risk_notes: [体育题材窄, 版权成本中等]

# === 元数据 ===
source: deepseek-v4-flash + v0-migrate + tags
source_run_id: 2026-05-18-tags-enrich
evidence_level: B
review_status: auto
last_updated: 2026-05-18
---

## 迁移备注

本样本由 v0 markdown 表格迁移而来,LLM 阶段尚未补 SDT 三需求 / T 任务态 / core_conflict / material_hooks 重判。
迁移完成后 evidence_level = C,LLM 增量打标后升至 B。
