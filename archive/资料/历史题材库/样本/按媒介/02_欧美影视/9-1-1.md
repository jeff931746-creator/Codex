---
# === 身份 ===
work_id: WORK-TVW-00716
work_name: 9-1-1
work_name_alt: []
media_type: TVW
year_first: 2025
year_range: [2025, 2026]
season_count: null

# === 三要素(v0 已有,迁移直翻)===
environment: 现代洛杉矶都市
cultural_paradigm: 应急救援体系+职业伦理
narrative_core: 生死救援与团队协作

# === SDT 心理需求(TBD,等 LLM 阶段补)===
n_main: N-Rel
n_others: [N-Comp]
rel_object: mixed
t_can_carry: [T4, T1]
core_emotion_note: ""
core_conflict: 消防员与急救人员在生死一线中协作救援，同时应对个人与团队危机

# === 三层标签(主检索路径,见 TAGS.md)===
theme_primary: 职场
theme_secondary: [警务, 医疗, 都市, 求生]
visual_tags: [都市繁华, 爆炸场面, 火场, 高空救援, 急救手术特写]


# === 评分(v0 已有)===
acquisition_score: 4

# === 素材与玩法 ===
material_hooks: [救援现场实拍感, 消防车疾驰, 高空绳索救援, 爆炸与火场, 急救手术特写]
existing_games: []
recommended_playstyles: []
risk_notes: [受众窄, 题材同质化]

# === 元数据 ===
source: deepseek-v4-flash + v0-migrate + tags
source_run_id: 2026-05-18-tags-enrich
evidence_level: B
review_status: auto
last_updated: 2026-05-18
# v0_duplicate_count: 2
---

## 迁移备注

本样本由 v0 markdown 表格迁移而来,LLM 阶段尚未补 SDT 三需求 / T 任务态 / core_conflict / material_hooks 重判。
迁移完成后 evidence_level = C,LLM 增量打标后升至 B。
