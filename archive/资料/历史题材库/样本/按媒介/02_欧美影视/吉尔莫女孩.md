---
# === 身份 ===
work_id: WORK-TVW-01024
work_name: 吉尔莫女孩
work_name_alt: []
media_type: TVW
year_first: 2000
year_range: [2000, 2006]
season_count: null

# === 三要素(v0 已有,迁移直翻)===
environment: 现代美国小镇
cultural_paradigm: 母女日常+小镇社交
narrative_core: 温馨成长与家庭羁绊

# === SDT 心理需求(TBD,等 LLM 阶段补)===
n_main: N-Rel
n_others: []
rel_object: virtual
t_can_carry: [T3]
core_emotion_note: ""
core_conflict: 母女在成长与独立中维系亲情纽带

# === 三层标签(主检索路径,见 TAGS.md)===
theme_primary: 家庭
theme_secondary: [母女, 小镇生活, 日常喜剧, 成长]
visual_tags: [小镇街景, 咖啡馆, 秋季落叶, 家庭晚餐]


# === 评分(v0 已有)===
acquisition_score: 3

# === 素材与玩法 ===
material_hooks: [小镇咖啡馆, 母女对话, 秋季落叶, 温馨家庭晚餐]
existing_games: []
recommended_playstyles: []
risk_notes: [受众窄, 付费深度低]

# === 元数据 ===
source: deepseek-v4-flash + v0-migrate + tags
source_run_id: 2026-05-18-tags-enrich
evidence_level: B
review_status: auto
last_updated: 2026-05-18
# v0_duplicate_count: 10
---

## 迁移备注

本样本由 v0 markdown 表格迁移而来,LLM 阶段尚未补 SDT 三需求 / T 任务态 / core_conflict / material_hooks 重判。
迁移完成后 evidence_level = C,LLM 增量打标后升至 B。
