---
# === 身份 ===
work_id: WORK-TVW-00902
work_name: 疯子
work_name_alt: []
media_type: TVW
year_first: 2018
year_range: [2018, 2018]
season_count: null

# === 三要素(v0 已有,迁移直翻)===
environment: 现代精神药物实验中心
cultural_paradigm: 精神冒险+科幻心理
narrative_core: 药物实验中的虚实冒险

# === SDT 心理需求(TBD,等 LLM 阶段补)===
n_main: N-Auto
n_others: []
rel_object: null
t_can_carry: [T5, T1]
core_emotion_note: ""
core_conflict: 主角在药物实验中分辨虚实，对抗精神控制

# === 三层标签(主检索路径,见 TAGS.md)===
theme_primary: 悬疑
theme_secondary: [心理悬疑, 科幻, 求生]
visual_tags: [实验室, 迷宫, 意识扭曲特效, 心理对抗画面, 黑暗压抑]


# === 评分(v0 已有)===
acquisition_score: 2

# === 素材与玩法 ===
material_hooks: [药物幻觉场景, 实验室迷宫, 意识扭曲特效, 心理对抗画面]
existing_games: []
recommended_playstyles: []
risk_notes: [受众窄, 题材难理解]

# === 元数据 ===
source: deepseek-v4-flash + v0-migrate + tags
source_run_id: 2026-05-18-tags-enrich
evidence_level: B
review_status: auto
last_updated: 2026-05-18
# v0_duplicate_count: 2
---

## 迁移备注

本样本由 v0 markdown 表格迁移而来,LLM 阶段尚未补 SDT 三需求 / T 任务态 / core_conflict / material_hooks 重判。
迁移完成后 evidence_level = C,LLM 增量打标后升至 B。
