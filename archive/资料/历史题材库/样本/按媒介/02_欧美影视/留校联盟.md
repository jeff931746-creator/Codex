---
# === 身份 ===
work_id: WORK-TVW-01219
work_name: 留校联盟
work_name_alt: []
media_type: TVW
year_first: 2023
year_range: [2023, 2023]
season_count: null

# === 三要素(v0 已有,迁移直翻)===
environment: 现代美国寄宿校园
cultural_paradigm: 师生日常+圣诞文化
narrative_core: 孤独治愈+成长和解

# === SDT 心理需求(TBD,等 LLM 阶段补)===
n_main: N-Rel
n_others: [N-Auto]
rel_object: virtual
t_can_carry: [T3, T1]
core_emotion_note: ""
core_conflict: 叛逆学生与固执老师相互治愈，在孤独圣诞中寻找归属

# === 三层标签(主检索路径,见 TAGS.md)===
theme_primary: 都市
theme_secondary: [校园, 家庭, 治愈, 成长]
visual_tags: [校园, 雪景, 圣诞树, 旧校舍走廊, 篝火]


# === 评分(v0 已有)===
acquisition_score: 3

# === 素材与玩法 ===
material_hooks: [雪中校园, 圣诞树光影, 师生对峙, 旧校舍走廊, 篝火旁对话]
existing_games: []
recommended_playstyles: []
risk_notes: [受众窄, 付费浅, 无战斗体系]

# === 元数据 ===
source: deepseek-v4-flash + v0-migrate + tags
source_run_id: 2026-05-18-tags-enrich
evidence_level: B
review_status: auto
last_updated: 2026-05-18
---

## 迁移备注

本样本由 v0 markdown 表格迁移而来,LLM 阶段尚未补 SDT 三需求 / T 任务态 / core_conflict / material_hooks 重判。
迁移完成后 evidence_level = C,LLM 增量打标后升至 B。
