---
# === 身份 ===
work_id: WORK-TVW-00568
work_name: 1/2的魔法
work_name_alt: []
media_type: TVW
year_first: 2020
year_range: [2020, 2020]
season_count: null

# === 三要素(v0 已有,迁移直翻)===
environment: 现代魔法世界
cultural_paradigm: 魔法科技+兄弟羁绊
narrative_core: 寻亲冒险+成长治愈

# === SDT 心理需求(TBD,等 LLM 阶段补)===
n_main: N-Rel
n_others: [N-Comp]
rel_object: virtual
t_can_carry: [T3, T2]
core_emotion_note: ""
core_conflict: 兄弟在有限时间内寻回父亲，克服魔法障碍与内心隔阂

# === 三层标签(主检索路径,见 TAGS.md)===
theme_primary: 冒险
theme_secondary: [奇幻, 家庭, 友情, 亲情]
visual_tags: [魔法阵, 城堡遗迹, 魔法摩托, 奇幻生物, 古风装扮]


# === 评分(v0 已有)===
acquisition_score: 4

# === 素材与玩法 ===
material_hooks: [兄弟魔法冒险, 半身父亲幻影, 魔法生物追逐, 城堡遗迹探索, 魔法摩托飞驰]
existing_games: []
recommended_playstyles: []
risk_notes: [版权成本高]

# === 元数据 ===
source: deepseek-v4-flash + v0-migrate + tags
source_run_id: 2026-05-18-tags-enrich
evidence_level: B
review_status: auto
last_updated: 2026-05-18
---

## 迁移备注

本样本由 v0 markdown 表格迁移而来,LLM 阶段尚未补 SDT 三需求 / T 任务态 / core_conflict / material_hooks 重判。
迁移完成后 evidence_level = C,LLM 增量打标后升至 B。
