---
# === 身份 ===
work_id: WORK-TVW-01236
work_name: 王与先知》（2016，欧美，圣经时代以色列王权斗争）
work_name_alt: []
media_type: TVW
year_first: 2016
year_range: [2016, 2016]
season_count: null

# === 三要素(v0 已有,迁移直翻)===
environment: 圣经时代以色列
cultural_paradigm: 宗教史诗+王权斗争
narrative_core: 信仰冲突+权力更迭

# === SDT 心理需求(TBD,等 LLM 阶段补)===
n_main: N-Rel
n_others: []
rel_object: real
t_can_carry: [T4]
core_emotion_note: ""
core_conflict: 先知与君王争夺神权与王权，信仰与政治交织的冲突

# === 三层标签(主检索路径,见 TAGS.md)===
theme_primary: 历史
theme_secondary: [政治权谋, 战争, 王朝兴衰]
visual_tags: [古战场, 宫殿, 圣殿, 古风装扮]


# === 评分(v0 已有)===
acquisition_score: 3

# === 素材与玩法 ===
material_hooks: [圣殿祭祀场景, 先知预言时刻, 王宫权谋对峙, 战场刀剑交锋]
existing_games: []
recommended_playstyles: []
risk_notes: [宗教敏感, 受众极窄]

# === 元数据 ===
source: deepseek-v4-flash + v0-migrate + tags
source_run_id: 2026-05-18-tags-enrich
evidence_level: B
review_status: auto
last_updated: 2026-05-18
---

## 迁移备注

本样本由 v0 markdown 表格迁移而来,LLM 阶段尚未补 SDT 三需求 / T 任务态 / core_conflict / material_hooks 重判。
迁移完成后 evidence_level = C,LLM 增量打标后升至 B。
