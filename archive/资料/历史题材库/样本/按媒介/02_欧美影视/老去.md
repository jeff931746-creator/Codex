---
# === 身份 ===
work_id: WORK-TVW-01207
work_name: 老去
work_name_alt: []
media_type: TVW
year_first: 2021
year_range: [2021, 2021]
season_count: null

# === 三要素(v0 已有,迁移直翻)===
environment: 神秘海滩
cultural_paradigm: 超自然衰老设定
narrative_core: 快速衰老的噩梦

# === SDT 心理需求(TBD,等 LLM 阶段补)===
n_main: N-Comp
n_others: []
rel_object: null
t_can_carry: [T5]
core_emotion_note: ""
core_conflict: 被困神秘海滩的人加速衰老，在时间耗尽前寻找逃离方法

# === 三层标签(主检索路径,见 TAGS.md)===
theme_primary: 悬疑
theme_secondary: [求生, 超自然悬疑, 时间轮回, 灾难]
visual_tags: [海滩迷雾笼罩, 衰老皮肤特写, 尸体风化场景, 时间流逝倒计时, 生存绝望]


# === 评分(v0 已有)===
acquisition_score: 3

# === 素材与玩法 ===
material_hooks: [衰老皮肤特写, 海滩迷雾笼罩, 时间流逝倒计时, 尸体风化场景]
existing_games: []
recommended_playstyles: []
risk_notes: [受众极窄, 玩法受限]

# === 元数据 ===
source: deepseek-v4-flash + v0-migrate + tags
source_run_id: 2026-05-18-tags-enrich
evidence_level: B
review_status: auto
last_updated: 2026-05-18
# v0_duplicate_count: 2
---

## 迁移备注

本样本由 v0 markdown 表格迁移而来,LLM 阶段尚未补 SDT 三需求 / T 任务态 / core_conflict / material_hooks 重判。
迁移完成后 evidence_level = C,LLM 增量打标后升至 B。
