---
# === 身份 ===
work_id: WORK-TVW-00900
work_name: 少年谢尔顿
work_name_alt: []
media_type: TVW
year_first: 2017
year_range: [2017, 2017]
season_count: null

# === 三要素(v0 已有,迁移直翻)===
environment: 1980年代美国德州
cultural_paradigm: 天才儿童+家庭喜剧
narrative_core: 童年成长+家庭日常

# === SDT 心理需求(TBD,等 LLM 阶段补)===
n_main: N-Rel
n_others: [N-Auto]
rel_object: mixed
t_can_carry: [T3, T1]
core_emotion_note: ""
core_conflict: 天才儿童在普通家庭中寻找理解与成长

# === 三层标签(主检索路径,见 TAGS.md)===
theme_primary: 家庭
theme_secondary: [校园, 喜剧, 成长, 亲情]
visual_tags: [校园青春, 家庭温情, 日常温馨, 搞笑荒诞]


# === 评分(v0 已有)===
acquisition_score: 4

# === 素材与玩法 ===
material_hooks: [谢尔顿的呆萌表情, 家庭餐桌对话, 校园搞笑场景, 天才实验翻车]
existing_games: []
recommended_playstyles: []
risk_notes: [付费能力弱, 玩法受限]

# === 元数据 ===
source: deepseek-v4-flash + v0-migrate + tags
source_run_id: 2026-05-18-tags-enrich
evidence_level: B
review_status: auto
last_updated: 2026-05-18
---

## 迁移备注

本样本由 v0 markdown 表格迁移而来,LLM 阶段尚未补 SDT 三需求 / T 任务态 / core_conflict / material_hooks 重判。
迁移完成后 evidence_level = C,LLM 增量打标后升至 B。
