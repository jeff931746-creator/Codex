---
# === 身份 ===
work_id: WORK-TVW-00460
work_name: 基地
work_name_alt: []
media_type: TVW
year_first: 2021
year_range: [2021, 2021]
season_count: null

# === 三要素(v0 已有,迁移直翻)===
environment: 银河帝国/未来星际
cultural_paradigm: 心理史学+科技文明
narrative_core: 文明兴衰与权谋博弈

# === SDT 心理需求(TBD,等 LLM 阶段补)===
n_main: N-Rel
n_others: [N-Comp]
rel_object: real
t_can_carry: [T4, T5]
core_emotion_note: ""
core_conflict: 心理史学家用科学预测对抗帝国衰亡与暗中的权力阴谋

# === 三层标签(主检索路径,见 TAGS.md)===
theme_primary: 科幻
theme_secondary: [太空, 政治权谋, 文明崩溃, 未来战争]
visual_tags: [星舰内部, 行星表面, 未来星际, 权谋阴谋]


# === 评分(v0 已有)===
acquisition_score: 4

# === 素材与玩法 ===
material_hooks: [银河帝国星舰, 心理史学公式, 谢顿计划, 端点星议会, 骡的突变]
existing_games: []
recommended_playstyles: []
risk_notes: [受众偏硬核, 版权成本高]

# === 元数据 ===
source: deepseek-v4-flash + v0-migrate + tags
source_run_id: 2026-05-18-tags-enrich
evidence_level: B
review_status: auto
last_updated: 2026-05-18
---

## 迁移备注

本样本由 v0 markdown 表格迁移而来,LLM 阶段尚未补 SDT 三需求 / T 任务态 / core_conflict / material_hooks 重判。
迁移完成后 evidence_level = C,LLM 增量打标后升至 B。
