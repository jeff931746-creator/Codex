---
# === 身份 ===
work_id: WORK-TVW-00885
work_name: 贝茨旅馆
work_name_alt: []
media_type: TVW
year_first: 2013
year_range: [2013, 2013]
season_count: null

# === 三要素(v0 已有,迁移直翻)===
environment: 现代小镇+老旅馆
cultural_paradigm: 心理扭曲+母子共生
narrative_core: 少年诺曼的黑暗成长

# === SDT 心理需求(TBD,等 LLM 阶段补)===
n_main: N-Rel
n_others: []
rel_object: real
t_can_carry: [T4, T5]
core_emotion_note: ""
core_conflict: 诺曼·贝茨在母亲控制与自我独立间的扭曲共生

# === 三层标签(主检索路径,见 TAGS.md)===
theme_primary: 悬疑
theme_secondary: [心理悬疑, 家庭秘密, 犯罪, 恐怖, 成长]
visual_tags: [老旅馆, 阴森走廊, 小镇, 母子共处, 黑暗压抑]


# === 评分(v0 已有)===
acquisition_score: 3

# === 素材与玩法 ===
material_hooks: [母子共处画面, 旅馆阴森走廊, 尸体藏匿场景, 诺曼眼神切换]
existing_games: []
recommended_playstyles: []
risk_notes: [题材敏感, 受众窄]

# === 元数据 ===
source: deepseek-v4-flash + v0-migrate + tags
source_run_id: 2026-05-18-tags-enrich
evidence_level: B
review_status: auto
last_updated: 2026-05-18
# v0_duplicate_count: 3
---

## 迁移备注

本样本由 v0 markdown 表格迁移而来,LLM 阶段尚未补 SDT 三需求 / T 任务态 / core_conflict / material_hooks 重判。
迁移完成后 evidence_level = C,LLM 增量打标后升至 B。
