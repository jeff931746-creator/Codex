---
# === 身份 ===
work_id: WORK-TVW-01227
work_name: 波士顿公立学校
work_name_alt: []
media_type: TVW
year_first: 2000
year_range: [2000, 2000]
season_count: null

# === 三要素(v0 已有,迁移直翻)===
environment: 现代美国高中
cultural_paradigm: 教师职场+校园百态
narrative_core: 教师奋斗与校园冲突

# === SDT 心理需求(TBD,等 LLM 阶段补)===
n_main: N-Rel
n_others: [N-Comp]
rel_object: real
t_can_carry: [T4, T1]
core_emotion_note: ""
core_conflict: 教师面对体制压力与学生问题，在职场中争取位置与改变

# === 三层标签(主检索路径,见 TAGS.md)===
theme_primary: 校园
theme_secondary: [职场, 都市, 日常喜剧, 成长]
visual_tags: [校园, 教室, 操场, 都市夜景]


# === 评分(v0 已有)===
acquisition_score: 2

# === 素材与玩法 ===
material_hooks: [课堂冲突, 教师办公室, 学生群像, 校园走廊]
existing_games: []
recommended_playstyles: []
risk_notes: [受众极窄, 付费深度低]

# === 元数据 ===
source: deepseek-v4-flash + v0-migrate + tags
source_run_id: 2026-05-18-tags-enrich
evidence_level: B
review_status: auto
last_updated: 2026-05-18
---

## 迁移备注

本样本由 v0 markdown 表格迁移而来,LLM 阶段尚未补 SDT 三需求 / T 任务态 / core_conflict / material_hooks 重判。
迁移完成后 evidence_level = C,LLM 增量打标后升至 B。
