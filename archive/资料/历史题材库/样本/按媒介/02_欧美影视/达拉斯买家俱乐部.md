---
# === 身份 ===
work_id: WORK-TVW-01279
work_name: 达拉斯买家俱乐部
work_name_alt: []
media_type: TVW
year_first: 2013
year_range: [2013, 2013]
season_count: null

# === 三要素(v0 已有,迁移直翻)===
environment: 1980年代美国
cultural_paradigm: 医疗抗争+地下药物
narrative_core: 绝症求生+体制抗争

# === SDT 心理需求(TBD,等 LLM 阶段补)===
n_main: N-Comp
n_others: []
rel_object: null
t_can_carry: [T2]
core_emotion_note: ""
core_conflict: 绝症患者对抗医疗体制与死神，争取生存权

# === 三层标签(主检索路径,见 TAGS.md)===
theme_primary: 求生
theme_secondary: [医疗, 犯罪, 传记, 都市]
visual_tags: [医院, 法庭, 地下实验室, 1980年代美国]


# === 评分(v0 已有)===
acquisition_score: 3

# === 素材与玩法 ===
material_hooks: [地下药走私, 病床抗争, 法庭对峙, 生命倒计时]
existing_games: []
recommended_playstyles: []
risk_notes: [受众窄, 敏感题材]

# === 元数据 ===
source: deepseek-v4-flash + v0-migrate + tags
source_run_id: 2026-05-18-tags-enrich
evidence_level: B
review_status: auto
last_updated: 2026-05-18
# v0_duplicate_count: 2
---

## 迁移备注

本样本由 v0 markdown 表格迁移而来,LLM 阶段尚未补 SDT 三需求 / T 任务态 / core_conflict / material_hooks 重判。
迁移完成后 evidence_level = C,LLM 增量打标后升至 B。
