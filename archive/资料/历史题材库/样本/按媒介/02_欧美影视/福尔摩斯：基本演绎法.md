---
# === 身份 ===
work_id: WORK-TVW-00421
work_name: 福尔摩斯：基本演绎法
work_name_alt: []
media_type: TVW
year_first: 2011
year_range: [2011, 2012]
season_count: null

# === 三要素(v0 已有,迁移直翻)===
environment: 现代纽约都市
cultural_paradigm: 现代推理+科学探案
narrative_core: 经典侦探破案

# === SDT 心理需求(TBD,等 LLM 阶段补)===
n_main: N-Comp
n_others: []
rel_object: null
t_can_carry: [T5, T2]
core_emotion_note: ""
core_conflict: 福尔摩斯与华生破解纽约离奇案件，对抗隐藏的犯罪网络

# === 三层标签(主检索路径,见 TAGS.md)===
theme_primary: 推理
theme_secondary: [刑侦, 都市, 犯罪, 悬疑]
visual_tags: [都市夜景, 实验室, 犯罪现场, 推理冷静]


# === 评分(v0 已有)===
acquisition_score: 4

# === 素材与玩法 ===
material_hooks: [福尔摩斯推理特写, 犯罪现场还原, 纽约街景追逐, 实验室科学探案]
existing_games: []
recommended_playstyles: []
risk_notes: [版权成本高, 同质化竞争]

# === 元数据 ===
source: deepseek-v4-flash + v0-migrate + tags
source_run_id: 2026-05-18-tags-enrich
evidence_level: B
review_status: auto
last_updated: 2026-05-18
# v0_duplicate_count: 2
---

## 迁移备注

本样本由 v0 markdown 表格迁移而来,LLM 阶段尚未补 SDT 三需求 / T 任务态 / core_conflict / material_hooks 重判。
迁移完成后 evidence_level = C,LLM 增量打标后升至 B。
