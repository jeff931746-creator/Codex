---
# === 身份 ===
work_id: WORK-TVW-00656
work_name: 芝加哥烈焰
work_name_alt: []
media_type: TVW
year_first: 2012
year_range: [2012, 2026]
season_count: null

# === 三要素(v0 已有,迁移直翻)===
environment: 现代都市消防站
cultural_paradigm: 消防员职业体系
narrative_core: 日常救援+团队成长

# === SDT 心理需求(TBD,等 LLM 阶段补)===
n_main: N-Rel
n_others: [N-Comp]
rel_object: real
t_can_carry: [T4, T3]
core_emotion_note: ""
core_conflict: 消防员在生死救援中维系团队情谊与个人成长

# === 三层标签(主检索路径,见 TAGS.md)===
theme_primary: 职场
theme_secondary: [医疗, 军旅, 友情, 成长, 救赎]
visual_tags: [消防车, 火场, 制服, 都市夜景, 救援现场]


# === 评分(v0 已有)===
acquisition_score: 3

# === 素材与玩法 ===
material_hooks: [消防车出动, 火场救援, 队友互助, 制服形象]
existing_games: []
recommended_playstyles: []
risk_notes: [受众窄, 玩法同质化]

# === 元数据 ===
source: deepseek-v4-flash + v0-migrate + tags
source_run_id: 2026-05-18-tags-enrich
evidence_level: B
review_status: auto
last_updated: 2026-05-18
# v0_duplicate_count: 3
---

## 迁移备注

本样本由 v0 markdown 表格迁移而来,LLM 阶段尚未补 SDT 三需求 / T 任务态 / core_conflict / material_hooks 重判。
迁移完成后 evidence_level = C,LLM 增量打标后升至 B。
