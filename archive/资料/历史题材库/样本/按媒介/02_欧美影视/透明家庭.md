---
# === 身份 ===
work_id: WORK-TVW-01089
work_name: 透明家庭
work_name_alt: []
media_type: TVW
year_first: 2016
year_range: [2016, 2016]
season_count: null

# === 三要素(v0 已有,迁移直翻)===
environment: 现代都市家庭
cultural_paradigm: LGBTQ+家庭伦理
narrative_core: 父亲变性后的家庭故事

# === SDT 心理需求(TBD,等 LLM 阶段补)===
n_main: N-Rel
n_others: []
rel_object: real
t_can_carry: [T3]
core_emotion_note: ""
core_conflict: 父亲变性后，家庭成员如何重新定义爱与身份

# === 三层标签(主检索路径,见 TAGS.md)===
theme_primary: 家庭
theme_secondary: [亲情, 都市, 喜剧, 身份认同, 代际冲突]
visual_tags: [都市夜景, 公寓, 家庭温情, 日常温馨]


# === 评分(v0 已有)===
acquisition_score: 3

# === 素材与玩法 ===
material_hooks: [变性父亲日常, 家庭聚餐对话, 孩子视角反应, 身份转变瞬间]
existing_games: []
recommended_playstyles: []
risk_notes: [受众窄, 敏感题材]

# === 元数据 ===
source: deepseek-v4-flash + v0-migrate + tags
source_run_id: 2026-05-18-tags-enrich
evidence_level: B
review_status: auto
last_updated: 2026-05-18
---

## 迁移备注

本样本由 v0 markdown 表格迁移而来,LLM 阶段尚未补 SDT 三需求 / T 任务态 / core_conflict / material_hooks 重判。
迁移完成后 evidence_level = C,LLM 增量打标后升至 B。
