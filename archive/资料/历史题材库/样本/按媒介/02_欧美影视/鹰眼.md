---
# === 身份 ===
work_id: WORK-TVW-00461
work_name: 鹰眼
work_name_alt: []
media_type: TVW
year_first: 2008
year_range: [2008, 2021]
season_count: null

# === 三要素(v0 已有,迁移直翻)===
environment: 现代都市/圣诞氛围
cultural_paradigm: 弓箭战术+师徒传承
narrative_core: 退休英雄训练新射手

# === SDT 心理需求(TBD,等 LLM 阶段补)===
n_main: N-Rel
n_others: [N-Comp]
rel_object: real
t_can_carry: [T3, T2]
core_emotion_note: ""
core_conflict: 退休英雄训练新射手，共同对抗街头犯罪

# === 三层标签(主检索路径,见 TAGS.md)===
theme_primary: 都市
theme_secondary: [犯罪, 成长]
visual_tags: [都市夜景, 弓箭特写, 圣诞街景, 高楼追击]


# === 评分(v0 已有)===
acquisition_score: 3

# === 素材与玩法 ===
material_hooks: [弓箭特写, 圣诞街景, 师徒对峙, 高楼追击]
existing_games: []
recommended_playstyles: []
risk_notes: [版权成本高, 玩法同质化]

# === 元数据 ===
source: deepseek-v4-flash + v0-migrate + tags
source_run_id: 2026-05-18-tags-enrich
evidence_level: B
review_status: auto
last_updated: 2026-05-18
# v0_duplicate_count: 3
---

## 迁移备注

本样本由 v0 markdown 表格迁移而来,LLM 阶段尚未补 SDT 三需求 / T 任务态 / core_conflict / material_hooks 重判。
迁移完成后 evidence_level = C,LLM 增量打标后升至 B。
