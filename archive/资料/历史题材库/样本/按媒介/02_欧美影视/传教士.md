---
# === 身份 ===
work_id: WORK-TVW-00436
work_name: 传教士
work_name_alt: []
media_type: TVW
year_first: 2016
year_range: [2016, 2016]
season_count: null

# === 三要素(v0 已有,迁移直翻)===
environment: 当代美国西部+超自然
cultural_paradigm: 宗教超能力+暴力美学
narrative_core: 寻找上帝与自我救赎

# === SDT 心理需求(TBD,等 LLM 阶段补)===
n_main: N-Comp
n_others: [N-Rel]
rel_object: real
t_can_carry: [T2, T4]
core_emotion_note: ""
core_conflict: 拥有上帝之力的传教士在西部对抗邪恶势力并寻找自我救赎

# === 三层标签(主检索路径,见 TAGS.md)===
theme_primary: 冒险
theme_secondary: [西部, 救赎]
visual_tags: [西部荒野, 枪林弹雨, 宗教符号, 超能力对决]


# === 评分(v0 已有)===
acquisition_score: 3

# === 素材与玩法 ===
material_hooks: [暴力枪战, 超能力对决, 西部荒野, 宗教符号]
existing_games: []
recommended_playstyles: []
risk_notes: [宗教敏感, 受众窄]

# === 元数据 ===
source: deepseek-v4-flash + v0-migrate + tags
source_run_id: 2026-05-18-tags-enrich
evidence_level: B
review_status: auto
last_updated: 2026-05-18
# v0_duplicate_count: 2
---

## 迁移备注

本样本由 v0 markdown 表格迁移而来,LLM 阶段尚未补 SDT 三需求 / T 任务态 / core_conflict / material_hooks 重判。
迁移完成后 evidence_level = C,LLM 增量打标后升至 B。
