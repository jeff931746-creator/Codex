---
# === 身份 ===
work_id: WORK-TVW-01150
work_name: 糊涂侦探
work_name_alt: []
media_type: TVW
year_first: 2008
year_range: [2008, 2008]
season_count: null

# === 三要素(v0 已有,迁移直翻)===
environment: 现代都市/特工世界
cultural_paradigm: 搞笑特工+高科技装备
narrative_core: 菜鸟逆袭+搞笑冒险

# === SDT 心理需求(TBD,等 LLM 阶段补)===
n_main: N-Comp
n_others: []
rel_object: null
t_can_carry: [T1, T2]
core_emotion_note: ""
core_conflict: 菜鸟特工用搞笑方式完成危险任务，证明自己能力

# === 三层标签(主检索路径,见 TAGS.md)===
theme_primary: 喜剧
theme_secondary: [都市, 特工任务, 逆袭, 成长]
visual_tags: [都市夜景, 高科技装备, 搞笑荒诞, 黑色西装]


# === 评分(v0 已有)===
acquisition_score: 3

# === 素材与玩法 ===
material_hooks: [搞笑装备展示, 笨拙特工动作, 高科技道具乱用]
existing_games: []
recommended_playstyles: []
risk_notes: [受众窄, IP热度低]

# === 元数据 ===
source: deepseek-v4-flash + v0-migrate + tags
source_run_id: 2026-05-18-tags-enrich
evidence_level: B
review_status: auto
last_updated: 2026-05-18
---

## 迁移备注

本样本由 v0 markdown 表格迁移而来,LLM 阶段尚未补 SDT 三需求 / T 任务态 / core_conflict / material_hooks 重判。
迁移完成后 evidence_level = C,LLM 增量打标后升至 B。
