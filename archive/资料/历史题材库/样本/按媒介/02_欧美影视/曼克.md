---
# === 身份 ===
work_id: WORK-TVW-01294
work_name: 曼克
work_name_alt: []
media_type: TVW
year_first: 2020
year_range: [2020, 2020]
season_count: null

# === 三要素(v0 已有,迁移直翻)===
environment: 1930年代好莱坞
cultural_paradigm: 编剧文化+电影工业
narrative_core: 创作博弈与回忆

# === SDT 心理需求(TBD,等 LLM 阶段补)===
n_main: N-Comp
n_others: []
rel_object: null
t_can_carry: [T5]
core_emotion_note: ""
core_conflict: 编剧在好莱坞体制中挣扎创作，与资本和回忆博弈

# === 三层标签(主检索路径,见 TAGS.md)===
theme_primary: 传记
theme_secondary: [职场, 历史, 都市, 成长]
visual_tags: [黑白胶片质感, 1930年代片场, 打字机特写, 酒会觥筹交错]


# === 评分(v0 已有)===
acquisition_score: 3

# === 素材与玩法 ===
material_hooks: [黑白胶片质感, 1930年代片场, 打字机特写, 酒会觥筹交错]
existing_games: []
recommended_playstyles: []
risk_notes: [受众极窄, 付费浅, 无动作元素]

# === 元数据 ===
source: deepseek-v4-flash + v0-migrate + tags
source_run_id: 2026-05-18-tags-enrich
evidence_level: B
review_status: auto
last_updated: 2026-05-18
---

## 迁移备注

本样本由 v0 markdown 表格迁移而来,LLM 阶段尚未补 SDT 三需求 / T 任务态 / core_conflict / material_hooks 重判。
迁移完成后 evidence_level = C,LLM 增量打标后升至 B。
