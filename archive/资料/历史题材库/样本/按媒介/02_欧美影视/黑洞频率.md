---
# === 身份 ===
work_id: WORK-TVW-00721
work_name: 黑洞频率
work_name_alt: []
media_type: TVW
year_first: 2000
year_range: [2000, 2000]
season_count: null

# === 三要素(v0 已有,迁移直翻)===
environment: 现代纽约+时空穿越
cultural_paradigm: 科幻穿越+亲情悬疑
narrative_core: 拯救亲人+改变历史

# === SDT 心理需求(TBD,等 LLM 阶段补)===
n_main: N-Rel
n_others: []
rel_object: real
t_can_carry: [T3, T5]
core_emotion_note: ""
core_conflict: 通过无线电跨越时空拯救父亲，同时修正历史连锁反应

# === 三层标签(主检索路径,见 TAGS.md)===
theme_primary: 科幻
theme_secondary: [亲情, 悬疑, 时间旅行, 救赎]
visual_tags: [都市夜景, 暴风雨夜, 无线电设备, 家庭温馨]


# === 评分(v0 已有)===
acquisition_score: 3

# === 素材与玩法 ===
material_hooks: [无线电通话, 父子隔空对话, 历史照片变化, 暴风雨夜]
existing_games: []
recommended_playstyles: []
risk_notes: [玩法创新难, 付费深度低]

# === 元数据 ===
source: deepseek-v4-flash + v0-migrate + tags
source_run_id: 2026-05-18-tags-enrich
evidence_level: B
review_status: auto
last_updated: 2026-05-18
# v0_duplicate_count: 2
---

## 迁移备注

本样本由 v0 markdown 表格迁移而来,LLM 阶段尚未补 SDT 三需求 / T 任务态 / core_conflict / material_hooks 重判。
迁移完成后 evidence_level = C,LLM 增量打标后升至 B。
