---
# === 身份 ===
work_id: WORK-TVW-00308
work_name: 死侍
work_name_alt: []
media_type: TVW
year_first: 2016
year_range: [2016, 2016]
season_count: null

# === 三要素(v0 已有,迁移直翻)===
environment: 现代超级英雄世界
cultural_paradigm: 超级英雄+嘴炮恶搞
narrative_core: 雇佣兵复仇与搞笑

# === SDT 心理需求(TBD,等 LLM 阶段补)===
n_main: N-Comp
n_others: []
rel_object: null
t_can_carry: [T2, T1]
core_emotion_note: ""
core_conflict: 嘴炮雇佣兵死侍在复仇与搞笑中打破第四面墙

# === 三层标签(主检索路径,见 TAGS.md)===
theme_primary: 喜剧
theme_secondary: [复仇, 黑色幽默]
visual_tags: [红色紧身衣, 双刀战斗, 都市夜景, 爆炸场面, 搞笑荒诞]


# === 评分(v0 已有)===
acquisition_score: 4

# === 素材与玩法 ===
material_hooks: [死侍面具表情, 双刀战斗, 红色紧身衣, 第四面墙打破]
existing_games: [死侍游戏]
recommended_playstyles: []
risk_notes: [成人向, 版权成本高]

# === 元数据 ===
source: deepseek-v4-flash + v0-migrate + tags
source_run_id: 2026-05-18-tags-enrich
evidence_level: B
review_status: auto
last_updated: 2026-05-18
# v0_duplicate_count: 2
---

## 迁移备注

本样本由 v0 markdown 表格迁移而来,LLM 阶段尚未补 SDT 三需求 / T 任务态 / core_conflict / material_hooks 重判。
迁移完成后 evidence_level = C,LLM 增量打标后升至 B。
