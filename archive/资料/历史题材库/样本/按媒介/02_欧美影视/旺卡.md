---
# === 身份 ===
work_id: WORK-TVW-00578
work_name: 旺卡
work_name_alt: []
media_type: TVW
year_first: 2023
year_range: [2023, 2023]
season_count: null

# === 三要素(v0 已有,迁移直翻)===
environment: 奇幻欧洲小镇
cultural_paradigm: 魔法巧克力+童话冒险
narrative_core: 创业与梦想

# === SDT 心理需求(TBD,等 LLM 阶段补)===
n_main: N-Auto
n_others: [N-Rel]
rel_object: mixed
t_can_carry: [T1, T6]
core_emotion_note: ""
core_conflict: 年轻巧克力师用魔法糖果对抗垄断，实现梦想与分享快乐

# === 三层标签(主检索路径,见 TAGS.md)===
theme_primary: 奇幻
theme_secondary: [冒险, 喜剧, 都市, 成长]
visual_tags: [奇幻瑰丽, 魔法糖果店, 彩色小镇, 巧克力瀑布]


# === 评分(v0 已有)===
acquisition_score: 4

# === 素材与玩法 ===
material_hooks: [巧克力瀑布, 魔法糖果店, 彩色小镇, 旺卡微笑, 奇幻配方]
existing_games: []
recommended_playstyles: []
risk_notes: [版权成本高, 付费浅]

# === 元数据 ===
source: deepseek-v4-flash + v0-migrate + tags
source_run_id: 2026-05-18-tags-enrich
evidence_level: B
review_status: auto
last_updated: 2026-05-18
---

## 迁移备注

本样本由 v0 markdown 表格迁移而来,LLM 阶段尚未补 SDT 三需求 / T 任务态 / core_conflict / material_hooks 重判。
迁移完成后 evidence_level = C,LLM 增量打标后升至 B。
