---
# === 身份 ===
work_id: WORK-TVW-00066
work_name: 美国队长3
work_name_alt: []
media_type: TVW
year_first: 2016
year_range: [2016, 2016]
season_count: null

# === 三要素(v0 已有,迁移直翻)===
environment: 现代超级英雄世界
cultural_paradigm: 超级英雄+科技异能
narrative_core: 英雄阵营分裂内战

# === SDT 心理需求(TBD,等 LLM 阶段补)===
n_main: N-Rel
n_others: [N-Comp]
rel_object: real
t_can_carry: [T4, T2]
core_emotion_note: ""
core_conflict: 超级英雄因监管法案分裂为两派，钢铁侠与美队阵营对抗

# === 三层标签(主检索路径,见 TAGS.md)===
theme_primary: 战争
theme_secondary: [政治权谋, 科幻, 冒险, 友情, 复仇]
visual_tags: [都市夜景, 机甲, 爆炸场面, 武器对决, 战场残酷]


# === 评分(v0 已有)===
acquisition_score: 4

# === 素材与玩法 ===
material_hooks: [机场大战, 美队盾牌对钢铁侠, 冬兵铁臂, 英雄分裂对峙]
existing_games: [漫威复仇者联盟系列]
recommended_playstyles: []
risk_notes: [版权成本高, 红海]

# === 元数据 ===
source: deepseek-v4-flash + v0-migrate + tags
source_run_id: 2026-05-18-tags-enrich
evidence_level: B
review_status: auto
last_updated: 2026-05-18
---

## 迁移备注

本样本由 v0 markdown 表格迁移而来,LLM 阶段尚未补 SDT 三需求 / T 任务态 / core_conflict / material_hooks 重判。
迁移完成后 evidence_level = C,LLM 增量打标后升至 B。
