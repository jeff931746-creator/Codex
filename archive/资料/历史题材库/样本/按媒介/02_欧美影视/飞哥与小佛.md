---
# === 身份 ===
work_id: WORK-TVW-00403
work_name: 飞哥与小佛
work_name_alt: []
media_type: TVW
year_first: 2007
year_range: [2007, 2007]
season_count: null

# === 三要素(v0 已有,迁移直翻)===
environment: 现代都市暑假日常
cultural_paradigm: 发明创造+冒险搞笑
narrative_core: 兄弟疯狂发明与冒险

# === SDT 心理需求(TBD,等 LLM 阶段补)===
n_main: N-Auto
n_others: []
rel_object: null
t_can_carry: [T1]
core_emotion_note: ""
core_conflict: 兄弟用发明创造对抗无聊暑假，同时躲避妈妈的发现

# === 三层标签(主检索路径,见 TAGS.md)===
theme_primary: 喜剧
theme_secondary: [日常喜剧, 冒险, 家庭, 成长]
visual_tags: [校园, 都市日常, 搞笑荒诞, 发明创造]


# === 评分(v0 已有)===
acquisition_score: 4

# === 素材与玩法 ===
material_hooks: [疯狂发明, 夏日冒险, 秘密行动, 搞笑日常]
existing_games: []
recommended_playstyles: []
risk_notes: [版权成本高, 受众低龄]

# === 元数据 ===
source: deepseek-v4-flash + v0-migrate + tags
source_run_id: 2026-05-18-tags-enrich
evidence_level: B
review_status: auto
last_updated: 2026-05-18
---

## 迁移备注

本样本由 v0 markdown 表格迁移而来,LLM 阶段尚未补 SDT 三需求 / T 任务态 / core_conflict / material_hooks 重判。
迁移完成后 evidence_level = C,LLM 增量打标后升至 B。
