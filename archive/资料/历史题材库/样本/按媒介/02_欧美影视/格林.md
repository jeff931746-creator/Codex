---
# === 身份 ===
work_id: WORK-TVW-00424
work_name: 格林
work_name_alt: []
media_type: TVW
year_first: 2011
year_range: [2011, 2013]
season_count: null

# === 三要素(v0 已有,迁移直翻)===
environment: 现代都市+童话暗黑世界
cultural_paradigm: 格林猎人+童话怪物体系
narrative_core: 猎魔+身份秘密

# === SDT 心理需求(TBD,等 LLM 阶段补)===
n_main: N-Comp
n_others: [N-Rel]
rel_object: mixed
t_can_carry: [T5, T2]
core_emotion_note: ""
core_conflict: 格林猎人对抗童话怪物，同时守护身份秘密

# === 三层标签(主检索路径,见 TAGS.md)===
theme_primary: 悬疑
theme_secondary: [都市, 奇幻, 恐怖, 身份谜团]
visual_tags: [都市夜景, 古堡, 怪物造型猎奇, 暗黑童话反转, 猎魔战斗场面]


# === 评分(v0 已有)===
acquisition_score: 3

# === 素材与玩法 ===
material_hooks: [怪物造型猎奇, 猎魔战斗场面, 格林生物图鉴, 暗黑童话反转]
existing_games: []
recommended_playstyles: []
risk_notes: [版权成本高, 受众中等]

# === 元数据 ===
source: deepseek-v4-flash + v0-migrate + tags
source_run_id: 2026-05-18-tags-enrich
evidence_level: B
review_status: auto
last_updated: 2026-05-18
# v0_duplicate_count: 2
---

## 迁移备注

本样本由 v0 markdown 表格迁移而来,LLM 阶段尚未补 SDT 三需求 / T 任务态 / core_conflict / material_hooks 重判。
迁移完成后 evidence_level = C,LLM 增量打标后升至 B。
