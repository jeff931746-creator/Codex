---
# === 身份 ===
work_id: WORK-TVW-01194
work_name: 好小子们
work_name_alt: []
media_type: TVW
year_first: 2019
year_range: [2019, 2019]
season_count: null

# === 三要素(v0 已有,迁移直翻)===
environment: 现代美国郊区小镇
cultural_paradigm: 儿童冒险+恶搞喜剧
narrative_core: 疯狂一天与成长友谊

# === SDT 心理需求(TBD,等 LLM 阶段补)===
n_main: N-Rel
n_others: [N-Auto]
rel_object: virtual
t_can_carry: [T3, T1]
core_emotion_note: ""
core_conflict: 三个男孩在疯狂的一天中维护友谊并逃避大人的惩罚

# === 三层标签(主检索路径,见 TAGS.md)===
theme_primary: 喜剧
theme_secondary: [日常喜剧, 友情, 冒险, 成长]
visual_tags: [校园, 小镇日常, 搞笑荒诞, 都市夜景]


# === 评分(v0 已有)===
acquisition_score: 2

# === 素材与玩法 ===
material_hooks: [儿童恶搞场景, 荒诞冒险画面, 三人组互动, 小镇日常反差]
existing_games: []
recommended_playstyles: []
risk_notes: [受众窄, 文化差异大]

# === 元数据 ===
source: deepseek-v4-flash + v0-migrate + tags
source_run_id: 2026-05-18-tags-enrich
evidence_level: B
review_status: auto
last_updated: 2026-05-18
---

## 迁移备注

本样本由 v0 markdown 表格迁移而来,LLM 阶段尚未补 SDT 三需求 / T 任务态 / core_conflict / material_hooks 重判。
迁移完成后 evidence_level = C,LLM 增量打标后升至 B。
