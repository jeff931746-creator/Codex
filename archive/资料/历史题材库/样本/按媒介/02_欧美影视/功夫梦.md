---
# === 身份 ===
work_id: WORK-TVW-00771
work_name: 功夫梦
work_name_alt: []
media_type: TVW
year_first: 2010
year_range: [2010, 2010]
season_count: null

# === 三要素(v0 已有,迁移直翻)===
environment: 当代中国城市
cultural_paradigm: 中国功夫+师徒传承
narrative_core: 异国少年逆袭

# === SDT 心理需求(TBD,等 LLM 阶段补)===
n_main: N-Comp
n_others: []
rel_object: null
t_can_carry: [T2]
core_emotion_note: ""
core_conflict: 异国少年通过学功夫战胜欺凌，实现自我成长

# === 三层标签(主检索路径,见 TAGS.md)===
theme_primary: 都市
theme_secondary: [成长, 武侠, 校园, 友情]
visual_tags: [都市夜景, 功夫对决, 校园, 师徒互动]


# === 评分(v0 已有)===
acquisition_score: 3

# === 素材与玩法 ===
material_hooks: [少年练功场景, 功夫对决, 师徒互动, 城市街头打斗]
existing_games: []
recommended_playstyles: []
risk_notes: [文化差异, 受众窄]

# === 元数据 ===
source: deepseek-v4-flash + v0-migrate + tags
source_run_id: 2026-05-18-tags-enrich
evidence_level: B
review_status: auto
last_updated: 2026-05-18
---

## 迁移备注

本样本由 v0 markdown 表格迁移而来,LLM 阶段尚未补 SDT 三需求 / T 任务态 / core_conflict / material_hooks 重判。
迁移完成后 evidence_level = C,LLM 增量打标后升至 B。
