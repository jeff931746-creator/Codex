---
# === 身份 ===
work_id: WORK-TVW-00473
work_name: 末日地堡
work_name_alt: []
media_type: TVW
year_first: 2023
year_range: [2023, 2025]
season_count: null

# === 三要素(v0 已有,迁移直翻)===
environment: 地下筒仓反乌托邦
cultural_paradigm: 封闭社会生存法则
narrative_core: 生存探索+真相追寻

# === SDT 心理需求(TBD,等 LLM 阶段补)===
n_main: N-Comp
n_others: []
rel_object: null
t_can_carry: [T2, T5]
core_emotion_note: ""
core_conflict: 地下筒仓居民在封闭环境中求生并追寻外界真相

# === 三层标签(主检索路径,见 TAGS.md)===
theme_primary: 末日
theme_secondary: [求生, 悬疑, 文明崩溃]
visual_tags: [废土, 地下基地, 昏暗灯光, 金属走廊, 防护服]


# === 评分(v0 已有)===
acquisition_score: 3

# === 素材与玩法 ===
material_hooks: [筒仓金属走廊, 昏暗灯光环境, 防护服与头盔, 地底爆炸场景, 逃生舱门]
existing_games: []
recommended_playstyles: []
risk_notes: [题材偏冷, 视觉色调单一]

# === 元数据 ===
source: deepseek-v4-flash + v0-migrate + tags
source_run_id: 2026-05-18-tags-enrich
evidence_level: B
review_status: auto
last_updated: 2026-05-18
# v0_duplicate_count: 2
---

## 迁移备注

本样本由 v0 markdown 表格迁移而来,LLM 阶段尚未补 SDT 三需求 / T 任务态 / core_conflict / material_hooks 重判。
迁移完成后 evidence_level = C,LLM 增量打标后升至 B。
