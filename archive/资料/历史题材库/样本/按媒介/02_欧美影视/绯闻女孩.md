---
# === 身份 ===
work_id: WORK-TVW-00398
work_name: 绯闻女孩
work_name_alt: []
media_type: TVW
year_first: 2007
year_range: [2007, 2008]
season_count: null

# === 三要素(v0 已有,迁移直翻)===
environment: 纽约上东区/现代
cultural_paradigm: 青少年社交+时尚文化
narrative_core: 社交秘闻+权力博弈

# === SDT 心理需求(TBD,等 LLM 阶段补)===
n_main: N-Rel
n_others: []
rel_object: real
t_can_carry: [T4, T3]
core_emotion_note: ""
core_conflict: 上东区青少年通过社交秘闻争夺群体地位与权力

# === 三层标签(主检索路径,见 TAGS.md)===
theme_primary: 都市
theme_secondary: [校园, 恋爱, 友情, 家庭]
visual_tags: [都市繁华, 校园青春, 霓虹灯, 摩天楼, 时尚穿搭]


# === 评分(v0 已有)===
acquisition_score: 4

# === 素材与玩法 ===
material_hooks: [上东区奢华派对, 角色时尚穿搭, 八卦短信截图, 校园权力对峙]
existing_games: []
recommended_playstyles: []
risk_notes: [受众窄, 付费深度中等]

# === 元数据 ===
source: deepseek-v4-flash + v0-migrate + tags
source_run_id: 2026-05-18-tags-enrich
evidence_level: B
review_status: auto
last_updated: 2026-05-18
# v0_duplicate_count: 3
---

## 迁移备注

本样本由 v0 markdown 表格迁移而来,LLM 阶段尚未补 SDT 三需求 / T 任务态 / core_conflict / material_hooks 重判。
迁移完成后 evidence_level = C,LLM 增量打标后升至 B。
