---
# === 身份 ===
work_id: WORK-TVW-00592
work_name: F1》（2025，欧美，F1赛车手传奇）
work_name_alt: []
media_type: TVW
year_first: 2025
year_range: [2025, 2025]
season_count: null

# === 三要素(v0 已有,迁移直翻)===
environment: 现代F1赛道世界
cultural_paradigm: 赛车竞技+职业体育
narrative_core: 职业成长+竞速夺冠

# === SDT 心理需求(TBD,等 LLM 阶段补)===
n_main: N-Comp
n_others: []
rel_object: null
t_can_carry: [T2, T1]
core_emotion_note: ""
core_conflict: 车手在极限速度中突破自我，争夺F1世界冠军

# === 三层标签(主检索路径,见 TAGS.md)===
theme_primary: 体育竞技
theme_secondary: [赛车, 职场, 传记]
visual_tags: [赛道, 赛车高速过弯, 引擎轰鸣声, 领奖台香槟]


# === 评分(v0 已有)===
acquisition_score: 4

# === 素材与玩法 ===
material_hooks: [赛车高速过弯, 引擎轰鸣声, 赛道超车瞬间, 维修区团队协作, 领奖台香槟]
existing_games: []
recommended_playstyles: []
risk_notes: [版权成本高, 受众窄]

# === 元数据 ===
source: deepseek-v4-flash + v0-migrate + tags
source_run_id: 2026-05-18-tags-enrich
evidence_level: B
review_status: auto
last_updated: 2026-05-18
---

## 迁移备注

本样本由 v0 markdown 表格迁移而来,LLM 阶段尚未补 SDT 三需求 / T 任务态 / core_conflict / material_hooks 重判。
迁移完成后 evidence_level = C,LLM 增量打标后升至 B。
