---
# === 身份 ===
work_id: WORK-TVW-00959
work_name: 华尔街：金钱永不眠
work_name_alt: []
media_type: TVW
year_first: 2010
year_range: [2010, 2010]
season_count: null

# === 三要素(v0 已有,迁移直翻)===
environment: 现代金融世界
cultural_paradigm: 金融博弈+资本战争
narrative_core: 商战复仇+金钱博弈

# === SDT 心理需求(TBD,等 LLM 阶段补)===
n_main: N-Rel
n_others: [N-Comp]
rel_object: real
t_can_carry: [T4, T2]
core_emotion_note: ""
core_conflict: 金融精英在资本战场上复仇与权力博弈

# === 三层标签(主检索路径,见 TAGS.md)===
theme_primary: 职场
theme_secondary: [商战, 复仇, 权力博弈, 都市]
visual_tags: [都市夜景, 摩天楼, 西装革履, 金融交易大厅]


# === 评分(v0 已有)===
acquisition_score: 3

# === 素材与玩法 ===
material_hooks: [华尔街交易大厅, 西装革履对峙, 金钱数字跳动, 高楼俯瞰城市]
existing_games: []
recommended_playstyles: []
risk_notes: [受众窄, 红海]

# === 元数据 ===
source: deepseek-v4-flash + v0-migrate + tags
source_run_id: 2026-05-18-tags-enrich
evidence_level: B
review_status: auto
last_updated: 2026-05-18
# v0_duplicate_count: 2
---

## 迁移备注

本样本由 v0 markdown 表格迁移而来,LLM 阶段尚未补 SDT 三需求 / T 任务态 / core_conflict / material_hooks 重判。
迁移完成后 evidence_level = C,LLM 增量打标后升至 B。
