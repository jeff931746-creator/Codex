---
# === 身份 ===
work_id: WORK-TVW-00336
work_name: 信条
work_name_alt: []
media_type: TVW
year_first: 2020
year_range: [2020, 2020]
season_count: null

# === 三要素(v0 已有,迁移直翻)===
environment: 现代全球+时间逆流
cultural_paradigm: 时间逆转+特工科技
narrative_core: 逆转时空拯救世界

# === SDT 心理需求(TBD,等 LLM 阶段补)===
n_main: N-Comp
n_others: []
rel_object: null
t_can_carry: [T5, T2]
core_emotion_note: ""
core_conflict: 主角在时间逆流中阻止未来反派毁灭世界

# === 三层标签(主检索路径,见 TAGS.md)===
theme_primary: 科幻
theme_secondary: [时间旅行, 未来战争, 阴谋论]
visual_tags: [时间逆流特效, 旋转门装置, 公路追车逆流, 红蓝队战术, 都市夜景]


# === 评分(v0 已有)===
acquisition_score: 4

# === 素材与玩法 ===
material_hooks: [时间逆流特效, 旋转门装置, 公路追车逆流, 红蓝队战术]
existing_games: []
recommended_playstyles: []
risk_notes: [版权成本高, 玩法设计难]

# === 元数据 ===
source: deepseek-v4-flash + v0-migrate + tags
source_run_id: 2026-05-18-tags-enrich
evidence_level: B
review_status: auto
last_updated: 2026-05-18
---

## 迁移备注

本样本由 v0 markdown 表格迁移而来,LLM 阶段尚未补 SDT 三需求 / T 任务态 / core_conflict / material_hooks 重判。
迁移完成后 evidence_level = C,LLM 增量打标后升至 B。
