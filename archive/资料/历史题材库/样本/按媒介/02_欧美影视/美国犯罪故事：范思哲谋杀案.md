---
# === 身份 ===
work_id: WORK-TVW-01097
work_name: 美国犯罪故事：范思哲谋杀案
work_name_alt: []
media_type: TVW
year_first: 2018
year_range: [2018, 2018]
season_count: null

# === 三要素(v0 已有,迁移直翻)===
environment: 90年代时尚界
cultural_paradigm: 真实犯罪+时尚圈层
narrative_core: 谋杀真相+人性剖析

# === SDT 心理需求(TBD,等 LLM 阶段补)===
n_main: N-Rel
n_others: [N-Comp]
rel_object: real
t_can_carry: [T4, T5]
core_emotion_note: ""
core_conflict: 时尚界权力博弈与谋杀真相交织

# === 三层标签(主检索路径,见 TAGS.md)===
theme_primary: 犯罪
theme_secondary: [连环杀手, 传记, 悬疑, 都市]
visual_tags: [都市夜景, 霓虹灯, 摩天楼, 古风装扮]


# === 评分(v0 已有)===
acquisition_score: 3

# === 素材与玩法 ===
material_hooks: [范思哲豪宅, 华丽时装秀, 谋杀现场, 人物对峙]
existing_games: []
recommended_playstyles: []
risk_notes: [版权成本高, 受众窄, 非战斗向]

# === 元数据 ===
source: deepseek-v4-flash + v0-migrate + tags
source_run_id: 2026-05-18-tags-enrich
evidence_level: B
review_status: auto
last_updated: 2026-05-18
---

## 迁移备注

本样本由 v0 markdown 表格迁移而来,LLM 阶段尚未补 SDT 三需求 / T 任务态 / core_conflict / material_hooks 重判。
迁移完成后 evidence_level = C,LLM 增量打标后升至 B。
