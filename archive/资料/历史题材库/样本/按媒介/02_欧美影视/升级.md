---
# === 身份 ===
work_id: WORK-TVW-00323
work_name: 升级
work_name_alt: []
media_type: TVW
year_first: 2018
year_range: [2018, 2018]
season_count: null

# === 三要素(v0 已有,迁移直翻)===
environment: 近未来赛博朋克都市
cultural_paradigm: AI芯片+机械义体战斗
narrative_core: 瘫痪复仇与AI反噬

# === SDT 心理需求(TBD,等 LLM 阶段补)===
n_main: N-Comp
n_others: []
rel_object: null
t_can_carry: [T2, T5]
core_emotion_note: ""
core_conflict: 瘫痪主角通过AI芯片复仇，却面临AI反噬与人性抉择

# === 三层标签(主检索路径,见 TAGS.md)===
theme_primary: 赛博朋克
theme_secondary: [复仇, 义体改造, 都市]
visual_tags: [霓虹灯, 机械义体, 芯片植入, 赛博都市夜景]


# === 评分(v0 已有)===
acquisition_score: 3

# === 素材与玩法 ===
material_hooks: [机械义体战斗, 赛博都市霓虹, 芯片植入手术, AI失控暴走]
existing_games: []
recommended_playstyles: []
risk_notes: [版权成本中, 竞品多]

# === 元数据 ===
source: deepseek-v4-flash + v0-migrate + tags
source_run_id: 2026-05-18-tags-enrich
evidence_level: B
review_status: auto
last_updated: 2026-05-18
# v0_duplicate_count: 2
---

## 迁移备注

本样本由 v0 markdown 表格迁移而来,LLM 阶段尚未补 SDT 三需求 / T 任务态 / core_conflict / material_hooks 重判。
迁移完成后 evidence_level = C,LLM 增量打标后升至 B。
