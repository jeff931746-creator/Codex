---
# === 身份 ===
work_id: WORK-TVW-00559
work_name: 安娜贝尔3：回家
work_name_alt: []
media_type: TVW
year_first: 2019
year_range: [2019, 2019]
season_count: null

# === 三要素(v0 已有,迁移直翻)===
environment: 现代美国鬼屋博物馆
cultural_paradigm: 鬼娃+驱魔体系
narrative_core: 鬼物逃逸与收容

# === SDT 心理需求(TBD,等 LLM 阶段补)===
n_main: N-Comp
n_others: []
rel_object: null
t_can_carry: [T5, T1]
core_emotion_note: ""
core_conflict: 鬼屋博物馆中鬼物逃逸，主角必须收容并生存

# === 三层标签(主检索路径,见 TAGS.md)===
theme_primary: 恐怖
theme_secondary: [灵异, 超自然悬疑, 生存恐怖, 家庭秘密]
visual_tags: [古堡, 黑暗压抑, 鬼娃, 驱魔仪式]


# === 评分(v0 已有)===
acquisition_score: 3

# === 素材与玩法 ===
material_hooks: [鬼娃安娜贝尔, 黑暗走廊, 鬼物逃逸, 驱魔仪式]
existing_games: []
recommended_playstyles: []
risk_notes: [版权成本高]

# === 元数据 ===
source: deepseek-v4-flash + v0-migrate + tags
source_run_id: 2026-05-18-tags-enrich
evidence_level: B
review_status: auto
last_updated: 2026-05-18
# v0_duplicate_count: 2
---

## 迁移备注

本样本由 v0 markdown 表格迁移而来,LLM 阶段尚未补 SDT 三需求 / T 任务态 / core_conflict / material_hooks 重判。
迁移完成后 evidence_level = C,LLM 增量打标后升至 B。
