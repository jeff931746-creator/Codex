---
# === 身份 ===
work_id: WORK-TVW-00911
work_name: 孽扣
work_name_alt: []
media_type: TVW
year_first: 2019
year_range: [2019, 2019]
season_count: null

# === 三要素(v0 已有,迁移直翻)===
environment: 现代美国
cultural_paradigm: 杀手学校+复仇暗黑
narrative_core: 少年复仇与成长

# === SDT 心理需求(TBD,等 LLM 阶段补)===
n_main: N-Comp
n_others: []
rel_object: null
t_can_carry: [T2]
core_emotion_note: ""
core_conflict: 少年在杀手学校中复仇与成长，挣扎于暴力与人性之间

# === 三层标签(主检索路径,见 TAGS.md)===
theme_primary: 犯罪
theme_secondary: [复仇, 成长, 校园]
visual_tags: [暗黑压抑, 残酷训练, 暴力打斗, 都市夜景]


# === 评分(v0 已有)===
acquisition_score: 2

# === 素材与玩法 ===
material_hooks: [暗黑复仇场景, 残酷训练画面, 暴力美学打斗]
existing_games: []
recommended_playstyles: []
risk_notes: [受众窄, 暴力内容限制]

# === 元数据 ===
source: deepseek-v4-flash + v0-migrate + tags
source_run_id: 2026-05-18-tags-enrich
evidence_level: B
review_status: auto
last_updated: 2026-05-18
---

## 迁移备注

本样本由 v0 markdown 表格迁移而来,LLM 阶段尚未补 SDT 三需求 / T 任务态 / core_conflict / material_hooks 重判。
迁移完成后 evidence_level = C,LLM 增量打标后升至 B。
