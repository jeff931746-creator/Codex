---
# === 身份 ===
work_id: WORK-TVW-01018
work_name: 非礼勿言
work_name_alt: []
media_type: TVW
year_first: 2024
year_range: [2024, 2024]
season_count: null

# === 三要素(v0 已有,迁移直翻)===
environment: 现代度假乡村
cultural_paradigm: 心理恐怖+社交陷阱
narrative_core: 信任崩塌与生存反击

# === SDT 心理需求(TBD,等 LLM 阶段补)===
n_main: N-Comp
n_others: []
rel_object: null
t_can_carry: [T5]
core_emotion_note: ""
core_conflict: 主角在度假中陷入精心设计的社交陷阱，信任崩塌后必须反击求生

# === 三层标签(主检索路径,见 TAGS.md)===
theme_primary: 恐怖
theme_secondary: [心理恐怖, 求生, 悬疑, 都市]
visual_tags: [乡村度假村, 诡异氛围, 心理压迫, 陷阱场景]


# === 评分(v0 已有)===
acquisition_score: 3

# === 素材与玩法 ===
material_hooks: [度假村诡异氛围, 角色表情特写, 陷阱场景反转, 心理压迫镜头]
existing_games: []
recommended_playstyles: []
risk_notes: [受众窄, 同质化]

# === 元数据 ===
source: deepseek-v4-flash + v0-migrate + tags
source_run_id: 2026-05-18-tags-enrich
evidence_level: B
review_status: auto
last_updated: 2026-05-18
---

## 迁移备注

本样本由 v0 markdown 表格迁移而来,LLM 阶段尚未补 SDT 三需求 / T 任务态 / core_conflict / material_hooks 重判。
迁移完成后 evidence_level = C,LLM 增量打标后升至 B。
