---
# === 身份 ===
work_id: WORK-TVW-00387
work_name: 劫后新生
work_name_alt: []
media_type: TVW
year_first: 2003
year_range: [2003, 2003]
season_count: null

# === 三要素(v0 已有,迁移直翻)===
environment: 现代FBI卧底世界
cultural_paradigm: 特工训练+犯罪心理
narrative_core: 卧底探员训练

# === SDT 心理需求(TBD,等 LLM 阶段补)===
n_main: N-Comp
n_others: []
rel_object: null
t_can_carry: [T2, T5]
core_emotion_note: ""
core_conflict: 卧底探员在犯罪组织中隐藏身份并完成任务

# === 三层标签(主检索路径,见 TAGS.md)===
theme_primary: 犯罪
theme_secondary: [心理悬疑, 成长]
visual_tags: [都市夜景, 枪林弹雨, 审讯室, 黑色西装, 紧张对峙]


# === 评分(v0 已有)===
acquisition_score: 3

# === 素材与玩法 ===
material_hooks: [卧底身份切换, 犯罪现场对峙, 心理博弈审讯, 枪战追车场面]
existing_games: []
recommended_playstyles: []
risk_notes: [红海竞争, 受众中等]

# === 元数据 ===
source: deepseek-v4-flash + v0-migrate + tags
source_run_id: 2026-05-18-tags-enrich
evidence_level: B
review_status: auto
last_updated: 2026-05-18
---

## 迁移备注

本样本由 v0 markdown 表格迁移而来,LLM 阶段尚未补 SDT 三需求 / T 任务态 / core_conflict / material_hooks 重判。
迁移完成后 evidence_level = C,LLM 增量打标后升至 B。
