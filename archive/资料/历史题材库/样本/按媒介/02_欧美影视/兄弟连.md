---
# === 身份 ===
work_id: WORK-TVW-00002
work_name: 兄弟连
work_name_alt: []
media_type: TVW
year_first: 2001
year_range: [2001, 2001]
season_count: null

# === 三要素(v0 已有,迁移直翻)===
environment: 二战欧洲战场
cultural_paradigm: 军事战术+兄弟情谊
narrative_core: 二战小队作战与牺牲

# === SDT 心理需求(TBD,等 LLM 阶段补)===
n_main: N-Rel
n_others: [N-Comp]
rel_object: real
t_can_carry: [T4, T3]
core_emotion_note: ""
core_conflict: 二战E连士兵在残酷战场上并肩作战，面对生死与忠诚的考验

# === 三层标签(主检索路径,见 TAGS.md)===
theme_primary: 战争
theme_secondary: [二战, 兄弟情, 求生]
visual_tags: [战场残酷, 阵地, 枪林弹雨, 雪原]


# === 评分(v0 已有)===
acquisition_score: 4

# === 素材与玩法 ===
material_hooks: [诺曼底登陆战, 坦克炮火轰鸣, 战壕近身肉搏, 兄弟并肩冲锋, 雪地狙击场景]
existing_games: [Brothers in Arms]
recommended_playstyles: []
risk_notes: [版权成本高]

# === 元数据 ===
source: deepseek-v4-flash + v0-migrate + tags
source_run_id: 2026-05-18-tags-enrich
evidence_level: B
review_status: auto
last_updated: 2026-05-18
# v0_duplicate_count: 2
---

## 迁移备注

本样本由 v0 markdown 表格迁移而来,LLM 阶段尚未补 SDT 三需求 / T 任务态 / core_conflict / material_hooks 重判。
迁移完成后 evidence_level = C,LLM 增量打标后升至 B。
