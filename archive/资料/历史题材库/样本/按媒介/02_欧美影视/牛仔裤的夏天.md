---
# === 身份 ===
work_id: WORK-TVW-01409
work_name: 牛仔裤的夏天
work_name_alt: []
media_type: TVW
year_first: 2005
year_range: [2005, 2005]
season_count: null

# === 三要素(v0 已有,迁移直翻)===
environment: —
cultural_paradigm: —
narrative_core: —

# === SDT 心理需求(TBD,等 LLM 阶段补)===
n_main: N-Rel
n_others: []
rel_object: virtual
t_can_carry: [T3]
core_emotion_note: ""
core_conflict: 四个女孩通过一条牛仔裤维系友谊，面对成长中的分离与变化

# === 三层标签(主检索路径,见 TAGS.md)===
theme_primary: 友情
theme_secondary: [成长, 青春恋爱, 家庭羁绊, 治愈]
visual_tags: [夏日海滩, 校园青春, 都市繁华, 阳光治愈]


# === 评分(v0 已有)===
acquisition_score: 2

# === 素材与玩法 ===
material_hooks: [牛仔裤传递, 夏日海滩, 姐妹拥抱, 信件旁白, 异国风景]
existing_games: []
recommended_playstyles: []
risk_notes: []

# === 元数据 ===
source: deepseek-v4-flash + v0-migrate + tags
source_run_id: 2026-05-18-tags-enrich
evidence_level: B
review_status: auto
last_updated: 2026-05-18
---

## 迁移备注

本样本由 v0 markdown 表格迁移而来,LLM 阶段尚未补 SDT 三需求 / T 任务态 / core_conflict / material_hooks 重判。
迁移完成后 evidence_level = C,LLM 增量打标后升至 B。
