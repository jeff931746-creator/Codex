---
# === 身份 ===
work_id: WORK-TVW-00169
work_name: 疯狂麦克斯：狂暴之路
work_name_alt: []
media_type: TVW
year_first: 2015
year_range: [2015, 2015]
season_count: null

# === 三要素(v0 已有,迁移直翻)===
environment: 末世废土荒漠
cultural_paradigm: 暴力飙车+废土生存
narrative_core: 末世逃亡+反抗暴政

# === SDT 心理需求(TBD,等 LLM 阶段补)===
n_main: N-Comp
n_others: [N-Rel]
rel_object: real
t_can_carry: [T2, T4]
core_emotion_note: ""
core_conflict: 幸存者穿越废土反抗暴君不死乔的独裁统治

# === 三层标签(主检索路径,见 TAGS.md)===
theme_primary: 末日
theme_secondary: [求生, 战争, 资源枯竭, 文明崩溃, 复仇]
visual_tags: [废土, 沙漠, 战车改装造型, 爆炸场面, 末日紧张]


# === 评分(v0 已有)===
acquisition_score: 4

# === 素材与玩法 ===
material_hooks: [废土飙车追逐, 火焰吉他车, 沙暴巨浪, 战车改装造型, 狂暴女神觉醒]
existing_games: [《疯狂麦克斯》游戏]
recommended_playstyles: []
risk_notes: [受众窄, 版权成本中]

# === 元数据 ===
source: deepseek-v4-flash + v0-migrate + tags
source_run_id: 2026-05-18-tags-enrich
evidence_level: B
review_status: auto
last_updated: 2026-05-18
---

## 迁移备注

本样本由 v0 markdown 表格迁移而来,LLM 阶段尚未补 SDT 三需求 / T 任务态 / core_conflict / material_hooks 重判。
迁移完成后 evidence_level = C,LLM 增量打标后升至 B。
