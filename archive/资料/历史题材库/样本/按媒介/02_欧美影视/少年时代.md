---
# === 身份 ===
work_id: WORK-TVW-01326
work_name: 少年时代
work_name_alt: []
media_type: TVW
year_first: 2014
year_range: [2014, 2014]
season_count: null

# === 三要素(v0 已有,迁移直翻)===
environment: 现代美国12年跨度
cultural_paradigm: 现实主义成长日常
narrative_core: 男孩从童年到成年的历程

# === SDT 心理需求(TBD,等 LLM 阶段补)===
n_main: N-Auto
n_others: [N-Rel]
rel_object: real
t_can_carry: [T1, T3]
core_emotion_note: ""
core_conflict: 男孩在家庭与校园中寻找自我，面对成长的迷茫与选择

# === 三层标签(主检索路径,见 TAGS.md)===
theme_primary: 传记
theme_secondary: [成长, 家庭, 校园, 青春恋爱]
visual_tags: [校园青春, 家庭温情, 都市繁华, 阳光治愈]


# === 评分(v0 已有)===
acquisition_score: 3

# === 素材与玩法 ===
material_hooks: [90年代美式街景, 少年骑车背影, 家庭餐桌对话, 毕业典礼瞬间]
existing_games: []
recommended_playstyles: []
risk_notes: [受众极窄, 付费深度低]

# === 元数据 ===
source: deepseek-v4-flash + v0-migrate + tags
source_run_id: 2026-05-18-tags-enrich
evidence_level: B
review_status: auto
last_updated: 2026-05-18
# v0_duplicate_count: 2
---

## 迁移备注

本样本由 v0 markdown 表格迁移而来,LLM 阶段尚未补 SDT 三需求 / T 任务态 / core_conflict / material_hooks 重判。
迁移完成后 evidence_level = C,LLM 增量打标后升至 B。
