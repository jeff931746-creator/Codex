---
# === 身份 ===
work_id: WORK-TVW-00759
work_name: 怪兽大战外星人
work_name_alt: []
media_type: TVW
year_first: 2009
year_range: [2009, 2009]
season_count: null

# === 三要素(v0 已有,迁移直翻)===
environment: 现代美国+外星入侵
cultural_paradigm: 巨型怪物+外星科技
narrative_core: 团队协作+反套路英雄

# === SDT 心理需求(TBD,等 LLM 阶段补)===
n_main: N-Comp
n_others: [N-Rel]
rel_object: mixed
t_can_carry: [T2, T4]
core_emotion_note: ""
core_conflict: 怪物团队对抗外星入侵，同时证明自己不是反派

# === 三层标签(主检索路径,见 TAGS.md)===
theme_primary: 科幻
theme_secondary: [异种入侵, 怪兽, 冒险, 喜剧]
visual_tags: [巨型怪物, 外星飞船, 城市破坏, 战斗场面]


# === 评分(v0 已有)===
acquisition_score: 3

# === 素材与玩法 ===
material_hooks: [巨型怪物战斗, 怪物变形能力, 外星飞船入侵, 城市破坏场面]
existing_games: []
recommended_playstyles: []
risk_notes: [IP热度一般, 受众偏年轻]

# === 元数据 ===
source: deepseek-v4-flash + v0-migrate + tags
source_run_id: 2026-05-18-tags-enrich
evidence_level: B
review_status: auto
last_updated: 2026-05-18
# v0_duplicate_count: 2
---

## 迁移备注

本样本由 v0 markdown 表格迁移而来,LLM 阶段尚未补 SDT 三需求 / T 任务态 / core_conflict / material_hooks 重判。
迁移完成后 evidence_level = C,LLM 增量打标后升至 B。
