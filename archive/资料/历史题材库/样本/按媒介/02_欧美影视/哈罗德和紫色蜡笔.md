---
# === 身份 ===
work_id: WORK-TVW-00846
work_name: 哈罗德和紫色蜡笔
work_name_alt: []
media_type: TVW
year_first: 2024
year_range: [2024, 2024]
season_count: null

# === 三要素(v0 已有,迁移直翻)===
environment: 儿童奇幻现实世界
cultural_paradigm: 画笔创造万物体系
narrative_core: 想象力创造与冒险

# === SDT 心理需求(TBD,等 LLM 阶段补)===
n_main: N-Auto
n_others: []
rel_object: null
t_can_carry: [T1]
core_emotion_note: ""
core_conflict: 小男孩用蜡笔创造世界并自由探索冒险

# === 三层标签(主检索路径,见 TAGS.md)===
theme_primary: 奇幻
theme_secondary: [冒险, 成长, 治愈, 日常喜剧]
visual_tags: [蜡笔绘画, 奇幻场景变换, 童趣冒险, 色彩斑斓]


# === 评分(v0 已有)===
acquisition_score: 3

# === 素材与玩法 ===
material_hooks: [蜡笔画出万物, 奇幻场景变换, 童趣冒险旅程]
existing_games: []
recommended_playstyles: []
risk_notes: [低龄化, 付费深度有限]

# === 元数据 ===
source: deepseek-v4-flash + v0-migrate + tags
source_run_id: 2026-05-18-tags-enrich
evidence_level: B
review_status: auto
last_updated: 2026-05-18
---

## 迁移备注

本样本由 v0 markdown 表格迁移而来,LLM 阶段尚未补 SDT 三需求 / T 任务态 / core_conflict / material_hooks 重判。
迁移完成后 evidence_level = C,LLM 增量打标后升至 B。
