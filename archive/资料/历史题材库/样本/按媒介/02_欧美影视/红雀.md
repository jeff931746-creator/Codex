---
# === 身份 ===
work_id: WORK-TVW-01183
work_name: 红雀
work_name_alt: []
media_type: TVW
year_first: 2018
year_range: [2018, 2018]
season_count: null

# === 三要素(v0 已有,迁移直翻)===
environment: 现代俄罗斯+间谍世界
cultural_paradigm: 特工训练+色诱暗杀
narrative_core: 特工成长+背叛与复仇

# === SDT 心理需求(TBD,等 LLM 阶段补)===
n_main: N-Comp
n_others: [N-Rel]
rel_object: real
t_can_carry: [T2, T4]
core_emotion_note: ""
core_conflict: 女特工在色诱与暗杀任务中挣扎，背叛与复仇交织

# === 三层标签(主检索路径,见 TAGS.md)===
theme_primary: 悬疑
theme_secondary: [谍战, 复仇, 成长, 政治权谋]
visual_tags: [都市夜景, 宫殿, 黑色西装, 枪林弹雨]


# === 评分(v0 已有)===
acquisition_score: 3

# === 素材与玩法 ===
material_hooks: [红雀舞姿, 特工训练场, 暗杀瞬间, 克宫走廊]
existing_games: []
recommended_playstyles: []
risk_notes: [受众窄, 成人向限制]

# === 元数据 ===
source: deepseek-v4-flash + v0-migrate + tags
source_run_id: 2026-05-18-tags-enrich
evidence_level: B
review_status: auto
last_updated: 2026-05-18
---

## 迁移备注

本样本由 v0 markdown 表格迁移而来,LLM 阶段尚未补 SDT 三需求 / T 任务态 / core_conflict / material_hooks 重判。
迁移完成后 evidence_level = C,LLM 增量打标后升至 B。
