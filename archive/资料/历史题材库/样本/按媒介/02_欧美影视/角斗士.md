---
# === 身份 ===
work_id: WORK-TVW-00137
work_name: 角斗士
work_name_alt: []
media_type: TVW
year_first: 2000
year_range: [2000, 2000]
season_count: null

# === 三要素(v0 已有,迁移直翻)===
environment: 古罗马帝国时期
cultural_paradigm: 角斗士战斗与复仇
narrative_core: 复仇与荣耀

# === SDT 心理需求(TBD,等 LLM 阶段补)===
n_main: N-Comp
n_others: []
rel_object: null
t_can_carry: [T2, T4]
core_emotion_note: ""
core_conflict: 将军沦为角斗士，在竞技场中复仇夺回荣耀

# === 三层标签(主检索路径,见 TAGS.md)===
theme_primary: 历史
theme_secondary: [战争, 复仇, 古代战争, 政治权谋]
visual_tags: [古罗马竞技场, 角斗士战斗, 军团列阵, 猛兽搏斗]


# === 评分(v0 已有)===
acquisition_score: 4

# === 素材与玩法 ===
material_hooks: [角斗场厮杀, 罗马军团列阵, 猛兽搏斗, 复仇决斗]
existing_games: []
recommended_playstyles: []
risk_notes: [版权成本高, 红海古装题材]

# === 元数据 ===
source: deepseek-v4-flash + v0-migrate + tags
source_run_id: 2026-05-18-tags-enrich
evidence_level: B
review_status: auto
last_updated: 2026-05-18
# v0_duplicate_count: 2
---

## 迁移备注

本样本由 v0 markdown 表格迁移而来,LLM 阶段尚未补 SDT 三需求 / T 任务态 / core_conflict / material_hooks 重判。
迁移完成后 evidence_level = C,LLM 增量打标后升至 B。
