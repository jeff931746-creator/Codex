---
# === 身份 ===
work_id: WORK-TVW-00664
work_name: 困兽
work_name_alt: []
media_type: TVW
year_first: 2015
year_range: [2015, 2015]
season_count: null

# === 三要素(v0 已有,迁移直翻)===
environment: 现代都市动物暴乱
cultural_paradigm: 动物攻击+生存危机
narrative_core: 末世求生+动物灾难

# === SDT 心理需求(TBD,等 LLM 阶段补)===
n_main: N-Comp
n_others: []
rel_object: null
t_can_carry: [T2, T1]
core_emotion_note: ""
core_conflict: 人类在动物暴乱的城市中求生并寻找生存法则

# === 三层标签(主检索路径,见 TAGS.md)===
theme_primary: 末日
theme_secondary: [求生, 灾难, 城市废墟求生]
visual_tags: [城市废墟, 动物暴乱, 血腥撕咬, 逃生追逐]


# === 评分(v0 已有)===
acquisition_score: 3

# === 素材与玩法 ===
material_hooks: [动物暴乱场面, 城市废墟, 逃生追逐, 血腥撕咬]
existing_games: []
recommended_playstyles: []
risk_notes: [IP知名度低]

# === 元数据 ===
source: deepseek-v4-flash + v0-migrate + tags
source_run_id: 2026-05-18-tags-enrich
evidence_level: B
review_status: auto
last_updated: 2026-05-18
# v0_duplicate_count: 2
---

## 迁移备注

本样本由 v0 markdown 表格迁移而来,LLM 阶段尚未补 SDT 三需求 / T 任务态 / core_conflict / material_hooks 重判。
迁移完成后 evidence_level = C,LLM 增量打标后升至 B。
