---
# === 身份 ===
work_id: WORK-TVW-00836
work_name: 惊声尖叫6
work_name_alt: []
media_type: TVW
year_first: 2023
year_range: [2023, 2023]
season_count: null

# === 三要素(v0 已有,迁移直翻)===
environment: 现代都市纽约
cultural_paradigm: 连环杀手+恐怖生存
narrative_core: 追杀逃生+身份谜团

# === SDT 心理需求(TBD,等 LLM 阶段补)===
n_main: N-Comp
n_others: []
rel_object: null
t_can_carry: [T1, T5]
core_emotion_note: ""
core_conflict: 幸存者被连环杀手追杀，同时揭开面具背后的身份谜团

# === 三层标签(主检索路径,见 TAGS.md)===
theme_primary: 恐怖
theme_secondary: [悬疑, 犯罪, 求生, 都市]
visual_tags: [都市夜景, 鬼脸面具, 血腥场面, 追杀逃生]


# === 评分(v0 已有)===
acquisition_score: 3

# === 素材与玩法 ===
material_hooks: [鬼脸面具突袭, 纽约街头追杀, 电话威胁场景, 血腥密室逃生]
existing_games: []
recommended_playstyles: []
risk_notes: [受众窄, 内容审查风险]

# === 元数据 ===
source: deepseek-v4-flash + v0-migrate + tags
source_run_id: 2026-05-18-tags-enrich
evidence_level: B
review_status: auto
last_updated: 2026-05-18
---

## 迁移备注

本样本由 v0 markdown 表格迁移而来,LLM 阶段尚未补 SDT 三需求 / T 任务态 / core_conflict / material_hooks 重判。
迁移完成后 evidence_level = C,LLM 增量打标后升至 B。
