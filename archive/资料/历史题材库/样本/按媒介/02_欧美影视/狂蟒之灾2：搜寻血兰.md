---
# === 身份 ===
work_id: WORK-TVW-00276
work_name: 狂蟒之灾2：搜寻血兰
work_name_alt: []
media_type: TVW
year_first: 2004
year_range: [2004, 2004]
season_count: null

# === 三要素(v0 已有,迁移直翻)===
environment: 南美热带雨林
cultural_paradigm: 巨蟒生物恐怖+探险战斗
narrative_core: 雨林猎杀+宝物争夺

# === SDT 心理需求(TBD,等 LLM 阶段补)===
n_main: N-Comp
n_others: []
rel_object: null
t_can_carry: [T2, T1]
core_emotion_note: ""
core_conflict: 探险队深入雨林争夺血兰，遭遇巨蟒猎杀与生存危机

# === 三层标签(主检索路径,见 TAGS.md)===
theme_primary: 求生
theme_secondary: [丛林求生, 小队求生, 恐怖, 冒险]
visual_tags: [森林, 沼泽, 巨蟒, 探险队装备, 生存绝望]


# === 评分(v0 已有)===
acquisition_score: 3

# === 素材与玩法 ===
material_hooks: [巨蟒突袭, 雨林沼泽, 血兰发光, 探险队装备, 猎杀追逐]
existing_games: []
recommended_playstyles: []
risk_notes: [版权成本中, 题材同质化]

# === 元数据 ===
source: deepseek-v4-flash + v0-migrate + tags
source_run_id: 2026-05-18-tags-enrich
evidence_level: B
review_status: auto
last_updated: 2026-05-18
---

## 迁移备注

本样本由 v0 markdown 表格迁移而来,LLM 阶段尚未补 SDT 三需求 / T 任务态 / core_conflict / material_hooks 重判。
迁移完成后 evidence_level = C,LLM 增量打标后升至 B。
