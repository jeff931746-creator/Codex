---
# === 身份 ===
work_id: WORK-TVW-01182
work_name: 摩天营救
work_name_alt: []
media_type: TVW
year_first: 2018
year_range: [2018, 2018]
season_count: null

# === 三要素(v0 已有,迁移直翻)===
environment: 现代都市+超高层建筑
cultural_paradigm: 特工技能+火海逃生
narrative_core: 火海救援+家庭守护

# === SDT 心理需求(TBD,等 LLM 阶段补)===
n_main: N-Comp
n_others: [N-Rel]
rel_object: real
t_can_carry: [T2, T1]
core_emotion_note: ""
core_conflict: 特工父亲在燃烧的摩天大楼中救出家人并阻止阴谋

# === 三层标签(主检索路径,见 TAGS.md)===
theme_primary: 灾难
theme_secondary: [城市火灾, 求生, 家庭]
visual_tags: [摩天楼, 火海, 高空攀爬, 爆炸场面, 都市夜景]


# === 评分(v0 已有)===
acquisition_score: 3

# === 素材与玩法 ===
material_hooks: [摩天楼火灾, 高空攀爬, 火海逃生, 特工装备, 家庭团聚]
existing_games: []
recommended_playstyles: []
risk_notes: [受众窄, 玩法受限]

# === 元数据 ===
source: deepseek-v4-flash + v0-migrate + tags
source_run_id: 2026-05-18-tags-enrich
evidence_level: B
review_status: auto
last_updated: 2026-05-18
---

## 迁移备注

本样本由 v0 markdown 表格迁移而来,LLM 阶段尚未补 SDT 三需求 / T 任务态 / core_conflict / material_hooks 重判。
迁移完成后 evidence_level = C,LLM 增量打标后升至 B。
