---
# === 身份 ===
work_id: WORK-TVW-00903
work_name: 迷失太空
work_name_alt: []
media_type: TVW
year_first: 2018
year_range: [2018, 2018]
season_count: null

# === 三要素(v0 已有,迁移直翻)===
environment: 外太空殖民星球
cultural_paradigm: 太空生存+科技求生
narrative_core: 家庭生存+冒险探索

# === SDT 心理需求(TBD,等 LLM 阶段补)===
n_main: N-Rel
n_others: [N-Comp]
rel_object: mixed
t_can_carry: [T4, T2]
core_emotion_note: ""
core_conflict: 一个家庭在陌生星球上对抗自然与科技危机，维系生存与团结

# === 三层标签(主检索路径,见 TAGS.md)===
theme_primary: 求生
theme_secondary: [太空冒险, 家庭, 科幻]
visual_tags: [外星荒原, 飞船残骸, 科技装置, 太空孤寂]


# === 评分(v0 已有)===
acquisition_score: 3

# === 素材与玩法 ===
material_hooks: [外星荒原景观, 飞船残骸, 家庭互动, 科技装置]
existing_games: []
recommended_playstyles: []
risk_notes: [受众窄, 红海竞争]

# === 元数据 ===
source: deepseek-v4-flash + v0-migrate + tags
source_run_id: 2026-05-18-tags-enrich
evidence_level: B
review_status: auto
last_updated: 2026-05-18
---

## 迁移备注

本样本由 v0 markdown 表格迁移而来,LLM 阶段尚未补 SDT 三需求 / T 任务态 / core_conflict / material_hooks 重判。
迁移完成后 evidence_level = C,LLM 增量打标后升至 B。
