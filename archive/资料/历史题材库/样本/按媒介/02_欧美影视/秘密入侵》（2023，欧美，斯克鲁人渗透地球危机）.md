---
# === 身份 ===
work_id: WORK-TVW-00476
work_name: 秘密入侵》（2023，欧美，斯克鲁人渗透地球危机）
work_name_alt: []
media_type: TVW
year_first: 2023
year_range: [2023, 2023]
season_count: null

# === 三要素(v0 已有,迁移直翻)===
environment: 现代都市+外星渗透
cultural_paradigm: 变形种族+间谍战
narrative_core: 身份信任危机+生存

# === SDT 心理需求(TBD,等 LLM 阶段补)===
n_main: N-Rel
n_others: [N-Comp]
rel_object: real
t_can_carry: [T4, T5]
core_emotion_note: ""
core_conflict: 斯克鲁人渗透地球高层，身份信任危机与生存博弈

# === 三层标签(主检索路径,见 TAGS.md)===
theme_primary: 悬疑
theme_secondary: [异种入侵, 身份谜团, 阴谋论]
visual_tags: [都市夜景, 绿色皮肤特写, 身份反转对峙, 秘密基地场景]


# === 评分(v0 已有)===
acquisition_score: 3

# === 素材与玩法 ===
material_hooks: [斯克鲁人变形瞬间, 身份反转对峙, 高层渗透阴谋, 绿色皮肤特写, 秘密基地场景]
existing_games: []
recommended_playstyles: []
risk_notes: [版权成本高, 红海竞争]

# === 元数据 ===
source: deepseek-v4-flash + v0-migrate + tags
source_run_id: 2026-05-18-tags-enrich
evidence_level: B
review_status: auto
last_updated: 2026-05-18
---

## 迁移备注

本样本由 v0 markdown 表格迁移而来,LLM 阶段尚未补 SDT 三需求 / T 任务态 / core_conflict / material_hooks 重判。
迁移完成后 evidence_level = C,LLM 增量打标后升至 B。
