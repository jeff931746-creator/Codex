---
# === 身份 ===
work_id: WORK-TVW-00401
work_name: 旅行者
work_name_alt: []
media_type: TVW
year_first: 2007
year_range: [2007, 2007]
season_count: null

# === 三要素(v0 已有,迁移直翻)===
environment: 现代+时空穿越
cultural_paradigm: 时空穿越+改变历史
narrative_core: 穿越冒险+命运改变

# === SDT 心理需求(TBD,等 LLM 阶段补)===
n_main: N-Comp
n_others: []
rel_object: null
t_can_carry: [T2, T5]
core_emotion_note: ""
core_conflict: 主角穿越时空改变历史，对抗命运与未知危机

# === 三层标签(主检索路径,见 TAGS.md)===
theme_primary: 冒险
theme_secondary: [穿越转生, 科幻, 悬疑, 成长]
visual_tags: [时空隧道, 历史场景, 都市夜景, 冒险伙伴]


# === 评分(v0 已有)===
acquisition_score: 3

# === 素材与玩法 ===
material_hooks: [时空隧道穿梭, 历史场景重现, 命运抉择瞬间, 冒险伙伴同行]
existing_games: []
recommended_playstyles: []
risk_notes: [红海, 版权成本低]

# === 元数据 ===
source: deepseek-v4-flash + v0-migrate + tags
source_run_id: 2026-05-18-tags-enrich
evidence_level: B
review_status: auto
last_updated: 2026-05-18
# v0_duplicate_count: 2
---

## 迁移备注

本样本由 v0 markdown 表格迁移而来,LLM 阶段尚未补 SDT 三需求 / T 任务态 / core_conflict / material_hooks 重判。
迁移完成后 evidence_level = C,LLM 增量打标后升至 B。
