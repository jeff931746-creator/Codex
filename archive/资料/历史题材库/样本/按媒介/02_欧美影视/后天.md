---
# === 身份 ===
work_id: WORK-TVW-00271
work_name: 后天
work_name_alt: []
media_type: TVW
year_first: 2004
year_range: [2004, 2004]
season_count: null

# === 三要素(v0 已有,迁移直翻)===
environment: 现代全球冰河时代
cultural_paradigm: 气候灾难生存体系
narrative_core: 末世求生+亲情救援

# === SDT 心理需求(TBD,等 LLM 阶段补)===
n_main: N-Rel
n_others: [N-Comp]
rel_object: real
t_can_carry: [T1, T2]
core_emotion_note: ""
core_conflict: 父亲穿越冰河末世救援儿子，人类对抗极端气候

# === 三层标签(主检索路径,见 TAGS.md)===
theme_primary: 灾难
theme_secondary: [自然灾难, 求生, 亲情, 极端气候]
visual_tags: [雪原, 废墟, 暴风雪, 冰封城市, 末日紧张]


# === 评分(v0 已有)===
acquisition_score: 4

# === 素材与玩法 ===
material_hooks: [冰封城市, 暴风雪追逐, 狼群袭击, 图书馆焚烧, 极寒逃生]
existing_games: []
recommended_playstyles: []
risk_notes: [红海竞争, 版权成本中等]

# === 元数据 ===
source: deepseek-v4-flash + v0-migrate + tags
source_run_id: 2026-05-18-tags-enrich
evidence_level: B
review_status: auto
last_updated: 2026-05-18
# v0_duplicate_count: 2
---

## 迁移备注

本样本由 v0 markdown 表格迁移而来,LLM 阶段尚未补 SDT 三需求 / T 任务态 / core_conflict / material_hooks 重判。
迁移完成后 evidence_level = C,LLM 增量打标后升至 B。
