---
# === 身份 ===
work_id: WORK-TVW-01178
work_name: 泰山归来：险战丛林
work_name_alt: []
media_type: TVW
year_first: 2016
year_range: [2016, 2016]
season_count: null

# === 三要素(v0 已有,迁移直翻)===
environment: 19世纪刚果丛林
cultural_paradigm: 丛林生存+野性战斗
narrative_core: 泰山重返丛林复仇

# === SDT 心理需求(TBD,等 LLM 阶段补)===
n_main: N-Comp
n_others: []
rel_object: null
t_can_carry: [T2]
core_emotion_note: ""
core_conflict: 泰山在丛林中对抗人类入侵者，复仇并保护家园

# === 三层标签(主检索路径,见 TAGS.md)===
theme_primary: 冒险
theme_secondary: [丛林求生, 复仇, 求生, 战斗]
visual_tags: [丛林, 猛兽搏斗, 丛林陷阱, 野性怒吼]


# === 评分(v0 已有)===
acquisition_score: 3

# === 素材与玩法 ===
material_hooks: [泰山丛林飞跃, 猛兽搏斗, 野性怒吼, 丛林陷阱]
existing_games: []
recommended_playstyles: []
risk_notes: [IP老化, 受众窄]

# === 元数据 ===
source: deepseek-v4-flash + v0-migrate + tags
source_run_id: 2026-05-18-tags-enrich
evidence_level: B
review_status: auto
last_updated: 2026-05-18
---

## 迁移备注

本样本由 v0 markdown 表格迁移而来,LLM 阶段尚未补 SDT 三需求 / T 任务态 / core_conflict / material_hooks 重判。
迁移完成后 evidence_level = C,LLM 增量打标后升至 B。
