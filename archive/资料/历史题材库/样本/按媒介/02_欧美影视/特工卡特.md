---
# === 身份 ===
work_id: WORK-TVW-00375
work_name: 特工卡特
work_name_alt: []
media_type: TVW
year_first: 2000
year_range: [2000, 2015]
season_count: null

# === 三要素(v0 已有,迁移直翻)===
environment: 1940年代战后美国
cultural_paradigm: 特工谍战+科技装备
narrative_core: 女特工秘密任务与成长

# === SDT 心理需求(TBD,等 LLM 阶段补)===
n_main: N-Comp
n_others: []
rel_object: null
t_can_carry: [T2, T5]
core_emotion_note: ""
core_conflict: 女特工在战后男性主导的谍报世界中证明自己并完成秘密任务

# === 三层标签(主检索路径,见 TAGS.md)===
theme_primary: 都市
theme_secondary: [谍战, 职场, 成长, 女性独立]
visual_tags: [1940年代街景, 西装与礼裙, 秘密实验室, 复古特工装备, 女特工格斗]


# === 评分(v0 已有)===
acquisition_score: 3

# === 素材与玩法 ===
material_hooks: [复古特工装备, 1940年代街景, 女特工格斗, 秘密实验室, 西装与礼裙]
existing_games: []
recommended_playstyles: []
risk_notes: [版权成本高, 受众偏窄]

# === 元数据 ===
source: deepseek-v4-flash + v0-migrate + tags
source_run_id: 2026-05-18-tags-enrich
evidence_level: B
review_status: auto
last_updated: 2026-05-18
# v0_duplicate_count: 2
---

## 迁移备注

本样本由 v0 markdown 表格迁移而来,LLM 阶段尚未补 SDT 三需求 / T 任务态 / core_conflict / material_hooks 重判。
迁移完成后 evidence_level = C,LLM 增量打标后升至 B。
