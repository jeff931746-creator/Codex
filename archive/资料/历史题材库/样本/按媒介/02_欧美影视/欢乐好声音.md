---
# === 身份 ===
work_id: WORK-TVW-00987
work_name: 欢乐好声音
work_name_alt: []
media_type: TVW
year_first: 2016
year_range: [2016, 2016]
season_count: null

# === 三要素(v0 已有,迁移直翻)===
environment: 现代动物都市
cultural_paradigm: 音乐选秀+动物拟人
narrative_core: 草根逆袭与音乐梦想

# === SDT 心理需求(TBD,等 LLM 阶段补)===
n_main: N-Comp
n_others: [N-Rel]
rel_object: mixed
t_can_carry: [T2, T1]
core_emotion_note: ""
core_conflict: 动物们通过音乐选秀实现草根逆袭与梦想

# === 三层标签(主检索路径,见 TAGS.md)===
theme_primary: 喜剧
theme_secondary: [都市, 励志, 音乐, 成长]
visual_tags: [都市夜景, 舞台, 动物拟人, 霓虹灯]


# === 评分(v0 已有)===
acquisition_score: 3

# === 素材与玩法 ===
material_hooks: [动物歌手舞台, 华丽音乐表演, 搞笑动物日常, 逆袭高光时刻]
existing_games: []
recommended_playstyles: []
risk_notes: [付费浅, 竞品多]

# === 元数据 ===
source: deepseek-v4-flash + v0-migrate + tags
source_run_id: 2026-05-18-tags-enrich
evidence_level: B
review_status: auto
last_updated: 2026-05-18
---

## 迁移备注

本样本由 v0 markdown 表格迁移而来,LLM 阶段尚未补 SDT 三需求 / T 任务态 / core_conflict / material_hooks 重判。
迁移完成后 evidence_level = C,LLM 增量打标后升至 B。
