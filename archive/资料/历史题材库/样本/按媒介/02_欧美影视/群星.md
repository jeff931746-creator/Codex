---
# === 身份 ===
work_id: WORK-TVW-00711
work_name: 群星
work_name_alt: []
media_type: TVW
year_first: 2024
year_range: [2024, 2024]
season_count: null

# === 三要素(v0 已有,迁移直翻)===
environment: 太空站与记忆迷宫
cultural_paradigm: 硬科幻+心理悬疑
narrative_core: 太空灾难与记忆谜团

# === SDT 心理需求(TBD,等 LLM 阶段补)===
n_main: N-Comp
n_others: []
rel_object: null
t_can_carry: [T5, T2]
core_emotion_note: ""
core_conflict: 太空站幸存者对抗未知灾难与记忆碎片中的真相

# === 三层标签(主检索路径,见 TAGS.md)===
theme_primary: 科幻
theme_secondary: [太空, 悬疑, 灾难, 心理悬疑]
visual_tags: [太空站内部, 记忆碎片闪烁, 迷宫走廊, 灾难瞬间爆发]


# === 评分(v0 已有)===
acquisition_score: 3

# === 素材与玩法 ===
material_hooks: [太空站幽闭, 记忆碎片闪烁, 灾难瞬间爆发, 迷宫走廊追逐]
existing_games: []
recommended_playstyles: []
risk_notes: [受众窄, 版权成本高]

# === 元数据 ===
source: deepseek-v4-flash + v0-migrate + tags
source_run_id: 2026-05-18-tags-enrich
evidence_level: B
review_status: auto
last_updated: 2026-05-18
---

## 迁移备注

本样本由 v0 markdown 表格迁移而来,LLM 阶段尚未补 SDT 三需求 / T 任务态 / core_conflict / material_hooks 重判。
迁移完成后 evidence_level = C,LLM 增量打标后升至 B。
