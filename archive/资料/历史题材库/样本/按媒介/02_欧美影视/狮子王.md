---
# === 身份 ===
work_id: WORK-TVW-00174
work_name: 狮子王
work_name_alt: []
media_type: TVW
year_first: 2019
year_range: [2019, 2019]
season_count: null

# === 三要素(v0 已有,迁移直翻)===
environment: 非洲草原动物王国
cultural_paradigm: 动物王国+自然法则
narrative_core: 王者归来+成长复仇

# === SDT 心理需求(TBD,等 LLM 阶段补)===
n_main: N-Rel
n_others: [N-Comp]
rel_object: virtual
t_can_carry: [T3, T2]
core_emotion_note: ""
core_conflict: 辛巴为父复仇夺回王位，对抗刀疤的黑暗统治

# === 三层标签(主检索路径,见 TAGS.md)===
theme_primary: 冒险
theme_secondary: [成长, 复仇, 家庭羁绊, 父子]
visual_tags: [非洲草原, 荣耀石, 日出, 动物朝拜, 自然法则]


# === 评分(v0 已有)===
acquisition_score: 5

# === 素材与玩法 ===
material_hooks: [荣耀石日出, 辛巴与娜娜奔跑, 刀疤阴险眼神, 木法沙坠崖, 草原动物朝拜]
existing_games: [狮子王游戏]
recommended_playstyles: []
risk_notes: [版权成本高, 玩法创新难]

# === 元数据 ===
source: deepseek-v4-flash + v0-migrate + tags
source_run_id: 2026-05-18-tags-enrich
evidence_level: B
review_status: auto
last_updated: 2026-05-18
---

## 迁移备注

本样本由 v0 markdown 表格迁移而来,LLM 阶段尚未补 SDT 三需求 / T 任务态 / core_conflict / material_hooks 重判。
迁移完成后 evidence_level = C,LLM 增量打标后升至 B。
