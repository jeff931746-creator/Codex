---
# === 身份 ===
work_id: WORK-TVW-01337
work_name: 黑水
work_name_alt: []
media_type: TVW
year_first: 2019
year_range: [2019, 2020]
season_count: null

# === 三要素(v0 已有,迁移直翻)===
environment: 现代美国工业小镇
cultural_paradigm: 法律诉讼+环保调查
narrative_core: 正义抗争+真相揭露

# === SDT 心理需求(TBD,等 LLM 阶段补)===
n_main: N-Comp
n_others: []
rel_object: null
t_can_carry: [T2, T5]
core_emotion_note: ""
core_conflict: 小镇居民对抗化工巨头污染真相

# === 三层标签(主检索路径,见 TAGS.md)===
theme_primary: 犯罪
theme_secondary: [悬疑, 职场, 都市, 复仇]
visual_tags: [小镇阴郁, 工业废墟, 法庭对峙, 调查文件堆叠]


# === 评分(v0 已有)===
acquisition_score: 3

# === 素材与玩法 ===
material_hooks: [工业废水暗流, 法庭对峙场景, 小镇阴郁色调, 调查文件堆叠]
existing_games: []
recommended_playstyles: []
risk_notes: [受众极窄, 无战斗系统]

# === 元数据 ===
source: deepseek-v4-flash + v0-migrate + tags
source_run_id: 2026-05-18-tags-enrich
evidence_level: B
review_status: auto
last_updated: 2026-05-18
# v0_duplicate_count: 2
---

## 迁移备注

本样本由 v0 markdown 表格迁移而来,LLM 阶段尚未补 SDT 三需求 / T 任务态 / core_conflict / material_hooks 重判。
迁移完成后 evidence_level = C,LLM 增量打标后升至 B。
