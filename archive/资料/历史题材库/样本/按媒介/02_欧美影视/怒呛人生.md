---
# === 身份 ===
work_id: WORK-TVW-00919
work_name: 怒呛人生
work_name_alt: []
media_type: TVW
year_first: 2023
year_range: [2023, 2023]
season_count: null

# === 三要素(v0 已有,迁移直翻)===
environment: 现代都市公路
cultural_paradigm: 路怒复仇+心理博弈
narrative_core: 复仇循环+人性失控

# === SDT 心理需求(TBD,等 LLM 阶段补)===
n_main: N-Rel
n_others: []
rel_object: real
t_can_carry: [T4]
core_emotion_note: ""
core_conflict: 两个陌生人因路怒陷入复仇循环，人性失控与权力博弈

# === 三层标签(主检索路径,见 TAGS.md)===
theme_primary: 都市
theme_secondary: [复仇, 黑色喜剧, 心理悬疑]
visual_tags: [公路追逐, 都市夜景, 荒诞互殴, 心理对峙]


# === 评分(v0 已有)===
acquisition_score: 3

# === 素材与玩法 ===
material_hooks: [公路追逐, 砸车对峙, 荒诞互殴, 心理对峙]
existing_games: []
recommended_playstyles: []
risk_notes: [受众窄, 情绪负面]

# === 元数据 ===
source: deepseek-v4-flash + v0-migrate + tags
source_run_id: 2026-05-18-tags-enrich
evidence_level: B
review_status: auto
last_updated: 2026-05-18
---

## 迁移备注

本样本由 v0 markdown 表格迁移而来,LLM 阶段尚未补 SDT 三需求 / T 任务态 / core_conflict / material_hooks 重判。
迁移完成后 evidence_level = C,LLM 增量打标后升至 B。
