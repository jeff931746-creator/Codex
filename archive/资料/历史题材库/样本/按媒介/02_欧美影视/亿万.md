---
# === 身份 ===
work_id: WORK-TVW-00441
work_name: 亿万
work_name_alt: []
media_type: TVW
year_first: 2016
year_range: [2016, 2016]
season_count: null

# === 三要素(v0 已有,迁移直翻)===
environment: 现代纽约华尔街
cultural_paradigm: 金融资本博弈+法律对抗
narrative_core: 权力博弈+猫鼠游戏

# === SDT 心理需求(TBD,等 LLM 阶段补)===
n_main: N-Rel
n_others: [N-Comp]
rel_object: real
t_can_carry: [T4, T2]
core_emotion_note: ""
core_conflict: 检察官与亿万富翁之间的猫鼠游戏，金融权力与法律正义的对抗

# === 三层标签(主检索路径,见 TAGS.md)===
theme_primary: 职场
theme_secondary: [商战, 律政, 都市, 权力游戏]
visual_tags: [都市繁华, 摩天楼, 黑色西装, 法庭对峙]


# === 评分(v0 已有)===
acquisition_score: 3

# === 素材与玩法 ===
material_hooks: [华尔街高楼, 豪车名表, 法庭对峙, 交易室紧张]
existing_games: []
recommended_playstyles: []
risk_notes: [受众窄, 版权成本高]

# === 元数据 ===
source: deepseek-v4-flash + v0-migrate + tags
source_run_id: 2026-05-18-tags-enrich
evidence_level: B
review_status: auto
last_updated: 2026-05-18
# v0_duplicate_count: 3
---

## 迁移备注

本样本由 v0 markdown 表格迁移而来,LLM 阶段尚未补 SDT 三需求 / T 任务态 / core_conflict / material_hooks 重判。
迁移完成后 evidence_level = C,LLM 增量打标后升至 B。
