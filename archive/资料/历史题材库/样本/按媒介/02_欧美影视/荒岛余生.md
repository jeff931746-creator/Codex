---
# === 身份 ===
work_id: WORK-TVW-00486
work_name: 荒岛余生
work_name_alt: []
media_type: TVW
year_first: 2000
year_range: [2000, 2000]
season_count: null

# === 三要素(v0 已有,迁移直翻)===
environment: 现代太平洋荒岛
cultural_paradigm: 现代人荒野求生
narrative_core: 孤独求生与回归

# === SDT 心理需求(TBD,等 LLM 阶段补)===
n_main: N-Comp
n_others: []
rel_object: null
t_can_carry: [T2, T1]
core_emotion_note: ""
core_conflict: 现代人在荒岛孤独求生，对抗自然与内心绝望

# === 三层标签(主检索路径,见 TAGS.md)===
theme_primary: 求生
theme_secondary: [荒岛求生, 孤独求生, 成长, 救赎]
visual_tags: [荒岛, 海洋, 森林, 废墟]


# === 评分(v0 已有)===
acquisition_score: 3

# === 素材与玩法 ===
material_hooks: [荒岛孤影, 海浪拍岸, 钻木取火, 椰子砸地, 木筏远航]
existing_games: []
recommended_playstyles: []
risk_notes: [红海生存题材, 玩法同质化]

# === 元数据 ===
source: deepseek-v4-flash + v0-migrate + tags
source_run_id: 2026-05-18-tags-enrich
evidence_level: B
review_status: auto
last_updated: 2026-05-18
# v0_duplicate_count: 2
---

## 迁移备注

本样本由 v0 markdown 表格迁移而来,LLM 阶段尚未补 SDT 三需求 / T 任务态 / core_conflict / material_hooks 重判。
迁移完成后 evidence_level = C,LLM 增量打标后升至 B。
