---
# === 身份 ===
work_id: WORK-TVW-00718
work_name: 小鸡快跑
work_name_alt: []
media_type: TVW
year_first: 2000
year_range: [2000, 2000]
season_count: null

# === 三要素(v0 已有,迁移直翻)===
environment: 英国农场监狱
cultural_paradigm: 动物越狱+喜剧冒险
narrative_core: 母鸡集体越狱逃命

# === SDT 心理需求(TBD,等 LLM 阶段补)===
n_main: N-Rel
n_others: [N-Auto]
rel_object: mixed
t_can_carry: [T1, T3]
core_emotion_note: ""
core_conflict: 母鸡们团结协作逃离农场监狱，对抗农场主追杀

# === 三层标签(主检索路径,见 TAGS.md)===
theme_primary: 喜剧
theme_secondary: [冒险, 求生, 友情, 成长]
visual_tags: [农场, 铁丝网围栏, 泥巴滑翔机, 田园清新, 搞笑荒诞]


# === 评分(v0 已有)===
acquisition_score: 3

# === 素材与玩法 ===
material_hooks: [鸡群越狱计划, 农场主反派压迫, 泥巴滑翔机, 铁丝网围栏, 铁锅炖危机]
existing_games: []
recommended_playstyles: []
risk_notes: [版权成本高]

# === 元数据 ===
source: deepseek-v4-flash + v0-migrate + tags
source_run_id: 2026-05-18-tags-enrich
evidence_level: B
review_status: auto
last_updated: 2026-05-18
---

## 迁移备注

本样本由 v0 markdown 表格迁移而来,LLM 阶段尚未补 SDT 三需求 / T 任务态 / core_conflict / material_hooks 重判。
迁移完成后 evidence_level = C,LLM 增量打标后升至 B。
