---
# === 身份 ===
work_id: WORK-TVW-01309
work_name: 佛西与沃登
work_name_alt: []
media_type: TVW
year_first: 2019
year_range: [2019, 2019]
season_count: null

# === 三要素(v0 已有,迁移直翻)===
environment: 20世纪百老汇
cultural_paradigm: 舞蹈艺术+传记
narrative_core: 编舞家创作与爱情

# === SDT 心理需求(TBD,等 LLM 阶段补)===
n_main: N-Rel
n_others: []
rel_object: real
t_can_carry: [T3]
core_emotion_note: ""
core_conflict: 两位编舞家艺术理念碰撞与情感羁绊

# === 三层标签(主检索路径,见 TAGS.md)===
theme_primary: 传记
theme_secondary: [职场, 恋爱, 都市]
visual_tags: [百老汇舞台, 排练室, 都市夜景, 古风装扮]


# === 评分(v0 已有)===
acquisition_score: 2

# === 素材与玩法 ===
material_hooks: [百老汇舞台光影, 双人舞蹈张力, 排练室争吵, 谢幕掌声]
existing_games: []
recommended_playstyles: []
risk_notes: [受众极窄, 玩法难转化]

# === 元数据 ===
source: deepseek-v4-flash + v0-migrate + tags
source_run_id: 2026-05-18-tags-enrich
evidence_level: B
review_status: auto
last_updated: 2026-05-18
---

## 迁移备注

本样本由 v0 markdown 表格迁移而来,LLM 阶段尚未补 SDT 三需求 / T 任务态 / core_conflict / material_hooks 重判。
迁移完成后 evidence_level = C,LLM 增量打标后升至 B。
