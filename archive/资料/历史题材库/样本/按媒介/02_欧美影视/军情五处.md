---
# === 身份 ===
work_id: WORK-TVW-00865
work_name: 军情五处
work_name_alt: []
media_type: TVW
year_first: 2005
year_range: [2005, 2005]
season_count: null

# === 三要素(v0 已有,迁移直翻)===
environment: 现代英国伦敦
cultural_paradigm: 情报机构+反恐行动
narrative_core: 反恐与间谍博弈

# === SDT 心理需求(TBD,等 LLM 阶段补)===
n_main: N-Rel
n_others: [N-Comp]
rel_object: real
t_can_carry: [T4, T5]
core_emotion_note: ""
core_conflict: 军情五处特工在反恐与间谍博弈中维护国家安全

# === 三层标签(主检索路径,见 TAGS.md)===
theme_primary: 职场
theme_secondary: [谍战, 反恐战争, 特工任务, 现代政坛]
visual_tags: [都市夜景, 情报室监控屏, 特工伪装行动, 炸弹倒计时, 审讯室对峙]


# === 评分(v0 已有)===
acquisition_score: 3

# === 素材与玩法 ===
material_hooks: [伦敦街景追踪, 情报室监控屏, 特工伪装行动, 炸弹倒计时, 审讯室对峙]
existing_games: []
recommended_playstyles: []
risk_notes: [受众窄, 政治敏感]

# === 元数据 ===
source: deepseek-v4-flash + v0-migrate + tags
source_run_id: 2026-05-18-tags-enrich
evidence_level: B
review_status: auto
last_updated: 2026-05-18
---

## 迁移备注

本样本由 v0 markdown 表格迁移而来,LLM 阶段尚未补 SDT 三需求 / T 任务态 / core_conflict / material_hooks 重判。
迁移完成后 evidence_level = C,LLM 增量打标后升至 B。
