---
# === 身份 ===
work_id: WORK-TVW-01015
work_name: 神秘窥视
work_name_alt: []
media_type: TVW
year_first: 2024
year_range: [2024, 2024]
season_count: null

# === 三要素(v0 已有,迁移直翻)===
environment: 现代神秘森林
cultural_paradigm: 超自然监视+生存恐怖
narrative_core: 被困求生+未知恐惧

# === SDT 心理需求(TBD,等 LLM 阶段补)===
n_main: N-Comp
n_others: []
rel_object: null
t_can_carry: [T5, T2]
core_emotion_note: ""
core_conflict: 被困神秘森林，对抗未知监视者并寻找逃生之路

# === 三层标签(主检索路径,见 TAGS.md)===
theme_primary: 恐怖
theme_secondary: [求生, 悬疑, 超自然悬疑, 生存恐怖]
visual_tags: [森林, 迷雾, 废墟, 黑暗压抑, 生存绝望]


# === 评分(v0 已有)===
acquisition_score: 2

# === 素材与玩法 ===
material_hooks: [森林迷雾, 监视镜头, 诡异声响, 逃生路线]
existing_games: []
recommended_playstyles: []
risk_notes: [受众窄, 视觉素材难吸量]

# === 元数据 ===
source: deepseek-v4-flash + v0-migrate + tags
source_run_id: 2026-05-18-tags-enrich
evidence_level: B
review_status: auto
last_updated: 2026-05-18
# v0_duplicate_count: 2
---

## 迁移备注

本样本由 v0 markdown 表格迁移而来,LLM 阶段尚未补 SDT 三需求 / T 任务态 / core_conflict / material_hooks 重判。
迁移完成后 evidence_level = C,LLM 增量打标后升至 B。
