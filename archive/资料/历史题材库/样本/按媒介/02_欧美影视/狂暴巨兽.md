---
# === 身份 ===
work_id: WORK-TVW-00808
work_name: 狂暴巨兽
work_name_alt: []
media_type: TVW
year_first: 2018
year_range: [2018, 2018]
season_count: null

# === 三要素(v0 已有,迁移直翻)===
environment: 现代都市+基因变异世界
cultural_paradigm: 巨兽变异+城市破坏
narrative_core: 巨兽大战+拯救城市

# === SDT 心理需求(TBD,等 LLM 阶段补)===
n_main: N-Comp
n_others: []
rel_object: null
t_can_carry: [T2]
core_emotion_note: ""
core_conflict: 基因变异巨兽肆虐城市，主角联合盟友阻止毁灭

# === 三层标签(主检索路径,见 TAGS.md)===
theme_primary: 灾难
theme_secondary: [热血战斗, 科幻]
visual_tags: [巨兽, 城市废墟, 爆炸场面, 高楼崩塌]


# === 评分(v0 已有)===
acquisition_score: 3

# === 素材与玩法 ===
material_hooks: [巨兽破坏城市, 变异怪物对决, 高楼崩塌场面, 热血战斗对抗]
existing_games: []
recommended_playstyles: []
risk_notes: [IP热度一般, 红海]

# === 元数据 ===
source: deepseek-v4-flash + v0-migrate + tags
source_run_id: 2026-05-18-tags-enrich
evidence_level: B
review_status: auto
last_updated: 2026-05-18
---

## 迁移备注

本样本由 v0 markdown 表格迁移而来,LLM 阶段尚未补 SDT 三需求 / T 任务态 / core_conflict / material_hooks 重判。
迁移完成后 evidence_level = C,LLM 增量打标后升至 B。
