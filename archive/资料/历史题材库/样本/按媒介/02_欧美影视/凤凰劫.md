---
# === 身份 ===
work_id: WORK-TVW-00741
work_name: 凤凰劫
work_name_alt: []
media_type: TVW
year_first: 2004
year_range: [2004, 2004]
season_count: null

# === 三要素(v0 已有,迁移直翻)===
environment: 蒙古戈壁沙漠
cultural_paradigm: 机械维修+生存协作
narrative_core: 绝境求生+团队重建

# === SDT 心理需求(TBD,等 LLM 阶段补)===
n_main: N-Rel
n_others: [N-Comp]
rel_object: real
t_can_carry: [T4, T2]
core_emotion_note: ""
core_conflict: 在沙漠绝境中修复飞机，团队协作对抗自然与内部矛盾

# === 三层标签(主检索路径,见 TAGS.md)===
theme_primary: 求生
theme_secondary: [沙漠求生, 小队求生, 灾难, 成长]
visual_tags: [沙漠, 废土, 废墟, 生存绝望]


# === 评分(v0 已有)===
acquisition_score: 2

# === 素材与玩法 ===
material_hooks: [沙漠风暴, 破旧飞机, 机械零件, 篝火夜话, 救援队背影]
existing_games: []
recommended_playstyles: []
risk_notes: [受众窄, 缺乏战斗系统]

# === 元数据 ===
source: deepseek-v4-flash + v0-migrate + tags
source_run_id: 2026-05-18-tags-enrich
evidence_level: B
review_status: auto
last_updated: 2026-05-18
---

## 迁移备注

本样本由 v0 markdown 表格迁移而来,LLM 阶段尚未补 SDT 三需求 / T 任务态 / core_conflict / material_hooks 重判。
迁移完成后 evidence_level = C,LLM 增量打标后升至 B。
