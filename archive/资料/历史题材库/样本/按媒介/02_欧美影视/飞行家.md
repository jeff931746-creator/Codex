---
# === 身份 ===
work_id: WORK-TVW-00740
work_name: 飞行家
work_name_alt: []
media_type: TVW
year_first: 2004
year_range: [2004, 2004]
season_count: null

# === 三要素(v0 已有,迁移直翻)===
environment: 20世纪航空黄金时代
cultural_paradigm: 航空工业+商业传奇
narrative_core: 梦想追逐+商业博弈

# === SDT 心理需求(TBD,等 LLM 阶段补)===
n_main: N-Comp
n_others: []
rel_object: null
t_can_carry: [T2, T5]
core_emotion_note: ""
core_conflict: 航空先驱突破技术极限与商业对手的残酷竞争

# === 三层标签(主检索路径,见 TAGS.md)===
theme_primary: 传记
theme_secondary: [职场, 历史, 成长, 竞技]
visual_tags: [飞机空中特技, 工厂装配线, 飞行竞赛场面, 机舱仪表盘]


# === 评分(v0 已有)===
acquisition_score: 3

# === 素材与玩法 ===
material_hooks: [飞机空中特技, 工厂装配线, 飞行竞赛场面, 机舱仪表盘]
existing_games: []
recommended_playstyles: []
risk_notes: [受众中等, 版权成本中]

# === 元数据 ===
source: deepseek-v4-flash + v0-migrate + tags
source_run_id: 2026-05-18-tags-enrich
evidence_level: B
review_status: auto
last_updated: 2026-05-18
# v0_duplicate_count: 2
---

## 迁移备注

本样本由 v0 markdown 表格迁移而来,LLM 阶段尚未补 SDT 三需求 / T 任务态 / core_conflict / material_hooks 重判。
迁移完成后 evidence_level = C,LLM 增量打标后升至 B。
