---
# === 身份 ===
work_id: WORK-TVW-01322
work_name: 乌云背后的幸福线
work_name_alt: []
media_type: TVW
year_first: 2012
year_range: [2012, 2012]
season_count: null

# === 三要素(v0 已有,迁移直翻)===
environment: 现代都市美国
cultural_paradigm: 心理康复+浪漫治愈
narrative_core: 心理救赎+爱情治愈

# === SDT 心理需求(TBD,等 LLM 阶段补)===
n_main: N-Rel
n_others: [N-Auto]
rel_object: real
t_can_carry: [T3]
core_emotion_note: 副N-Auto体现在主角自主选择康复路径的自由感
core_conflict: 两个心理创伤者通过舞蹈与爱情互相治愈

# === 三层标签(主检索路径,见 TAGS.md)===
theme_primary: 都市
theme_secondary: [恋爱, 家庭, 喜剧]
visual_tags: [都市夜景, 公寓, 咖啡馆, 校园青春, 恋爱甜蜜]


# === 评分(v0 已有)===
acquisition_score: 3

# === 素材与玩法 ===
material_hooks: [双人舞蹈场景, 雨夜奔跑, 家庭聚餐冲突, 心理治疗对话]
existing_games: []
recommended_playstyles: []
risk_notes: [受众极窄, 玩法难设计]

# === 元数据 ===
source: deepseek-v4-flash + v0-migrate + tags
source_run_id: 2026-05-18-tags-enrich
evidence_level: B
review_status: auto
last_updated: 2026-05-18
---

## 迁移备注

本样本由 v0 markdown 表格迁移而来,LLM 阶段尚未补 SDT 三需求 / T 任务态 / core_conflict / material_hooks 重判。
迁移完成后 evidence_level = C,LLM 增量打标后升至 B。
