---
# === 身份 ===
work_id: WORK-TVW-00687
work_name: 叶卡捷琳娜大帝
work_name_alt: []
media_type: TVW
year_first: 2019
year_range: [2019, 2019]
season_count: null

# === 三要素(v0 已有,迁移直翻)===
environment: 18世纪俄国宫廷
cultural_paradigm: 欧洲宫廷权谋+历史
narrative_core: 女皇权谋与爱情

# === SDT 心理需求(TBD,等 LLM 阶段补)===
n_main: N-Rel
n_others: [N-Comp]
rel_object: real
t_can_carry: [T4, T2]
core_emotion_note: ""
core_conflict: 叶卡捷琳娜在宫廷斗争中夺权并巩固统治

# === 三层标签(主检索路径,见 TAGS.md)===
theme_primary: 传记
theme_secondary: [政治权谋, 历史, 宫廷权谋, 权力游戏, 女性传记]
visual_tags: [宫殿, 古风装扮, 权谋阴谋, 政治紧张]


# === 评分(v0 已有)===
acquisition_score: 4

# === 素材与玩法 ===
material_hooks: [华丽宫廷服饰, 权谋对峙场面, 女皇加冕仪式, 情欲与权力交织]
existing_games: []
recommended_playstyles: []
risk_notes: [受众窄, 文化门槛高]

# === 元数据 ===
source: deepseek-v4-flash + v0-migrate + tags
source_run_id: 2026-05-18-tags-enrich
evidence_level: B
review_status: auto
last_updated: 2026-05-18
---

## 迁移备注

本样本由 v0 markdown 表格迁移而来,LLM 阶段尚未补 SDT 三需求 / T 任务态 / core_conflict / material_hooks 重判。
迁移完成后 evidence_level = C,LLM 增量打标后升至 B。
