---
# === 身份 ===
work_id: WORK-TVW-01213
work_name: 塔尔
work_name_alt: []
media_type: TVW
year_first: 2022
year_range: [2022, 2022]
season_count: null

# === 三要素(v0 已有,迁移直翻)===
environment: 现代古典音乐界
cultural_paradigm: 权力斗争+艺术圈文化
narrative_core: 权力崩塌与丑闻

# === SDT 心理需求(TBD,等 LLM 阶段补)===
n_main: N-Rel
n_others: []
rel_object: real
t_can_carry: [T4]
core_emotion_note: ""
core_conflict: 天才女指挥家攀登权力巅峰，却因丑闻与自我崩塌

# === 三层标签(主检索路径,见 TAGS.md)===
theme_primary: 职场
theme_secondary: [权力博弈, 阶级压迫, 都市, 传记]
visual_tags: [舞台聚光灯, 都市夜景, 权力走廊, 黑色西装]


# === 评分(v0 已有)===
acquisition_score: 3

# === 素材与玩法 ===
material_hooks: [指挥棒挥动, 乐团对峙, 权力走廊, 舞台聚光灯]
existing_games: []
recommended_playstyles: []
risk_notes: [受众窄, 题材敏感]

# === 元数据 ===
source: deepseek-v4-flash + v0-migrate + tags
source_run_id: 2026-05-18-tags-enrich
evidence_level: B
review_status: auto
last_updated: 2026-05-18
# v0_duplicate_count: 2
---

## 迁移备注

本样本由 v0 markdown 表格迁移而来,LLM 阶段尚未补 SDT 三需求 / T 任务态 / core_conflict / material_hooks 重判。
迁移完成后 evidence_level = C,LLM 增量打标后升至 B。
