---
# === 身份 ===
work_id: WORK-TVW-00944
work_name: 芝加哥
work_name_alt: []
media_type: TVW
year_first: 2002
year_range: [2002, 2003]
season_count: null

# === 三要素(v0 已有,迁移直翻)===
environment: 1920年代芝加哥爵士时代
cultural_paradigm: 歌舞+犯罪+媒体炒作
narrative_core: 利用媒体脱罪成名

# === SDT 心理需求(TBD,等 LLM 阶段补)===
n_main: N-Rel
n_others: [N-Comp]
rel_object: real
t_can_carry: [T4, T1]
core_emotion_note: ""
core_conflict: 罪犯利用媒体操纵舆论脱罪，与法律和公众博弈

# === 三层标签(主检索路径,见 TAGS.md)===
theme_primary: 犯罪
theme_secondary: [黑帮, 历史]
visual_tags: [都市夜景, 霓虹灯, 爵士时代服饰, 法庭]


# === 评分(v0 已有)===
acquisition_score: 4

# === 素材与玩法 ===
material_hooks: [华丽歌舞场面, 爵士时代服饰, 法庭对峙戏, 媒体闪光灯]
existing_games: []
recommended_playstyles: []
risk_notes: [受众中等, 付费深度低, 版权成本高]

# === 元数据 ===
source: deepseek-v4-flash + v0-migrate + tags
source_run_id: 2026-05-18-tags-enrich
evidence_level: B
review_status: auto
last_updated: 2026-05-18
# v0_duplicate_count: 3
---

## 迁移备注

本样本由 v0 markdown 表格迁移而来,LLM 阶段尚未补 SDT 三需求 / T 任务态 / core_conflict / material_hooks 重判。
迁移完成后 evidence_level = C,LLM 增量打标后升至 B。
