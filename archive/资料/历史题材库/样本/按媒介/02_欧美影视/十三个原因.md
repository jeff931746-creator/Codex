---
# === 身份 ===
work_id: WORK-TVW-00897
work_name: 十三个原因
work_name_alt: []
media_type: TVW
year_first: 2017
year_range: [2017, 2017]
season_count: null

# === 三要素(v0 已有,迁移直翻)===
environment: 现代美国高中校园
cultural_paradigm: 青少年社交与心理创伤
narrative_core: 校园霸凌与自杀谜团

# === SDT 心理需求(TBD,等 LLM 阶段补)===
n_main: N-Rel
n_others: []
rel_object: real
t_can_carry: [T3, T5]
core_emotion_note: ""
core_conflict: 少女自杀背后的霸凌真相与同学间的责任推诿

# === 三层标签(主检索路径,见 TAGS.md)===
theme_primary: 校园
theme_secondary: [校园悬疑, 青春恋爱, 成长, 心理悬疑, 社会派推理]
visual_tags: [校园, 教室, 走廊, 都市夜景, 悬疑诡异]


# === 评分(v0 已有)===
acquisition_score: 3

# === 素材与玩法 ===
material_hooks: [磁带录音, 校园走廊, 自杀现场, 角色特写]
existing_games: []
recommended_playstyles: []
risk_notes: [敏感题材, 付费浅]

# === 元数据 ===
source: deepseek-v4-flash + v0-migrate + tags
source_run_id: 2026-05-18-tags-enrich
evidence_level: B
review_status: auto
last_updated: 2026-05-18
# v0_duplicate_count: 2
---

## 迁移备注

本样本由 v0 markdown 表格迁移而来,LLM 阶段尚未补 SDT 三需求 / T 任务态 / core_conflict / material_hooks 重判。
迁移完成后 evidence_level = C,LLM 增量打标后升至 B。
