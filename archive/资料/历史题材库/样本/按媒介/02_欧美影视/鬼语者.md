---
# === 身份 ===
work_id: WORK-TVW-00631
work_name: 鬼语者
work_name_alt: []
media_type: TVW
year_first: 2005
year_range: [2005, 2006]
season_count: null

# === 三要素(v0 已有,迁移直翻)===
environment: 现代美国小镇
cultural_paradigm: 灵异通灵+亡灵文化
narrative_core: 与亡灵沟通化解遗憾

# === SDT 心理需求(TBD,等 LLM 阶段补)===
n_main: N-Rel
n_others: [N-Auto]
rel_object: virtual
t_can_carry: [T3, T1]
core_emotion_note: ""
core_conflict: 主角帮助亡灵完成未了心愿，同时平衡自身与生者的生活

# === 三层标签(主检索路径,见 TAGS.md)===
theme_primary: 悬疑
theme_secondary: [灵异, 家庭, 治愈, 都市]
visual_tags: [小镇黄昏, 亡灵现身, 通灵手势, 家庭温情]


# === 评分(v0 已有)===
acquisition_score: 3

# === 素材与玩法 ===
material_hooks: [亡灵现身特效, 通灵仪式手势, 小镇黄昏氛围, 角色情感对视]
existing_games: []
recommended_playstyles: []
risk_notes: [受众偏女性, 付费深度中等]

# === 元数据 ===
source: deepseek-v4-flash + v0-migrate + tags
source_run_id: 2026-05-18-tags-enrich
evidence_level: B
review_status: auto
last_updated: 2026-05-18
# v0_duplicate_count: 3
---

## 迁移备注

本样本由 v0 markdown 表格迁移而来,LLM 阶段尚未补 SDT 三需求 / T 任务态 / core_conflict / material_hooks 重判。
迁移完成后 evidence_level = C,LLM 增量打标后升至 B。
