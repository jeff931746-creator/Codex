---
# === 身份 ===
work_id: WORK-TVW-00566
work_name: 永生守卫
work_name_alt: []
media_type: TVW
year_first: 2020
year_range: [2020, 2020]
season_count: null

# === 三要素(v0 已有,迁移直翻)===
environment: 现代全球秘密世界
cultural_paradigm: 不死战士+雇佣兵体系
narrative_core: 不死者守护世界秘密

# === SDT 心理需求(TBD,等 LLM 阶段补)===
n_main: N-Comp
n_others: [N-Rel]
rel_object: real
t_can_carry: [T2, T4]
core_emotion_note: ""
core_conflict: 不死战士对抗追杀者，守护永生秘密与团队存续

# === 三层标签(主检索路径,见 TAGS.md)===
theme_primary: 冒险
theme_secondary: [求生, 战斗, 身份认同]
visual_tags: [现代都市, 冷兵器对决, 枪林弹雨, 复活场景, 黑暗压抑]


# === 评分(v0 已有)===
acquisition_score: 3

# === 素材与玩法 ===
material_hooks: [不死战士复活, 冷兵器近战, 千年回忆闪回, 团队战术配合]
existing_games: []
recommended_playstyles: []
risk_notes: [版权成本中, 续作未定]

# === 元数据 ===
source: deepseek-v4-flash + v0-migrate + tags
source_run_id: 2026-05-18-tags-enrich
evidence_level: B
review_status: auto
last_updated: 2026-05-18
---

## 迁移备注

本样本由 v0 markdown 表格迁移而来,LLM 阶段尚未补 SDT 三需求 / T 任务态 / core_conflict / material_hooks 重判。
迁移完成后 evidence_level = C,LLM 增量打标后升至 B。
