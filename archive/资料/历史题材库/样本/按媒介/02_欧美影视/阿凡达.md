---
# === 身份 ===
work_id: WORK-TVW-00040
work_name: 阿凡达
work_name_alt: []
media_type: TVW
year_first: 2009
year_range: [2009, 2009]
season_count: null

# === 三要素(v0 已有,迁移直翻)===
environment: 潘多拉星球+外星丛林
cultural_paradigm: 纳美族+自然共生
narrative_core: 守护家园+身份抉择

# === SDT 心理需求(TBD,等 LLM 阶段补)===
n_main: N-Auto
n_others: [N-Rel]
rel_object: mixed
t_can_carry: [T1, T2]
core_emotion_note: ""
core_conflict: 人类殖民者与纳美族争夺潘多拉星球生存权

# === 三层标签(主检索路径,见 TAGS.md)===
theme_primary: 科幻
theme_secondary: [冒险, 战争, 生态崩坏, 异种入侵, 救赎]
visual_tags: [外星丛林, 悬浮山, 发光森林, 机甲大战, 飞龙骑乘]


# === 评分(v0 已有)===
acquisition_score: 5

# === 素材与玩法 ===
material_hooks: [悬浮山奇观, 发光森林, 纳美族蓝皮肤, 飞龙骑乘, 机甲大战]
existing_games: [《阿凡达》游戏]
recommended_playstyles: []
risk_notes: [版权成本极高, 红海]

# === 元数据 ===
source: deepseek-v4-flash + v0-migrate + tags
source_run_id: 2026-05-18-tags-enrich
evidence_level: B
review_status: auto
last_updated: 2026-05-18
# v0_duplicate_count: 2
---

## 迁移备注

本样本由 v0 markdown 表格迁移而来,LLM 阶段尚未补 SDT 三需求 / T 任务态 / core_conflict / material_hooks 重判。
迁移完成后 evidence_level = C,LLM 增量打标后升至 B。
