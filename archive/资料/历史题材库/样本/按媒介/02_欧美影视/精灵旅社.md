---
# === 身份 ===
work_id: WORK-TVW-00521
work_name: 精灵旅社
work_name_alt: []
media_type: TVW
year_first: 2012
year_range: [2012, 2012]
season_count: null

# === 三要素(v0 已有,迁移直翻)===
environment: 现代奇幻怪物酒店
cultural_paradigm: 吸血鬼+怪物家族文化
narrative_core: 父女亲情与接纳

# === SDT 心理需求(TBD,等 LLM 阶段补)===
n_main: N-Rel
n_others: []
rel_object: virtual
t_can_carry: [T3]
core_emotion_note: ""
core_conflict: 吸血鬼父亲保护女儿与接纳人类女婿的冲突

# === 三层标签(主检索路径,见 TAGS.md)===
theme_primary: 亲情
theme_secondary: [家庭, 喜剧, 奇幻, 父女]
visual_tags: [古堡, 怪物萌化造型, 搞笑荒诞, 奇幻瑰丽]


# === 评分(v0 已有)===
acquisition_score: 4

# === 素材与玩法 ===
material_hooks: [怪物酒店奇观, 德古拉变身, 怪物萌化造型, 搞笑怪物互动]
existing_games: []
recommended_playstyles: []
risk_notes: [版权成本高, 受众偏轻]

# === 元数据 ===
source: deepseek-v4-flash + v0-migrate + tags
source_run_id: 2026-05-18-tags-enrich
evidence_level: B
review_status: auto
last_updated: 2026-05-18
# v0_duplicate_count: 2
---

## 迁移备注

本样本由 v0 markdown 表格迁移而来,LLM 阶段尚未补 SDT 三需求 / T 任务态 / core_conflict / material_hooks 重判。
迁移完成后 evidence_level = C,LLM 增量打标后升至 B。
