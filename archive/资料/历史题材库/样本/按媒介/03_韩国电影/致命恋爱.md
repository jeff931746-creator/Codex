---
# === 身份 ===
work_id: WORK-MVK-00317
work_name: 致命恋爱
work_name_alt: []
media_type: MVK
year_first: 2015
year_range: [2015, 2016]
season_count: null

# === 三要素(v0 已有,迁移直翻)===
environment: 现代韩国都市
cultural_paradigm: 浪漫喜剧+连环杀手
narrative_core: 爱情与追凶交织

# === SDT 心理需求(TBD,等 LLM 阶段补)===
n_main: N-Rel
n_others: []
rel_object: virtual
t_can_carry: [T3]
core_emotion_note: ""
core_conflict: 爱情与连环杀手追凶交织，甜蜜与惊悚并存

# === 三层标签(主检索路径,见 TAGS.md)===
theme_primary: 恋爱
theme_secondary: [都市, 喜剧, 悬疑, 犯罪]
visual_tags: [都市夜景, 咖啡馆, 霓虹灯, 甜蜜约会, 悬疑诡异]


# === 评分(v0 已有)===
acquisition_score: 2

# === 素材与玩法 ===
material_hooks: [甜蜜约会场景, 惊悚追凶画面, 幽默互动瞬间]
existing_games: []
recommended_playstyles: []
risk_notes: [玩法同质化]

# === 元数据 ===
source: deepseek-v4-flash + v0-migrate + tags
source_run_id: 2026-05-18-tags-enrich
evidence_level: B
review_status: auto
last_updated: 2026-05-18
# v0_duplicate_count: 2
---

## 迁移备注

本样本由 v0 markdown 表格迁移而来,LLM 阶段尚未补 SDT 三需求 / T 任务态 / core_conflict / material_hooks 重判。
迁移完成后 evidence_level = C,LLM 增量打标后升至 B。
