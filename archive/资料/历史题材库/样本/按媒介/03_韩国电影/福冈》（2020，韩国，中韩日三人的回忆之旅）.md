---
# === 身份 ===
work_id: WORK-MVK-00776
work_name: 福冈》（2020，韩国，中韩日三人的回忆之旅）
work_name_alt: []
media_type: MVK
year_first: 2020
year_range: [2020, 2020]
season_count: null

# === 三要素(v0 已有,迁移直翻)===
environment: 现代日本福冈
cultural_paradigm: 文艺回忆+跨文化羁绊
narrative_core: 回忆之旅+情感和解

# === SDT 心理需求(TBD,等 LLM 阶段补)===
n_main: N-Rel
n_others: [N-Auto]
rel_object: mixed
t_can_carry: [T3, T1]
core_emotion_note: ""
core_conflict: 三人通过回忆之旅化解文化隔阂与情感遗憾

# === 三层标签(主检索路径,见 TAGS.md)===
theme_primary: 都市
theme_secondary: [友情, 成长, 治愈]
visual_tags: [都市夜景, 樱花飘落, 列车穿行, 老街巷, 咖啡馆]


# === 评分(v0 已有)===
acquisition_score: 2

# === 素材与玩法 ===
material_hooks: [福冈街景, 三人合影, 樱花飘落, 列车穿行, 旧照片翻动]
existing_games: []
recommended_playstyles: []
risk_notes: [受众极窄, 付费极浅]

# === 元数据 ===
source: deepseek-v4-flash + v0-migrate + tags
source_run_id: 2026-05-18-tags-enrich
evidence_level: B
review_status: auto
last_updated: 2026-05-18
---

## 迁移备注

本样本由 v0 markdown 表格迁移而来,LLM 阶段尚未补 SDT 三需求 / T 任务态 / core_conflict / material_hooks 重判。
迁移完成后 evidence_level = C,LLM 增量打标后升至 B。
