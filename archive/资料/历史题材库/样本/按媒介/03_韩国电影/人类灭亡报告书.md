---
# === 身份 ===
work_id: WORK-MVK-00024
work_name: 人类灭亡报告书
work_name_alt: []
media_type: MVK
year_first: 2011
year_range: [2011, 2012]
season_count: null

# === 三要素(v0 已有,迁移直翻)===
environment: 近未来末日世界
cultural_paradigm: 科幻+末日生存
narrative_core: 末世求生+人性抉择

# === SDT 心理需求(TBD,等 LLM 阶段补)===
n_main: N-Comp
n_others: []
rel_object: null
t_can_carry: [T5, T2]
core_emotion_note: ""
core_conflict: 人类在末日中挣扎求生，面对人性与生存的终极抉择

# === 三层标签(主检索路径,见 TAGS.md)===
theme_primary: 末日
theme_secondary: [文明崩溃, 异种入侵, 求生]
visual_tags: [废墟, 变异生物, 绝望逃亡, 末日紧张]


# === 评分(v0 已有)===
acquisition_score: 2

# === 素材与玩法 ===
material_hooks: [末日废墟, 变异生物, 绝望逃亡, 人性拷问]
existing_games: []
recommended_playstyles: []
risk_notes: [红海]

# === 元数据 ===
source: deepseek-v4-flash + v0-migrate + tags
source_run_id: 2026-05-18-tags-enrich
evidence_level: B
review_status: auto
last_updated: 2026-05-18
# v0_duplicate_count: 3
---

## 迁移备注

本样本由 v0 markdown 表格迁移而来,LLM 阶段尚未补 SDT 三需求 / T 任务态 / core_conflict / material_hooks 重判。
迁移完成后 evidence_level = C,LLM 增量打标后升至 B。
