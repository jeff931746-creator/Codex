---
# === 身份 ===
work_id: WORK-MVK-00524
work_name: 神父教育
work_name_alt: []
media_type: MVK
year_first: 2004
year_range: [2004, 2004]
season_count: null

# === 三要素(v0 已有,迁移直翻)===
environment: 现代韩国教会
cultural_paradigm: 宗教喜剧+禁忌恋爱
narrative_core: 神父修女的搞笑恋爱

# === SDT 心理需求(TBD,等 LLM 阶段补)===
n_main: N-Rel
n_others: []
rel_object: virtual
t_can_carry: [T3]
core_emotion_note: ""
core_conflict: 神父与修女在教会戒律下偷偷恋爱

# === 三层标签(主检索路径,见 TAGS.md)===
theme_primary: 喜剧
theme_secondary: [恋爱, 都市, 日常喜剧, 禁忌恋爱]
visual_tags: [教堂, 神父制服, 修女服, 都市夜景]


# === 评分(v0 已有)===
acquisition_score: 2

# === 素材与玩法 ===
material_hooks: [神父修女制服, 教堂场景, 搞笑肢体冲突, 甜蜜互动]
existing_games: []
recommended_playstyles: []
risk_notes: [受众窄, 宗教敏感, 付费浅]

# === 元数据 ===
source: deepseek-v4-flash + v0-migrate + tags
source_run_id: 2026-05-18-tags-enrich
evidence_level: B
review_status: auto
last_updated: 2026-05-18
# v0_duplicate_count: 2
---

## 迁移备注

本样本由 v0 markdown 表格迁移而来,LLM 阶段尚未补 SDT 三需求 / T 任务态 / core_conflict / material_hooks 重判。
迁移完成后 evidence_level = C,LLM 增量打标后升至 B。
