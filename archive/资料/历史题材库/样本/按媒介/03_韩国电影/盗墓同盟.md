---
# === 身份 ===
work_id: WORK-MVK-00047
work_name: 盗墓同盟
work_name_alt: []
media_type: MVK
year_first: 2020
year_range: [2020, 2020]
season_count: null

# === 三要素(v0 已有,迁移直翻)===
environment: 现代韩国地下古墓
cultural_paradigm: 盗墓+寻宝+机关陷阱
narrative_core: 团队寻宝与背叛

# === SDT 心理需求(TBD,等 LLM 阶段补)===
n_main: N-Comp
n_others: [N-Rel]
rel_object: mixed
t_can_carry: [T2, T4]
core_emotion_note: ""
core_conflict: 盗墓团队在古墓中寻宝，同时面临内部背叛与外部追捕

# === 三层标签(主检索路径,见 TAGS.md)===
theme_primary: 冒险
theme_secondary: [寻宝, 地下迷宫]
visual_tags: [古墓, 地下迷宫, 机关陷阱, 宝藏金光, 黑暗压抑]


# === 评分(v0 已有)===
acquisition_score: 3

# === 素材与玩法 ===
material_hooks: [古墓机关陷阱, 地下迷宫探险, 宝藏金光闪烁, 团队对峙场面]
existing_games: []
recommended_playstyles: []
risk_notes: [题材敏感, 版权风险]

# === 元数据 ===
source: deepseek-v4-flash + v0-migrate + tags
source_run_id: 2026-05-18-tags-enrich
evidence_level: B
review_status: auto
last_updated: 2026-05-18
# v0_duplicate_count: 2
---

## 迁移备注

本样本由 v0 markdown 表格迁移而来,LLM 阶段尚未补 SDT 三需求 / T 任务态 / core_conflict / material_hooks 重判。
迁移完成后 evidence_level = C,LLM 增量打标后升至 B。
