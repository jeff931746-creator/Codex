---
# === 身份 ===
work_id: WORK-MVK-00096
work_name: 伟大的族谱
work_name_alt: []
media_type: MVK
year_first: 2006
year_range: [2006, 2006]
season_count: null

# === 三要素(v0 已有,迁移直翻)===
environment: 现代韩国黑帮世界
cultural_paradigm: 黑帮家族+遗产争夺
narrative_core: 黑帮家族遗产争夺

# === SDT 心理需求(TBD,等 LLM 阶段补)===
n_main: N-Rel
n_others: [N-Comp]
rel_object: real
t_can_carry: [T4, T2]
core_emotion_note: ""
core_conflict: 黑帮家族内部为遗产和权力展开残酷争夺

# === 三层标签(主检索路径,见 TAGS.md)===
theme_primary: 犯罪
theme_secondary: [黑帮帝国, 权力游戏, 家庭羁绊, 复仇]
visual_tags: [都市夜景, 黑色西装, 枪林弹雨, 权力阴谋]


# === 评分(v0 已有)===
acquisition_score: 2

# === 素材与玩法 ===
material_hooks: [黑帮火拼场面, 家族会议对峙, 遗产文件特写, 权力交接仪式]
existing_games: []
recommended_playstyles: []
risk_notes: [暴力内容, 受众中等]

# === 元数据 ===
source: deepseek-v4-flash + v0-migrate + tags
source_run_id: 2026-05-18-tags-enrich
evidence_level: B
review_status: auto
last_updated: 2026-05-18
---

## 迁移备注

本样本由 v0 markdown 表格迁移而来,LLM 阶段尚未补 SDT 三需求 / T 任务态 / core_conflict / material_hooks 重判。
迁移完成后 evidence_level = C,LLM 增量打标后升至 B。
