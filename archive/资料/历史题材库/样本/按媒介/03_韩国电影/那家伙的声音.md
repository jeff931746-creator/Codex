---
# === 身份 ===
work_id: WORK-MVK-00408
work_name: 那家伙的声音
work_name_alt: []
media_type: MVK
year_first: 2007
year_range: [2007, 2007]
season_count: null

# === 三要素(v0 已有,迁移直翻)===
environment: 1990年代韩国
cultural_paradigm: 刑侦+绑架犯罪
narrative_core: 追凶无果的绝望

# === SDT 心理需求(TBD,等 LLM 阶段补)===
n_main: N-Comp
n_others: []
rel_object: null
t_can_carry: [T5]
core_emotion_note: ""
core_conflict: 父亲追查绑架犯却始终无法救回儿子

# === 三层标签(主检索路径,见 TAGS.md)===
theme_primary: 犯罪
theme_secondary: [刑侦, 悬疑, 悲剧]
visual_tags: [审讯室, 电话, 都市夜景, 家庭温情]


# === 评分(v0 已有)===
acquisition_score: 3

# === 素材与玩法 ===
material_hooks: [绑匪电话声, 审讯室对峙, 儿子最后影像, 警方无力感]
existing_games: []
recommended_playstyles: []
risk_notes: [题材压抑, 付费模式单一]

# === 元数据 ===
source: deepseek-v4-flash + v0-migrate + tags
source_run_id: 2026-05-18-tags-enrich
evidence_level: B
review_status: auto
last_updated: 2026-05-18
# v0_duplicate_count: 2
---

## 迁移备注

本样本由 v0 markdown 表格迁移而来,LLM 阶段尚未补 SDT 三需求 / T 任务态 / core_conflict / material_hooks 重判。
迁移完成后 evidence_level = C,LLM 增量打标后升至 B。
