---
# === 身份 ===
work_id: WORK-MVK-00727
work_name: 我们
work_name_alt: []
media_type: MVK
year_first: 2016
year_range: [2016, 2016]
season_count: null

# === 三要素(v0 已有,迁移直翻)===
environment: 现代韩国小学
cultural_paradigm: 校园生活+儿童心理
narrative_core: 友谊与背叛+成长阵痛

# === SDT 心理需求(TBD,等 LLM 阶段补)===
n_main: N-Rel
n_others: []
rel_object: virtual
t_can_carry: [T3]
core_emotion_note: ""
core_conflict: 一群小学生在友谊与背叛中经历成长阵痛

# === 三层标签(主检索路径,见 TAGS.md)===
theme_primary: 友情
theme_secondary: [校园, 成长, 日常喜剧, 青春恋爱]
visual_tags: [校园青春, 教室, 操场, 放学小路, 阳光治愈]


# === 评分(v0 已有)===
acquisition_score: 2

# === 素材与玩法 ===
material_hooks: [教室课桌, 放学小路, 操场秋千, 书包文具]
existing_games: []
recommended_playstyles: []
risk_notes: [受众窄, 付费浅]

# === 元数据 ===
source: deepseek-v4-flash + v0-migrate + tags
source_run_id: 2026-05-18-tags-enrich
evidence_level: B
review_status: auto
last_updated: 2026-05-18
---

## 迁移备注

本样本由 v0 markdown 表格迁移而来,LLM 阶段尚未补 SDT 三需求 / T 任务态 / core_conflict / material_hooks 重判。
迁移完成后 evidence_level = C,LLM 增量打标后升至 B。
