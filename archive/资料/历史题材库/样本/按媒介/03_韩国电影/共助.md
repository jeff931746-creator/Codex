---
# === 身份 ===
work_id: WORK-MVK-00141
work_name: 共助
work_name_alt: []
media_type: MVK
year_first: 2017
year_range: [2017, 2017]
season_count: null

# === 三要素(v0 已有,迁移直翻)===
environment: 现代韩国/朝鲜
cultural_paradigm: 南北韩特工+枪战动作
narrative_core: 双雄联手+追捕

# === SDT 心理需求(TBD,等 LLM 阶段补)===
n_main: N-Rel
n_others: [N-Comp]
rel_object: mixed
t_can_carry: [T4, T2]
core_emotion_note: ""
core_conflict: 南北韩特工被迫联手追捕叛徒，在信任与背叛间博弈

# === 三层标签(主检索路径,见 TAGS.md)===
theme_primary: 犯罪
theme_secondary: [谍战, 政治权谋, 兄弟情]
visual_tags: [都市夜景, 枪林弹雨, 爆炸场面, 黑色西装]


# === 评分(v0 已有)===
acquisition_score: 3

# === 素材与玩法 ===
material_hooks: [双雄对峙, 枪战巷战, 追车爆破, 韩朝符号]
existing_games: []
recommended_playstyles: []
risk_notes: [政治敏感]

# === 元数据 ===
source: deepseek-v4-flash + v0-migrate + tags
source_run_id: 2026-05-18-tags-enrich
evidence_level: B
review_status: auto
last_updated: 2026-05-18
---

## 迁移备注

本样本由 v0 markdown 表格迁移而来,LLM 阶段尚未补 SDT 三需求 / T 任务态 / core_conflict / material_hooks 重判。
迁移完成后 evidence_level = C,LLM 增量打标后升至 B。
