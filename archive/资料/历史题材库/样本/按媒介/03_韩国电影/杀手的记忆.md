---
# === 身份 ===
work_id: WORK-MVK-00387
work_name: 杀手的记忆
work_name_alt: []
media_type: MVK
year_first: 2001
year_range: [2001, 2001]
season_count: null

# === 三要素(v0 已有,迁移直翻)===
environment: 现代都市黑帮
cultural_paradigm: 杀手组织+搞笑内讧
narrative_core: 内部搞笑冲突

# === SDT 心理需求(TBD,等 LLM 阶段补)===
n_main: N-Rel
n_others: []
rel_object: real
t_can_carry: [T4]
core_emotion_note: ""
core_conflict: 杀手组织内部搞笑内讧与任务执行间的矛盾

# === 三层标签(主检索路径,见 TAGS.md)===
theme_primary: 犯罪
theme_secondary: [黑帮, 喜剧, 都市, 黑色幽默]
visual_tags: [都市夜景, 黑色西装, 枪林弹雨, 搞笑荒诞]


# === 评分(v0 已有)===
acquisition_score: 2

# === 素材与玩法 ===
material_hooks: [黑帮火并, 搞笑内讧, 反转刺杀, 兄弟互坑]
existing_games: []
recommended_playstyles: []
risk_notes: [IP冷门, 受众窄]

# === 元数据 ===
source: deepseek-v4-flash + v0-migrate + tags
source_run_id: 2026-05-18-tags-enrich
evidence_level: B
review_status: auto
last_updated: 2026-05-18
---

## 迁移备注

本样本由 v0 markdown 表格迁移而来,LLM 阶段尚未补 SDT 三需求 / T 任务态 / core_conflict / material_hooks 重判。
迁移完成后 evidence_level = C,LLM 增量打标后升至 B。
