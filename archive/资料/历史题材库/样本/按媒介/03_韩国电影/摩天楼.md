---
# === 身份 ===
work_id: WORK-MVK-00027
work_name: 摩天楼
work_name_alt: []
media_type: MVK
year_first: 2012
year_range: [2012, 2013]
season_count: null

# === 三要素(v0 已有,迁移直翻)===
environment: 现代韩国圣诞夜摩天楼
cultural_paradigm: 灾难逃生与消防体系
narrative_core: 高楼火灾极限求生

# === SDT 心理需求(TBD,等 LLM 阶段补)===
n_main: N-Comp
n_others: []
rel_object: null
t_can_carry: [T2, T1]
core_emotion_note: ""
core_conflict: 被困摩天楼火灾中，消防员与幸存者争分夺秒逃生

# === 三层标签(主检索路径,见 TAGS.md)===
theme_primary: 灾难
theme_secondary: [城市火灾, 求生, 小队求生, 职场, 都市]
visual_tags: [摩天楼, 火灾, 浓烟, 玻璃幕墙碎裂, 直升机救援]


# === 评分(v0 已有)===
acquisition_score: 3

# === 素材与玩法 ===
material_hooks: [摩天楼大火, 电梯井坠落, 玻璃幕墙碎裂, 直升机救援, 浓烟窒息]
existing_games: []
recommended_playstyles: []
risk_notes: [版权成本高, 同质化风险]

# === 元数据 ===
source: deepseek-v4-flash + v0-migrate + tags
source_run_id: 2026-05-18-tags-enrich
evidence_level: B
review_status: auto
last_updated: 2026-05-18
# v0_duplicate_count: 3
---

## 迁移备注

本样本由 v0 markdown 表格迁移而来,LLM 阶段尚未补 SDT 三需求 / T 任务态 / core_conflict / material_hooks 重判。
迁移完成后 evidence_level = C,LLM 增量打标后升至 B。
