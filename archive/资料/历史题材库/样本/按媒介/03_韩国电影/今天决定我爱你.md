---
# === 身份 ===
work_id: WORK-MVK-00677
work_name: 今天决定我爱你
work_name_alt: []
media_type: MVK
year_first: 2021
year_range: [2021, 2021]
season_count: null

# === 三要素(v0 已有,迁移直翻)===
environment: 现代都市/跨年夜
cultural_paradigm: 都市爱情+多线叙事
narrative_core: 多对情侣跨年故事

# === SDT 心理需求(TBD,等 LLM 阶段补)===
n_main: N-Rel
n_others: []
rel_object: virtual
t_can_carry: [T3]
core_emotion_note: ""
core_conflict: 多对情侣在跨年夜面对爱情抉择与情感考验

# === 三层标签(主检索路径,见 TAGS.md)===
theme_primary: 都市
theme_secondary: [恋爱, 日常喜剧, 成长]
visual_tags: [都市夜景, 霓虹灯, 雪中亲吻, 烟花]


# === 评分(v0 已有)===
acquisition_score: 2

# === 素材与玩法 ===
material_hooks: [跨年烟花, 情侣拥抱, 城市夜景, 圣诞装饰, 雪中亲吻]
existing_games: []
recommended_playstyles: []
risk_notes: [受众窄, 时效性短]

# === 元数据 ===
source: deepseek-v4-flash + v0-migrate + tags
source_run_id: 2026-05-18-tags-enrich
evidence_level: B
review_status: auto
last_updated: 2026-05-18
---

## 迁移备注

本样本由 v0 markdown 表格迁移而来,LLM 阶段尚未补 SDT 三需求 / T 任务态 / core_conflict / material_hooks 重判。
迁移完成后 evidence_level = C,LLM 增量打标后升至 B。
