---
# === 身份 ===
work_id: WORK-MVK-00244
work_name: 举起金刚
work_name_alt: []
media_type: MVK
year_first: 2009
year_range: [2009, 2009]
season_count: null

# === 三要素(v0 已有,迁移直翻)===
environment: 现代韩国乡村
cultural_paradigm: 体育竞技+举重训练
narrative_core: 追梦逆袭

# === SDT 心理需求(TBD,等 LLM 阶段补)===
n_main: N-Comp
n_others: [N-Rel]
rel_object: mixed
t_can_carry: [T2, T4]
core_emotion_note: ""
core_conflict: 乡村少女举重队冲击全国冠军，对抗训练伤痛与外界偏见

# === 三层标签(主检索路径,见 TAGS.md)===
theme_primary: 体育竞技
theme_secondary: [成长, 励志, 逆袭, 乡村田园]
visual_tags: [杠铃砸地, 肌肉线条, 训练汗水, 乡村田野, 比赛怒吼]


# === 评分(v0 已有)===
acquisition_score: 2

# === 素材与玩法 ===
material_hooks: [杠铃砸地瞬间, 肌肉线条特写, 训练汗水飞溅, 比赛怒吼表情, 乡村田野背景]
existing_games: []
recommended_playstyles: []
risk_notes: [题材小众, 玩法同质化]

# === 元数据 ===
source: deepseek-v4-flash + v0-migrate + tags
source_run_id: 2026-05-18-tags-enrich
evidence_level: B
review_status: auto
last_updated: 2026-05-18
# v0_duplicate_count: 2
---

## 迁移备注

本样本由 v0 markdown 表格迁移而来,LLM 阶段尚未补 SDT 三需求 / T 任务态 / core_conflict / material_hooks 重判。
迁移完成后 evidence_level = C,LLM 增量打标后升至 B。
