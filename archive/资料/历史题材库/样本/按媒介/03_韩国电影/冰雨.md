---
# === 身份 ===
work_id: WORK-MVK-00215
work_name: 冰雨
work_name_alt: []
media_type: MVK
year_first: 2004
year_range: [2004, 2004]
season_count: null

# === 三要素(v0 已有,迁移直翻)===
environment: 韩国雪山/登山场景
cultural_paradigm: 极限运动+三角恋
narrative_core: 生存冒险+爱情抉择

# === SDT 心理需求(TBD,等 LLM 阶段补)===
n_main: N-Rel
n_others: [N-Comp]
rel_object: real
t_can_carry: [T3, T2]
core_emotion_note: ""
core_conflict: 雪山极限生存中，爱情与背叛的抉择

# === 三层标签(主检索路径,见 TAGS.md)===
theme_primary: 求生
theme_secondary: [极地求生, 恋爱, 灾难, 小队求生]
visual_tags: [雪原, 山脉, 暴风雪, 生存绝望]


# === 评分(v0 已有)===
acquisition_score: 3

# === 素材与玩法 ===
material_hooks: [雪山崩塌, 冰壁攀爬, 暴风雪救援, 悬崖对峙]
existing_games: []
recommended_playstyles: []
risk_notes: [受众中等, 题材冷门]

# === 元数据 ===
source: deepseek-v4-flash + v0-migrate + tags
source_run_id: 2026-05-18-tags-enrich
evidence_level: B
review_status: auto
last_updated: 2026-05-18
---

## 迁移备注

本样本由 v0 markdown 表格迁移而来,LLM 阶段尚未补 SDT 三需求 / T 任务态 / core_conflict / material_hooks 重判。
迁移完成后 evidence_level = C,LLM 增量打标后升至 B。
