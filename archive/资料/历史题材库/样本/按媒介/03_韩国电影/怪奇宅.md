---
# === 身份 ===
work_id: WORK-MVK-00350
work_name: 怪奇宅
work_name_alt: []
media_type: MVK
year_first: 2021
year_range: [2021, 2021]
season_count: null

# === 三要素(v0 已有,迁移直翻)===
environment: 现代都市/老旧公寓
cultural_paradigm: 东亚都市怪谈+灵异体系
narrative_core: 单元式恐怖探索

# === SDT 心理需求(TBD,等 LLM 阶段补)===
n_main: N-Comp
n_others: []
rel_object: null
t_can_carry: [T5, T1]
core_emotion_note: ""
core_conflict: 住户在老旧公寓中遭遇诡异事件，探索隐藏的恐怖真相

# === 三层标签(主检索路径,见 TAGS.md)===
theme_primary: 恐怖
theme_secondary: [灵异, 怪谈, 悬疑, 都市]
visual_tags: [老旧公寓, 黑暗楼梯间, 血迹斑斑墙壁, 诡异房间门缝, 模糊人影闪现]


# === 评分(v0 已有)===
acquisition_score: 2

# === 素材与玩法 ===
material_hooks: [老旧公寓走廊, 诡异房间门缝, 黑暗楼梯间, 模糊人影闪现, 血迹斑斑墙壁]
existing_games: []
recommended_playstyles: []
risk_notes: [受众窄, 恐怖审查风险]

# === 元数据 ===
source: deepseek-v4-flash + v0-migrate + tags
source_run_id: 2026-05-18-tags-enrich
evidence_level: B
review_status: auto
last_updated: 2026-05-18
---

## 迁移备注

本样本由 v0 markdown 表格迁移而来,LLM 阶段尚未补 SDT 三需求 / T 任务态 / core_conflict / material_hooks 重判。
迁移完成后 evidence_level = C,LLM 增量打标后升至 B。
