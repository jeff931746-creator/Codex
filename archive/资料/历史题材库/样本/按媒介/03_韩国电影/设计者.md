---
# === 身份 ===
work_id: WORK-MVK-00369
work_name: 设计者
work_name_alt: []
media_type: MVK
year_first: 2024
year_range: [2024, 2024]
season_count: null

# === 三要素(v0 已有,迁移直翻)===
environment: 现代都市犯罪圈
cultural_paradigm: 犯罪策划+心理博弈
narrative_core: 设局复仇+反转

# === SDT 心理需求(TBD,等 LLM 阶段补)===
n_main: N-Comp
n_others: []
rel_object: null
t_can_carry: [T5, T2]
core_emotion_note: ""
core_conflict: 主角设局复仇，与犯罪集团展开心理博弈与反转

# === 三层标签(主检索路径,见 TAGS.md)===
theme_primary: 犯罪
theme_secondary: [复仇, 心理悬疑, 都市, 智斗]
visual_tags: [都市夜景, 犯罪现场, 心理对峙, 反转瞬间]


# === 评分(v0 已有)===
acquisition_score: 3

# === 素材与玩法 ===
material_hooks: [精密陷阱, 反转瞬间, 犯罪现场, 心理对峙]
existing_games: []
recommended_playstyles: []
risk_notes: [红海竞争, 剧情依赖强]

# === 元数据 ===
source: deepseek-v4-flash + v0-migrate + tags
source_run_id: 2026-05-18-tags-enrich
evidence_level: B
review_status: auto
last_updated: 2026-05-18
---

## 迁移备注

本样本由 v0 markdown 表格迁移而来,LLM 阶段尚未补 SDT 三需求 / T 任务态 / core_conflict / material_hooks 重判。
迁移完成后 evidence_level = C,LLM 增量打标后升至 B。
