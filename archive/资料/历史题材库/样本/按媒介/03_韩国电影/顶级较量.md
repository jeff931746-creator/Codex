---
# === 身份 ===
work_id: WORK-MVK-00130
work_name: 顶级较量
work_name_alt: []
media_type: MVK
year_first: 2014
year_range: [2014, 2014]
season_count: null

# === 三要素(v0 已有,迁移直翻)===
environment: 现代韩国地下格斗场
cultural_paradigm: 格斗搏击+黑帮势力
narrative_core: 逆袭复仇+热血战斗

# === SDT 心理需求(TBD,等 LLM 阶段补)===
n_main: N-Comp
n_others: [N-Rel]
rel_object: real
t_can_carry: [T2, T4]
core_emotion_note: ""
core_conflict: 主角在地下格斗场逆袭复仇，同时对抗黑帮势力的压迫

# === 三层标签(主检索路径,见 TAGS.md)===
theme_primary: 都市
theme_secondary: [热血战斗, 复仇, 犯罪, 成长]
visual_tags: [地下擂台, 霓虹灯, 都市夜景, 拳拳到肉]


# === 评分(v0 已有)===
acquisition_score: 3

# === 素材与玩法 ===
material_hooks: [拳拳到肉格斗, 地下擂台场景, 逆袭反击瞬间, 黑帮对峙场面]
existing_games: []
recommended_playstyles: []
risk_notes: [红海, 版权成本中等]

# === 元数据 ===
source: deepseek-v4-flash + v0-migrate + tags
source_run_id: 2026-05-18-tags-enrich
evidence_level: B
review_status: auto
last_updated: 2026-05-18
# v0_duplicate_count: 2
---

## 迁移备注

本样本由 v0 markdown 表格迁移而来,LLM 阶段尚未补 SDT 三需求 / T 任务态 / core_conflict / material_hooks 重判。
迁移完成后 evidence_level = C,LLM 增量打标后升至 B。
