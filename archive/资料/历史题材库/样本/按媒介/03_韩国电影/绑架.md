---
# === 身份 ===
work_id: WORK-MVK-00346
work_name: 绑架
work_name_alt: []
media_type: MVK
year_first: 2021
year_range: [2021, 2024]
season_count: null

# === 三要素(v0 已有,迁移直翻)===
environment: 现代韩国都市
cultural_paradigm: 人质营救+逃亡追捕
narrative_core: 人质营救与逃亡

# === SDT 心理需求(TBD,等 LLM 阶段补)===
n_main: N-Comp
n_others: []
rel_object: null
t_can_carry: [T2, T5]
core_emotion_note: ""
core_conflict: 人质在警方与绑匪的博弈中求生并反击

# === 三层标签(主检索路径,见 TAGS.md)===
theme_primary: 犯罪
theme_secondary: [求生, 悬疑, 都市, 复仇]
visual_tags: [都市夜景, 枪林弹雨, 爆炸场面, 审讯室对峙]


# === 评分(v0 已有)===
acquisition_score: 3

# === 素材与玩法 ===
material_hooks: [绑匪面具特写, 追逐枪战, 人质逃脱瞬间, 审讯室对峙]
existing_games: []
recommended_playstyles: []
risk_notes: [红海, 版权成本中]

# === 元数据 ===
source: deepseek-v4-flash + v0-migrate + tags
source_run_id: 2026-05-18-tags-enrich
evidence_level: B
review_status: auto
last_updated: 2026-05-18
# v0_duplicate_count: 2
---

## 迁移备注

本样本由 v0 markdown 表格迁移而来,LLM 阶段尚未补 SDT 三需求 / T 任务态 / core_conflict / material_hooks 重判。
迁移完成后 evidence_level = C,LLM 增量打标后升至 B。
