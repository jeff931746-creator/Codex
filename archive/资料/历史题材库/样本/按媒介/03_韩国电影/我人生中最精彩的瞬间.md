---
# === 身份 ===
work_id: WORK-MVK-00412
work_name: 我人生中最精彩的瞬间
work_name_alt: []
media_type: MVK
year_first: 2008
year_range: [2008, 2008]
season_count: null

# === 三要素(v0 已有,迁移直翻)===
environment: 现代韩国体育赛场
cultural_paradigm: 女子手球竞技体育
narrative_core: 团队逆袭冲击奖牌

# === SDT 心理需求(TBD,等 LLM 阶段补)===
n_main: N-Rel
n_others: [N-Comp]
rel_object: real
t_can_carry: [T4, T2]
core_emotion_note: ""
core_conflict: 女子手球队在逆境中团结拼搏，冲击奥运奖牌

# === 三层标签(主检索路径,见 TAGS.md)===
theme_primary: 体育竞技
theme_secondary: [成长, 励志, 热血战斗]
visual_tags: [赛场激烈对抗, 团队击掌庆祝, 汗水与泪水, 绝杀瞬间]


# === 评分(v0 已有)===
acquisition_score: 2

# === 素材与玩法 ===
material_hooks: [赛场激烈对抗, 团队击掌庆祝, 汗水与泪水, 绝杀瞬间]
existing_games: []
recommended_playstyles: []
risk_notes: [受众窄, 红海]

# === 元数据 ===
source: deepseek-v4-flash + v0-migrate + tags
source_run_id: 2026-05-18-tags-enrich
evidence_level: B
review_status: auto
last_updated: 2026-05-18
# v0_duplicate_count: 2
---

## 迁移备注

本样本由 v0 markdown 表格迁移而来,LLM 阶段尚未补 SDT 三需求 / T 任务态 / core_conflict / material_hooks 重判。
迁移完成后 evidence_level = C,LLM 增量打标后升至 B。
