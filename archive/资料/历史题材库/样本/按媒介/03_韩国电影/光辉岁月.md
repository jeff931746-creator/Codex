---
# === 身份 ===
work_id: WORK-MVK-00450
work_name: 光辉岁月
work_name_alt: []
media_type: MVK
year_first: 2015
year_range: [2015, 2015]
season_count: null

# === 三要素(v0 已有,迁移直翻)===
environment: 现代韩国校园
cultural_paradigm: 青春友情+街头斗殴
narrative_core: 成长逆袭+兄弟情

# === SDT 心理需求(TBD,等 LLM 阶段补)===
n_main: N-Rel
n_others: [N-Comp]
rel_object: real
t_can_carry: [T4, T2]
core_emotion_note: ""
core_conflict: 校园帮派争斗中兄弟情义与个人成长的冲突

# === 三层标签(主检索路径,见 TAGS.md)===
theme_primary: 友情
theme_secondary: [校园, 成长, 青春恋爱, 热血战斗]
visual_tags: [校园, 操场, 教室, 夕阳, 街头斗殴]


# === 评分(v0 已有)===
acquisition_score: 2

# === 素材与玩法 ===
material_hooks: [街头群架场面, 兄弟并肩背影, 校园夕阳奔跑, 拳头对决特写]
existing_games: []
recommended_playstyles: []
risk_notes: [红海, 受众年龄窄]

# === 元数据 ===
source: deepseek-v4-flash + v0-migrate + tags
source_run_id: 2026-05-18-tags-enrich
evidence_level: B
review_status: auto
last_updated: 2026-05-18
---

## 迁移备注

本样本由 v0 markdown 表格迁移而来,LLM 阶段尚未补 SDT 三需求 / T 任务态 / core_conflict / material_hooks 重判。
迁移完成后 evidence_level = C,LLM 增量打标后升至 B。
