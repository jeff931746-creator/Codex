---
# === 身份 ===
work_id: WORK-MVK-00198
work_name: 光复节特赦
work_name_alt: []
media_type: MVK
year_first: 2002
year_range: [2002, 2002]
season_count: null

# === 三要素(v0 已有,迁移直翻)===
environment: 现代韩国监狱
cultural_paradigm: 越狱+父女亲情
narrative_core: 越狱冒险+亲情救赎

# === SDT 心理需求(TBD,等 LLM 阶段补)===
n_main: N-Rel
n_others: [N-Auto]
rel_object: real
t_can_carry: [T4, T3]
core_emotion_note: ""
core_conflict: 父亲为救女儿越狱，与监狱系统和追捕力量对抗

# === 三层标签(主检索路径,见 TAGS.md)===
theme_primary: 亲情
theme_secondary: [冒险, 救赎, 父女]
visual_tags: [监狱高墙, 越狱追逐, 都市夜景, 父女重逢]


# === 评分(v0 已有)===
acquisition_score: 3

# === 素材与玩法 ===
material_hooks: [监狱高墙压迫, 越狱惊险追逐, 父女重逢瞬间, 自由曙光]
existing_games: []
recommended_playstyles: []
risk_notes: [题材敏感, 受众中等]

# === 元数据 ===
source: deepseek-v4-flash + v0-migrate + tags
source_run_id: 2026-05-18-tags-enrich
evidence_level: B
review_status: auto
last_updated: 2026-05-18
# v0_duplicate_count: 2
---

## 迁移备注

本样本由 v0 markdown 表格迁移而来,LLM 阶段尚未补 SDT 三需求 / T 任务态 / core_conflict / material_hooks 重判。
迁移完成后 evidence_level = C,LLM 增量打标后升至 B。
