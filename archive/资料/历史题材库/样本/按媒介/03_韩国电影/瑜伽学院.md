---
# === 身份 ===
work_id: WORK-MVK-00103
work_name: 瑜伽学院
work_name_alt: []
media_type: MVK
year_first: 2009
year_range: [2009, 2009]
season_count: null

# === 三要素(v0 已有,迁移直翻)===
environment: 现代韩国都市
cultural_paradigm: 瑜伽修炼+恐怖超自然
narrative_core: 恐怖事件与生存

# === SDT 心理需求(TBD,等 LLM 阶段补)===
n_main: N-Comp
n_others: []
rel_object: null
t_can_carry: [T2, T5]
core_emotion_note: ""
core_conflict: 瑜伽修炼引发身体异变与恐怖事件，主角在生存与失控间挣扎

# === 三层标签(主检索路径,见 TAGS.md)===
theme_primary: 恐怖
theme_secondary: [超自然悬疑, 求生, 都市, 心理恐怖]
visual_tags: [黑暗压抑, 瑜伽馆, 身体撕裂变形, 诡异镜中倒影]


# === 评分(v0 已有)===
acquisition_score: 2

# === 素材与玩法 ===
material_hooks: [扭曲瑜伽姿势, 诡异镜中倒影, 身体撕裂变形, 黑暗瑜伽馆, 血染瑜伽垫]
existing_games: []
recommended_playstyles: []
risk_notes: [题材敏感, 受众窄]

# === 元数据 ===
source: deepseek-v4-flash + v0-migrate + tags
source_run_id: 2026-05-18-tags-enrich
evidence_level: B
review_status: auto
last_updated: 2026-05-18
# v0_duplicate_count: 2
---

## 迁移备注

本样本由 v0 markdown 表格迁移而来,LLM 阶段尚未补 SDT 三需求 / T 任务态 / core_conflict / material_hooks 重判。
迁移完成后 evidence_level = C,LLM 增量打标后升至 B。
