---
# === 身份 ===
work_id: WORK-MVK-00703
work_name: 素媛
work_name_alt: []
media_type: MVK
year_first: 2013
year_range: [2013, 2013]
season_count: null

# === 三要素(v0 已有,迁移直翻)===
environment: 现代韩国小城
cultural_paradigm: 性侵创伤+家庭疗伤
narrative_core: 受害者家庭治愈之路

# === SDT 心理需求(TBD,等 LLM 阶段补)===
n_main: N-Rel
n_others: []
rel_object: real
t_can_carry: [T3]
core_emotion_note: ""
core_conflict: 受害女孩家庭在创伤中重建信任与爱

# === 三层标签(主检索路径,见 TAGS.md)===
theme_primary: 家庭
theme_secondary: [亲情, 犯罪, 治愈, 成长]
visual_tags: [校园, 家庭温情, 法庭, 心理治疗室]


# === 评分(v0 已有)===
acquisition_score: 4

# === 素材与玩法 ===
material_hooks: [小女孩纯真眼神, 法庭对峙场景, 父亲笨拙守护, 心理治疗室]
existing_games: []
recommended_playstyles: []
risk_notes: [题材极敏感, 受众极窄, 版权成本高]

# === 元数据 ===
source: deepseek-v4-flash + v0-migrate + tags
source_run_id: 2026-05-18-tags-enrich
evidence_level: B
review_status: auto
last_updated: 2026-05-18
# v0_duplicate_count: 2
---

## 迁移备注

本样本由 v0 markdown 表格迁移而来,LLM 阶段尚未补 SDT 三需求 / T 任务态 / core_conflict / material_hooks 重判。
迁移完成后 evidence_level = C,LLM 增量打标后升至 B。
