---
# === 身份 ===
work_id: WORK-MVK-00343
work_name: 通话
work_name_alt: []
media_type: MVK
year_first: 2020
year_range: [2020, 2021]
season_count: null

# === 三要素(v0 已有,迁移直翻)===
environment: 现代韩国+跨时空
cultural_paradigm: 时空连接与悬疑推理
narrative_core: 跨时空通话改变命运

# === SDT 心理需求(TBD,等 LLM 阶段补)===
n_main: N-Rel
n_others: [N-Comp]
rel_object: mixed
t_can_carry: [T5, T2]
core_emotion_note: ""
core_conflict: 通过跨时空通话试图改变亲人命运，却引发连锁悬疑事件

# === 三层标签(主检索路径,见 TAGS.md)===
theme_primary: 悬疑
theme_secondary: [时间轮回, 亲情, 犯罪, 心理悬疑]
visual_tags: [都市夜景, 电话铃声, 时空交错, 悬疑诡异]


# === 评分(v0 已有)===
acquisition_score: 3

# === 素材与玩法 ===
material_hooks: [电话铃声悬念, 时空交错画面, 关键线索闪现, 亲情对峙瞬间]
existing_games: []
recommended_playstyles: []
risk_notes: [玩法同质化风险]

# === 元数据 ===
source: deepseek-v4-flash + v0-migrate + tags
source_run_id: 2026-05-18-tags-enrich
evidence_level: B
review_status: auto
last_updated: 2026-05-18
# v0_duplicate_count: 2
---

## 迁移备注

本样本由 v0 markdown 表格迁移而来,LLM 阶段尚未补 SDT 三需求 / T 任务态 / core_conflict / material_hooks 重判。
迁移完成后 evidence_level = C,LLM 增量打标后升至 B。
