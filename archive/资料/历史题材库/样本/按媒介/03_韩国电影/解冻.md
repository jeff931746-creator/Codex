---
# === 身份 ===
work_id: WORK-MVK-00634
work_name: 解冻
work_name_alt: []
media_type: MVK
year_first: 2017
year_range: [2017, 2017]
season_count: null

# === 三要素(v0 已有,迁移直翻)===
environment: 现代韩国冬季小镇
cultural_paradigm: 冰封尸体+连环杀人案
narrative_core: 冰封秘密与连环追凶

# === SDT 心理需求(TBD,等 LLM 阶段补)===
n_main: N-Comp
n_others: []
rel_object: null
t_can_carry: [T5, T2]
core_emotion_note: ""
core_conflict: 冰封尸体隐藏连环杀人真相，追凶者与凶手博弈

# === 三层标签(主检索路径,见 TAGS.md)===
theme_primary: 悬疑
theme_secondary: [犯罪, 连环案, 身份谜团, 心理悬疑]
visual_tags: [雪原, 小镇雪夜, 冰面下尸体浮现, 悬疑诡异]


# === 评分(v0 已有)===
acquisition_score: 2

# === 素材与玩法 ===
material_hooks: [冰封尸体特写, 雪地追凶场景, 冰面下尸体浮现, 小镇雪夜氛围]
existing_games: []
recommended_playstyles: []
risk_notes: [受众窄, 题材压抑]

# === 元数据 ===
source: deepseek-v4-flash + v0-migrate + tags
source_run_id: 2026-05-18-tags-enrich
evidence_level: B
review_status: auto
last_updated: 2026-05-18
---

## 迁移备注

本样本由 v0 markdown 表格迁移而来,LLM 阶段尚未补 SDT 三需求 / T 任务态 / core_conflict / material_hooks 重判。
迁移完成后 evidence_level = C,LLM 增量打标后升至 B。
