---
# === 身份 ===
work_id: WORK-MVK-00744
work_name: 大韩民国宪法
work_name_alt: []
media_type: MVK
year_first: 2003
year_range: [2003, 2003]
season_count: null

# === 三要素(v0 已有,迁移直翻)===
environment: 现代韩国
cultural_paradigm: 法律政治+荒诞喜剧
narrative_core: 宪法条款引发的闹剧

# === SDT 心理需求(TBD,等 LLM 阶段补)===
n_main: N-Rel
n_others: [N-Auto]
rel_object: real
t_can_carry: [T4, T5]
core_emotion_note: ""
core_conflict: 宪法条款被荒诞解读，引发各方势力争夺解释权

# === 三层标签(主检索路径,见 TAGS.md)===
theme_primary: 政治权谋
theme_secondary: [喜剧, 现代政坛, 权力游戏, 荒诞喜剧]
visual_tags: [法庭, 都市夜景, 黑色西装, 政治紧张]


# === 评分(v0 已有)===
acquisition_score: 2

# === 素材与玩法 ===
material_hooks: [法庭对峙, 宪法条文特写, 荒诞闹剧场面]
existing_games: []
recommended_playstyles: []
risk_notes: [受众极窄, 政治敏感]

# === 元数据 ===
source: deepseek-v4-flash + v0-migrate + tags
source_run_id: 2026-05-18-tags-enrich
evidence_level: B
review_status: auto
last_updated: 2026-05-18
# v0_duplicate_count: 2
---

## 迁移备注

本样本由 v0 markdown 表格迁移而来,LLM 阶段尚未补 SDT 三需求 / T 任务态 / core_conflict / material_hooks 重判。
迁移完成后 evidence_level = C,LLM 增量打标后升至 B。
