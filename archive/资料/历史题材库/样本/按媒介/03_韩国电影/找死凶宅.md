---
# === 身份 ===
work_id: WORK-MVK-00364
work_name: 找死凶宅
work_name_alt: []
media_type: MVK
year_first: 2024
year_range: [2024, 2024]
season_count: null

# === 三要素(v0 已有,迁移直翻)===
environment: 现代韩国/凶宅灵异
cultural_paradigm: 骗子+鬼怪悬疑
narrative_core: 追查真相/反转求生

# === SDT 心理需求(TBD,等 LLM 阶段补)===
n_main: N-Comp
n_others: []
rel_object: null
t_can_carry: [T5, T2]
core_emotion_note: ""
core_conflict: 骗子误入凶宅，在鬼怪追杀中反转求生并揭开真相

# === 三层标签(主检索路径,见 TAGS.md)===
theme_primary: 悬疑
theme_secondary: [灵异, 恐怖, 求生]
visual_tags: [凶宅阴森走廊, 鬼怪突脸惊吓, 反转逃生陷阱, 骗子假扮驱魔]


# === 评分(v0 已有)===
acquisition_score: 2

# === 素材与玩法 ===
material_hooks: [凶宅阴森走廊, 鬼怪突脸惊吓, 反转逃生陷阱, 骗子假扮驱魔]
existing_games: []
recommended_playstyles: []
risk_notes: [审核风险, 受众窄]

# === 元数据 ===
source: deepseek-v4-flash + v0-migrate + tags
source_run_id: 2026-05-18-tags-enrich
evidence_level: B
review_status: auto
last_updated: 2026-05-18
---

## 迁移备注

本样本由 v0 markdown 表格迁移而来,LLM 阶段尚未补 SDT 三需求 / T 任务态 / core_conflict / material_hooks 重判。
迁移完成后 evidence_level = C,LLM 增量打标后升至 B。
