---
# === 身份 ===
work_id: WORK-MVK-00495
work_name: 青春》
work_name_alt: []
media_type: MVK
year_first: 2000
year_range: [2000, 2000]
season_count: null

# === 三要素(v0 已有,迁移直翻)===
environment: 1990年代韩国小镇
cultural_paradigm: 青春成长+性启蒙
narrative_core: 性探索与小镇青春

# === SDT 心理需求(TBD,等 LLM 阶段补)===
n_main: N-Auto
n_others: []
rel_object: null
t_can_carry: [T1]
core_emotion_note: ""
core_conflict: 小镇少年在性启蒙与青春迷茫中寻找自我

# === 三层标签(主检索路径,见 TAGS.md)===
theme_primary: 都市
theme_secondary: [青春恋爱, 成长, 小镇生活, 日常喜剧]
visual_tags: [小镇夏日, 单车穿行, 校园青春, 阳光治愈]


# === 评分(v0 已有)===
acquisition_score: 3

# === 素材与玩法 ===
material_hooks: [小镇夏日, 少年群像, 秘密基地, 单车穿行]
existing_games: []
recommended_playstyles: []
risk_notes: [题材敏感, 受众窄, 付费浅]

# === 元数据 ===
source: deepseek-v4-flash + v0-migrate + tags
source_run_id: 2026-05-18-tags-enrich
evidence_level: B
review_status: auto
last_updated: 2026-05-18
---

## 迁移备注

本样本由 v0 markdown 表格迁移而来,LLM 阶段尚未补 SDT 三需求 / T 任务态 / core_conflict / material_hooks 重判。
迁移完成后 evidence_level = C,LLM 增量打标后升至 B。
