---
# === 身份 ===
work_id: WORK-MVK-00718
work_name: 完美广播
work_name_alt: []
media_type: MVK
year_first: 2011
year_range: [2011, 2011]
season_count: null

# === 三要素(v0 已有,迁移直翻)===
environment: 现代韩国/电台与都市
cultural_paradigm: 浪漫喜剧+职场日常
narrative_core: 爱情成长+梦想追逐

# === SDT 心理需求(TBD,等 LLM 阶段补)===
n_main: N-Rel
n_others: [N-Auto]
rel_object: virtual
t_can_carry: [T3, T1]
core_emotion_note: ""
core_conflict: 电台DJ与听众/同事间的情感纠葛与自我成长

# === 三层标签(主检索路径,见 TAGS.md)===
theme_primary: 都市
theme_secondary: [恋爱, 职场, 成长, 日常喜剧]
visual_tags: [都市夜景, 咖啡馆, 电台录音室, 雨伞共撑, 耳机特写]


# === 评分(v0 已有)===
acquisition_score: 2

# === 素材与玩法 ===
material_hooks: [电台录音室, 都市夜景, 雨伞共撑, 耳机特写, 咖啡杯]
existing_games: []
recommended_playstyles: []
risk_notes: [受众窄, 付费浅, 红海]

# === 元数据 ===
source: deepseek-v4-flash + v0-migrate + tags
source_run_id: 2026-05-18-tags-enrich
evidence_level: B
review_status: auto
last_updated: 2026-05-18
---

## 迁移备注

本样本由 v0 markdown 表格迁移而来,LLM 阶段尚未补 SDT 三需求 / T 任务态 / core_conflict / material_hooks 重判。
迁移完成后 evidence_level = C,LLM 增量打标后升至 B。
