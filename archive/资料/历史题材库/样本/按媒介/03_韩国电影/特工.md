---
# === 身份 ===
work_id: WORK-MVK-00316
work_name: 特工
work_name_alt: []
media_type: MVK
year_first: 2018
year_range: [2018, 2018]
season_count: null

# === 三要素(v0 已有,迁移直翻)===
environment: 1990年代朝鲜韩国
cultural_paradigm: 间谍渗透与政治博弈
narrative_core: 潜伏与信仰抉择

# === SDT 心理需求(TBD,等 LLM 阶段补)===
n_main: N-Rel
n_others: [N-Comp]
rel_object: real
t_can_carry: [T4, T5]
core_emotion_note: ""
core_conflict: 朝鲜特工在韩国潜伏，面临信仰与生存的双重抉择

# === 三层标签(主检索路径,见 TAGS.md)===
theme_primary: 政治权谋
theme_secondary: [谍战, 冷战, 国际博弈, 身份认同, 信仰冲突]
visual_tags: [都市夜景, 军装, 政治紧张, 1990年代质感]


# === 评分(v0 已有)===
acquisition_score: 3

# === 素材与玩法 ===
material_hooks: [冷战对峙氛围, 身份伪装细节, 谍战追捕场面, 政治博弈暗流]
existing_games: []
recommended_playstyles: []
risk_notes: [政治敏感, 受众窄]

# === 元数据 ===
source: deepseek-v4-flash + v0-migrate + tags
source_run_id: 2026-05-18-tags-enrich
evidence_level: B
review_status: auto
last_updated: 2026-05-18
---

## 迁移备注

本样本由 v0 markdown 表格迁移而来,LLM 阶段尚未补 SDT 三需求 / T 任务态 / core_conflict / material_hooks 重判。
迁移完成后 evidence_level = C,LLM 增量打标后升至 B。
