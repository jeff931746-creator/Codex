---
# === 身份 ===
work_id: WORK-MVK-00728
work_name: 1987：黎明到来的那一天
work_name_alt: []
media_type: MVK
year_first: 2017
year_range: [2017, 2017]
season_count: null

# === 三要素(v0 已有,迁移直翻)===
environment: 1987年韩国民主运动
cultural_paradigm: 学生抗争+社会纪实
narrative_core: 抗争+牺牲

# === SDT 心理需求(TBD,等 LLM 阶段补)===
n_main: N-Rel
n_others: [N-Comp]
rel_object: real
t_can_carry: [T4, T2]
core_emotion_note: ""
core_conflict: 学生与市民对抗军政府独裁，追求民主与正义

# === 三层标签(主检索路径,见 TAGS.md)===
theme_primary: 历史
theme_secondary: [革命与反抗, 阶级压迫, 政治权谋, 成长]
visual_tags: [街头示威, 催泪弹烟雾, 军警对峙, 标语旗帜, 历史纪实]


# === 评分(v0 已有)===
acquisition_score: 3

# === 素材与玩法 ===
material_hooks: [街头示威人群, 催泪弹烟雾, 学生手举标语, 军警对峙场面]
existing_games: []
recommended_playstyles: []
risk_notes: [政治敏感, 受众窄]

# === 元数据 ===
source: deepseek-v4-flash + v0-migrate + tags
source_run_id: 2026-05-18-tags-enrich
evidence_level: B
review_status: auto
last_updated: 2026-05-18
---

## 迁移备注

本样本由 v0 markdown 表格迁移而来,LLM 阶段尚未补 SDT 三需求 / T 任务态 / core_conflict / material_hooks 重判。
迁移完成后 evidence_level = C,LLM 增量打标后升至 B。
