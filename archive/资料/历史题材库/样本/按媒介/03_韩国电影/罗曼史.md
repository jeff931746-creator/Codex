---
# === 身份 ===
work_id: WORK-MVK-00711
work_name: 罗曼史
work_name_alt: []
media_type: MVK
year_first: 2006
year_range: [2006, 2006]
season_count: null

# === 三要素(v0 已有,迁移直翻)===
environment: 现代韩国都市
cultural_paradigm: 不伦之恋+情感纠葛
narrative_core: 已婚男女不伦恋

# === SDT 心理需求(TBD,等 LLM 阶段补)===
n_main: N-Rel
n_others: []
rel_object: real
t_can_carry: [T3]
core_emotion_note: ""
core_conflict: 已婚男女在道德与情感间挣扎的不伦之恋

# === 三层标签(主检索路径,见 TAGS.md)===
theme_primary: 都市
theme_secondary: [恋爱, 禁忌恋爱, 家庭, 职场]
visual_tags: [都市夜景, 咖啡馆, 公寓, 雨中拥抱, 泪眼对视]


# === 评分(v0 已有)===
acquisition_score: 2

# === 素材与玩法 ===
material_hooks: [雨中拥抱, 泪眼对视, 深夜电话, 街头偶遇]
existing_games: []
recommended_playstyles: []
risk_notes: [题材敏感, 付费浅, 受众窄]

# === 元数据 ===
source: deepseek-v4-flash + v0-migrate + tags
source_run_id: 2026-05-18-tags-enrich
evidence_level: B
review_status: auto
last_updated: 2026-05-18
# v0_duplicate_count: 2
---

## 迁移备注

本样本由 v0 markdown 表格迁移而来,LLM 阶段尚未补 SDT 三需求 / T 任务态 / core_conflict / material_hooks 重判。
迁移完成后 evidence_level = C,LLM 增量打标后升至 B。
