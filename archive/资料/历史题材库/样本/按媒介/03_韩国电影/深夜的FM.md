---
# === 身份 ===
work_id: WORK-MVK-00260
work_name: 深夜的FM
work_name_alt: []
media_type: MVK
year_first: 2010
year_range: [2010, 2010]
season_count: null

# === 三要素(v0 已有,迁移直翻)===
environment: 现代都市广播电台
cultural_paradigm: 劫持人质+心理博弈
narrative_core: 生死直播+解谜逃生

# === SDT 心理需求(TBD,等 LLM 阶段补)===
n_main: N-Comp
n_others: []
rel_object: null
t_can_carry: [T5, T1]
core_emotion_note: ""
core_conflict: 劫匪在深夜电台直播中劫持人质，主角必须通过解谜逃生并揭穿真相

# === 三层标签(主检索路径,见 TAGS.md)===
theme_primary: 悬疑
theme_secondary: [犯罪, 心理悬疑, 都市, 求生]
visual_tags: [都市夜景, 霓虹灯, 电台直播间, 倒计时时钟, 紧张压抑]


# === 评分(v0 已有)===
acquisition_score: 2

# === 素材与玩法 ===
material_hooks: [电台直播间, 劫匪面具, 人质绑绳, 倒计时时钟, 电话连线]
existing_games: []
recommended_playstyles: []
risk_notes: [受众窄, 玩法创新要求高]

# === 元数据 ===
source: deepseek-v4-flash + v0-migrate + tags
source_run_id: 2026-05-18-tags-enrich
evidence_level: B
review_status: auto
last_updated: 2026-05-18
# v0_duplicate_count: 2
---

## 迁移备注

本样本由 v0 markdown 表格迁移而来,LLM 阶段尚未补 SDT 三需求 / T 任务态 / core_conflict / material_hooks 重判。
迁移完成后 evidence_level = C,LLM 增量打标后升至 B。
