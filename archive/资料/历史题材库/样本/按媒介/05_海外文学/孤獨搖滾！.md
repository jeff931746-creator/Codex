---
# === 身份 ===
work_id: WORK-LIT-00460
work_name: 孤獨搖滾！
work_name_alt: []
media_type: LIT
year_first: 2018
year_range: [2018, 2018]
season_count: null

# === 三要素(v0 已有,迁移直翻)===
environment: 现代日本校园
cultural_paradigm: 乐队文化+青春日常
narrative_core: 社恐成长+乐队爆笑

# === SDT 心理需求(TBD,等 LLM 阶段补)===
n_main: N-Rel
n_others: [N-Auto]
rel_object: virtual
t_can_carry: [T3, T1]
core_emotion_note: ""
core_conflict: 社恐少女在乐队中克服恐惧、寻找归属与自我表达

# === 三层标签(主检索路径,见 TAGS.md)===
theme_primary: 校园
theme_secondary: [青春恋爱, 日常喜剧, 音乐, 成长]
visual_tags: [校园, 教室, Live演出场面, 吉他, 搞笑荒诞]


# === 评分(v0 已有)===
acquisition_score: 3

# === 素材与玩法 ===
material_hooks: [社恐爆笑日常, 吉他演奏特写, Live演出场面, 角色夸张表情, 乐队互动温馨]
existing_games: []
recommended_playstyles: []
risk_notes: [付费深度浅, 玩法单一]

# === 元数据 ===
source: deepseek-v4-flash + v0-migrate + tags
source_run_id: 2026-05-18-tags-enrich
evidence_level: B
review_status: auto
last_updated: 2026-05-18
---

## 迁移备注

本样本由 v0 markdown 表格迁移而来,LLM 阶段尚未补 SDT 三需求 / T 任务态 / core_conflict / material_hooks 重判。
迁移完成后 evidence_level = C,LLM 增量打标后升至 B。
