---
# === 身份 ===
work_id: WORK-LIT-00526
work_name: The Paris Library
work_name_alt: []
media_type: LIT
year_first: 2021
year_range: [2021, 2021]
season_count: null

# === 三要素(v0 已有,迁移直翻)===
environment: 二战巴黎
cultural_paradigm: 图书馆员+抵抗运动
narrative_core: 战争抗争+守护知识

# === SDT 心理需求(TBD,等 LLM 阶段补)===
n_main: N-Rel
n_others: [N-Auto]
rel_object: real
t_can_carry: [T3, T4]
core_emotion_note: ""
core_conflict: 图书馆员在纳粹占领下秘密守护书籍与读者，同时面临背叛与生存抉择

# === 三层标签(主检索路径,见 TAGS.md)===
theme_primary: 战争
theme_secondary: [二战, 历史, 求生, 救赎, 成长]
visual_tags: [图书馆, 古堡, 废墟, 军装, 战场残酷]


# === 评分(v0 已有)===
acquisition_score: 3

# === 素材与玩法 ===
material_hooks: [图书馆书架, 地下抵抗传单, 纳粹搜查场景, 秘密借书暗号]
existing_games: []
recommended_playstyles: []
risk_notes: [受众窄, 付费浅]

# === 元数据 ===
source: deepseek-v4-flash + v0-migrate + tags
source_run_id: 2026-05-18-tags-enrich
evidence_level: B
review_status: auto
last_updated: 2026-05-18
---

## 迁移备注

本样本由 v0 markdown 表格迁移而来,LLM 阶段尚未补 SDT 三需求 / T 任务态 / core_conflict / material_hooks 重判。
迁移完成后 evidence_level = C,LLM 增量打标后升至 B。
