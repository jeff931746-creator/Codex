---
# === 身份 ===
work_id: WORK-LIT-00171
work_name: 文豪野犬
work_name_alt: []
media_type: LIT
year_first: 2012
year_range: [2012, 2024]
season_count: null

# === 三要素(v0 已有,迁移直翻)===
environment: 现代日本横滨异能都市
cultural_paradigm: 文豪化身+超能力战斗
narrative_core: 异能组织对抗与成长

# === SDT 心理需求(TBD,等 LLM 阶段补)===
n_main: N-Comp
n_others: [N-Rel]
rel_object: mixed
t_can_carry: [T2, T4]
core_emotion_note: ""
core_conflict: 异能者组织间的对抗与主角在力量与羁绊中寻找自我

# === 三层标签(主检索路径,见 TAGS.md)===
theme_primary: 都市
theme_secondary: [异能都市, 战斗, 成长, 黑帮都市, 超能力]
visual_tags: [都市夜景, 霓虹灯, 异能战斗特效, 黑色西装, 组织徽章与制服]


# === 评分(v0 已有)===
acquisition_score: 4

# === 素材与玩法 ===
material_hooks: [异能战斗特效, 文豪角色设计, 横滨都市夜景, 组织徽章与制服]
existing_games: []
recommended_playstyles: []
risk_notes: [红海, 版权成本高]

# === 元数据 ===
source: deepseek-v4-flash + v0-migrate + tags
source_run_id: 2026-05-18-tags-enrich
evidence_level: B
review_status: auto
last_updated: 2026-05-18
# v0_duplicate_count: 2
---

## 迁移备注

本样本由 v0 markdown 表格迁移而来,LLM 阶段尚未补 SDT 三需求 / T 任务态 / core_conflict / material_hooks 重判。
迁移完成后 evidence_level = C,LLM 增量打标后升至 B。
