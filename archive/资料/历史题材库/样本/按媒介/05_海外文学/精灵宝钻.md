---
# === 身份 ===
work_id: WORK-LIT-00066
work_name: 精灵宝钻
work_name_alt: []
media_type: LIT
year_first: 1977
year_range: [1977, 2002]
season_count: null

# === 三要素(v0 已有,迁移直翻)===
environment: 中土世界创世时期
cultural_paradigm: 精灵神话+魔法体系
narrative_core: 创世史诗+英雄悲歌

# === SDT 心理需求(TBD,等 LLM 阶段补)===
n_main: N-Comp
n_others: []
rel_object: null
t_can_carry: [T2, T5]
core_emotion_note: ""
core_conflict: 精灵族对抗魔苟斯，争夺中土世界的命运与自由

# === 三层标签(主检索路径,见 TAGS.md)===
theme_primary: 神话
theme_secondary: [史诗奇幻, 战争, 精灵]
visual_tags: [仙境, 古堡, 龙焰战场, 精灵, 魔法阵]


# === 评分(v0 已有)===
acquisition_score: 4

# === 素材与玩法 ===
material_hooks: [精灵宝钻光芒, 双圣树光辉, 魔苟斯铁王座, 费艾诺誓言, 龙焰战场]
existing_games: []
recommended_playstyles: []
risk_notes: [版权成本极高, 改编压力大]

# === 元数据 ===
source: deepseek-v4-flash + v0-migrate + tags
source_run_id: 2026-05-18-tags-enrich
evidence_level: B
review_status: auto
last_updated: 2026-05-18
# v0_duplicate_count: 2
---

## 迁移备注

本样本由 v0 markdown 表格迁移而来,LLM 阶段尚未补 SDT 三需求 / T 任务态 / core_conflict / material_hooks 重判。
迁移完成后 evidence_level = C,LLM 增量打标后升至 B。
