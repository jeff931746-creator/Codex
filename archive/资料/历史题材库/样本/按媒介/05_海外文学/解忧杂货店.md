---
# === 身份 ===
work_id: WORK-LIT-00428
work_name: 解忧杂货店
work_name_alt: []
media_type: LIT
year_first: 2012
year_range: [2012, 2012]
season_count: null

# === 三要素(v0 已有,迁移直翻)===
environment: 现代日本/时空穿越
cultural_paradigm: 温情治愈+时间循环
narrative_core: 解忧+命运交织

# === SDT 心理需求(TBD,等 LLM 阶段补)===
n_main: N-Rel
n_others: []
rel_object: virtual
t_can_carry: [T3]
core_emotion_note: ""
core_conflict: 陌生人通过信件跨越时空，彼此救赎与和解

# === 三层标签(主检索路径,见 TAGS.md)===
theme_primary: 都市
theme_secondary: [治愈, 时间轮回, 成长, 救赎, 家庭羁绊]
visual_tags: [老街巷, 深夜灯光, 泛黄信纸, 日常温馨]


# === 评分(v0 已有)===
acquisition_score: 3

# === 素材与玩法 ===
material_hooks: [杂货店信箱, 泛黄信纸, 深夜灯光, 牛奶箱回信]
existing_games: []
recommended_playstyles: []
risk_notes: [付费浅, 改编难度高]

# === 元数据 ===
source: deepseek-v4-flash + v0-migrate + tags
source_run_id: 2026-05-18-tags-enrich
evidence_level: B
review_status: auto
last_updated: 2026-05-18
---

## 迁移备注

本样本由 v0 markdown 表格迁移而来,LLM 阶段尚未补 SDT 三需求 / T 任务态 / core_conflict / material_hooks 重判。
迁移完成后 evidence_level = C,LLM 增量打标后升至 B。
