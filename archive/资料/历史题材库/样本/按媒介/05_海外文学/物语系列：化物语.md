---
# === 身份 ===
work_id: WORK-LIT-00417
work_name: 物语系列：化物语
work_name_alt: []
media_type: LIT
year_first: 2006
year_range: [2006, 2006]
season_count: null

# === 三要素(v0 已有,迁移直翻)===
environment: 现代日本小镇+怪异世界
cultural_paradigm: 怪谈传说+青春治愈
narrative_core: 怪异事件与少女救赎

# === SDT 心理需求(TBD,等 LLM 阶段补)===
n_main: N-Rel
n_others: [N-Comp]
rel_object: virtual
t_can_carry: [T3, T5]
core_emotion_note: ""
core_conflict: 少年与怪异少女们相遇，解决心结与怪异事件

# === 三层标签(主检索路径,见 TAGS.md)===
theme_primary: 悬疑
theme_secondary: [怪谈都市, 青春恋爱, 成长, 超自然悬疑, 校园]
visual_tags: [校园, 都市夜景, 怪异少女造型, 刀光剑影, 日常温馨]


# === 评分(v0 已有)===
acquisition_score: 4

# === 素材与玩法 ===
material_hooks: [怪异少女造型, 战场原黑仪, 阿良良木历, 怪异战斗场面, 校园日常反差]
existing_games: []
recommended_playstyles: []
risk_notes: [受众窄, 付费浅]

# === 元数据 ===
source: deepseek-v4-flash + v0-migrate + tags
source_run_id: 2026-05-18-tags-enrich
evidence_level: B
review_status: auto
last_updated: 2026-05-18
---

## 迁移备注

本样本由 v0 markdown 表格迁移而来,LLM 阶段尚未补 SDT 三需求 / T 任务态 / core_conflict / material_hooks 重判。
迁移完成后 evidence_level = C,LLM 增量打标后升至 B。
