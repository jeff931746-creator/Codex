---
# === 身份 ===
work_id: WORK-LIT-00423
work_name: 火车上的女孩
work_name_alt: []
media_type: LIT
year_first: 2015
year_range: [2015, 2016]
season_count: null

# === 三要素(v0 已有,迁移直翻)===
environment: 现代伦敦郊区
cultural_paradigm: 悬疑惊悚+心理推理
narrative_core: 通勤女卷入失踪案

# === SDT 心理需求(TBD,等 LLM 阶段补)===
n_main: N-Comp
n_others: []
rel_object: null
t_can_carry: [T5]
core_emotion_note: ""
core_conflict: 通勤女试图破解失踪案真相，却陷入记忆与谎言的迷宫

# === 三层标签(主检索路径,见 TAGS.md)===
theme_primary: 悬疑
theme_secondary: [心理悬疑, 犯罪, 都市, 失踪]
visual_tags: [都市夜景, 火车窗景, 公寓, 悬疑诡异]


# === 评分(v0 已有)===
acquisition_score: 3

# === 素材与玩法 ===
material_hooks: [火车窗景, 失踪女人照片, 酒精迷离, 真相反转]
existing_games: []
recommended_playstyles: []
risk_notes: [受众窄, 付费浅]

# === 元数据 ===
source: deepseek-v4-flash + v0-migrate + tags
source_run_id: 2026-05-18-tags-enrich
evidence_level: B
review_status: auto
last_updated: 2026-05-18
# v0_duplicate_count: 2
---

## 迁移备注

本样本由 v0 markdown 表格迁移而来,LLM 阶段尚未补 SDT 三需求 / T 任务态 / core_conflict / material_hooks 重判。
迁移完成后 evidence_level = C,LLM 增量打标后升至 B。
