---
# === 身份 ===
work_id: WORK-LIT-00475
work_name: 偷书贼
work_name_alt: []
media_type: LIT
year_first: 2005
year_range: [2005, 2014]
season_count: null

# === 三要素(v0 已有,迁移直翻)===
environment: 二战德国小镇
cultural_paradigm: 文字与生存信念
narrative_core: 战火中成长与救赎

# === SDT 心理需求(TBD,等 LLM 阶段补)===
n_main: N-Rel
n_others: [N-Auto]
rel_object: mixed
t_can_carry: [T3, T1]
core_emotion_note: N-Rel温和归属为主，N-Auto提供自由阅读与精神避难所
core_conflict: 女孩在纳粹阴影下用文字偷取温暖与人性

# === 三层标签(主检索路径,见 TAGS.md)===
theme_primary: 历史
theme_secondary: [二战, 成长, 救赎, 家庭羁绊]
visual_tags: [雪原, 废墟, 古战场, 小镇, 书信]


# === 评分(v0 已有)===
acquisition_score: 3

# === 素材与玩法 ===
material_hooks: [偷书场景, 地下室朗读, 手写信件, 雪中送面包, 死神旁白]
existing_games: []
recommended_playstyles: []
risk_notes: [受众窄, 付费浅]

# === 元数据 ===
source: deepseek-v4-flash + v0-migrate + tags
source_run_id: 2026-05-18-tags-enrich
evidence_level: B
review_status: auto
last_updated: 2026-05-18
# v0_duplicate_count: 3
---

## 迁移备注

本样本由 v0 markdown 表格迁移而来,LLM 阶段尚未补 SDT 三需求 / T 任务态 / core_conflict / material_hooks 重判。
迁移完成后 evidence_level = C,LLM 增量打标后升至 B。
