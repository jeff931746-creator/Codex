---
# === 身份 ===
work_id: WORK-LIT-00528
work_name: The Other Black Girl
work_name_alt: []
media_type: LIT
year_first: 2021
year_range: [2021, 2021]
season_count: null

# === 三要素(v0 已有,迁移直翻)===
environment: 现代纽约出版界
cultural_paradigm: 职场种族+心理惊悚
narrative_core: 职场悬疑+身份博弈

# === SDT 心理需求(TBD,等 LLM 阶段补)===
n_main: N-Rel
n_others: [N-Comp]
rel_object: real
t_can_carry: [T4, T5]
core_emotion_note: ""
core_conflict: 黑人女编辑在出版界职场中面临种族身份与权力博弈的悬疑斗争

# === 三层标签(主检索路径,见 TAGS.md)===
theme_primary: 职场
theme_secondary: [悬疑, 都市, 犯罪, 身份认同, 阶级压迫]
visual_tags: [都市夜景, 办公室, 摩天楼, 悬疑诡异, 黑色西装]


# === 评分(v0 已有)===
acquisition_score: 2

# === 素材与玩法 ===
material_hooks: [办公室对峙, 神秘信件, 电梯压迫, 身份反转]
existing_games: []
recommended_playstyles: []
risk_notes: [受众窄, 文化敏感]

# === 元数据 ===
source: deepseek-v4-flash + v0-migrate + tags
source_run_id: 2026-05-18-tags-enrich
evidence_level: B
review_status: auto
last_updated: 2026-05-18
---

## 迁移备注

本样本由 v0 markdown 表格迁移而来,LLM 阶段尚未补 SDT 三需求 / T 任务态 / core_conflict / material_hooks 重判。
迁移完成后 evidence_level = C,LLM 增量打标后升至 B。
