---
# === 身份 ===
work_id: WORK-LIT-00033
work_name: 冰与火之歌4：群鸦的盛宴
work_name_alt: []
media_type: LIT
year_first: 2005
year_range: [2005, 2005]
season_count: null

# === 三要素(v0 已有,迁移直翻)===
environment: 架空奇幻大陆/战后乱世
cultural_paradigm: 骑士魔法+暗流权谋
narrative_core: 战后各方势力暗流涌动

# === SDT 心理需求(TBD,等 LLM 阶段补)===
n_main: N-Rel
n_others: [N-Comp]
rel_object: real
t_can_carry: [T4, T5]
core_emotion_note: ""
core_conflict: 七大家族在战后权力真空期争夺铁王座，长城外异鬼威胁暗涌

# === 三层标签(主检索路径,见 TAGS.md)===
theme_primary: 政治权谋
theme_secondary: [王朝兴衰, 权力游戏, 战争, 奇幻, 悬疑]
visual_tags: [城堡, 宫殿, 古战场, 中世纪战争, 权谋阴谋]


# === 评分(v0 已有)===
acquisition_score: 5

# === 素材与玩法 ===
material_hooks: [铁王座争夺, 血色婚礼阴影, 龙母崛起, 君临城暗杀, 北境长城危机]
existing_games: [权力的游戏手游]
recommended_playstyles: []
risk_notes: [版权成本极高, 红海]

# === 元数据 ===
source: deepseek-v4-flash + v0-migrate + tags
source_run_id: 2026-05-18-tags-enrich
evidence_level: B
review_status: auto
last_updated: 2026-05-18
---

## 迁移备注

本样本由 v0 markdown 表格迁移而来,LLM 阶段尚未补 SDT 三需求 / T 任务态 / core_conflict / material_hooks 重判。
迁移完成后 evidence_level = C,LLM 增量打标后升至 B。
