---
# === 身份 ===
work_id: WORK-LIT-00001
work_name: 哈利·波特与火焰杯
work_name_alt: []
media_type: LIT
year_first: 2000
year_range: [2000, 2000]
season_count: null

# === 三要素(v0 已有,迁移直翻)===
environment: 架空魔法世界
cultural_paradigm: 魔法学院+三强争霸赛
narrative_core: 成长冒险+竞技挑战

# === SDT 心理需求(TBD,等 LLM 阶段补)===
n_main: N-Comp
n_others: [N-Rel]
rel_object: mixed
t_can_carry: [T2, T4]
core_emotion_note: ""
core_conflict: 哈利在三强争霸赛中对抗伏地魔的阴谋与黑魔法威胁

# === 三层标签(主检索路径,见 TAGS.md)===
theme_primary: 奇幻
theme_secondary: [魔法, 冒险, 校园, 成长, 竞技]
visual_tags: [城堡, 魔法阵, 龙, 迷宫, 校园青春]


# === 评分(v0 已有)===
acquisition_score: 5

# === 素材与玩法 ===
material_hooks: [魁地奇世界杯, 三强争霸赛任务, 火龙与迷宫, 魔法特效对决]
existing_games: [哈利波特系列游戏]
recommended_playstyles: []
risk_notes: [版权成本极高, 红海]

# === 元数据 ===
source: deepseek-v4-flash + v0-migrate + tags
source_run_id: 2026-05-18-tags-enrich
evidence_level: B
review_status: auto
last_updated: 2026-05-18
# v0_duplicate_count: 2
---

## 迁移备注

本样本由 v0 markdown 表格迁移而来,LLM 阶段尚未补 SDT 三需求 / T 任务态 / core_conflict / material_hooks 重判。
迁移完成后 evidence_level = C,LLM 增量打标后升至 B。
