---
# === 身份 ===
work_id: WORK-LIT-00536
work_name: The Paper Palace
work_name_alt: []
media_type: LIT
year_first: 2021
year_range: [2021, 2021]
season_count: null

# === 三要素(v0 已有,迁移直翻)===
environment: 现代夏日度假屋
cultural_paradigm: 家庭伦理+婚姻困境
narrative_core: 情感抉择与回忆交织

# === SDT 心理需求(TBD,等 LLM 阶段补)===
n_main: N-Rel
n_others: []
rel_object: real
t_can_carry: [T3]
core_emotion_note: ""
core_conflict: 女主在婚姻与旧爱之间挣扎，家庭秘密与自我真相交织

# === 三层标签(主检索路径,见 TAGS.md)===
theme_primary: 家庭
theme_secondary: [代际冲突, 家庭秘密, 成长]
visual_tags: [夏日度假屋, 湖畔, 家庭聚餐, 田园清新]


# === 评分(v0 已有)===
acquisition_score: 2

# === 素材与玩法 ===
material_hooks: [夏日度假屋, 家庭聚餐场景, 湖畔回忆, 秘密信件]
existing_games: []
recommended_playstyles: []
risk_notes: [受众窄, 付费浅]

# === 元数据 ===
source: deepseek-v4-flash + v0-migrate + tags
source_run_id: 2026-05-18-tags-enrich
evidence_level: B
review_status: auto
last_updated: 2026-05-18
---

## 迁移备注

本样本由 v0 markdown 表格迁移而来,LLM 阶段尚未补 SDT 三需求 / T 任务态 / core_conflict / material_hooks 重判。
迁移完成后 evidence_level = C,LLM 增量打标后升至 B。
