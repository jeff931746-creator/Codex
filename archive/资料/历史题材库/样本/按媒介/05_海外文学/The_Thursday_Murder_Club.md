---
# === 身份 ===
work_id: WORK-LIT-00531
work_name: The Thursday Murder Club
work_name_alt: []
media_type: LIT
year_first: 2020
year_range: [2020, 2021]
season_count: null

# === 三要素(v0 已有,迁移直翻)===
environment: 现代英国养老院
cultural_paradigm: 老年侦探团+日常推理
narrative_core: 轻松破案与友情羁绊

# === SDT 心理需求(TBD,等 LLM 阶段补)===
n_main: N-Rel
n_others: [N-Auto]
rel_object: mixed
t_can_carry: [T3, T5]
core_emotion_note: ""
core_conflict: 养老院居民用智慧破解谋杀案，同时维系友情与生活节奏

# === 三层标签(主检索路径,见 TAGS.md)===
theme_primary: 悬疑
theme_secondary: [推理, 友情, 都市, 喜剧]
visual_tags: [田园清新, 养老院, 咖啡馆, 老街巷, 日常温馨]


# === 评分(v0 已有)===
acquisition_score: 3

# === 素材与玩法 ===
material_hooks: [老人侦探团, 英式田园养老院, 茶话会推理, 谋杀案线索拼图]
existing_games: []
recommended_playstyles: []
risk_notes: [受众窄, 付费浅]

# === 元数据 ===
source: deepseek-v4-flash + v0-migrate + tags
source_run_id: 2026-05-18-tags-enrich
evidence_level: B
review_status: auto
last_updated: 2026-05-18
# v0_duplicate_count: 2
---

## 迁移备注

本样本由 v0 markdown 表格迁移而来,LLM 阶段尚未补 SDT 三需求 / T 任务态 / core_conflict / material_hooks 重判。
迁移完成后 evidence_level = C,LLM 增量打标后升至 B。
