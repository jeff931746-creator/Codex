---
# === 身份 ===
work_id: WORK-LIT-00148
work_name: 银河帝国
work_name_alt: []
media_type: LIT
year_first: 1951
year_range: [1951, 2013]
season_count: null

# === 三要素(v0 已有,迁移直翻)===
environment: 银河系未来星际
cultural_paradigm: 心理史学+科技帝国
narrative_core: 文明兴衰+权谋博弈

# === SDT 心理需求(TBD,等 LLM 阶段补)===
n_main: N-Rel
n_others: [N-Comp]
rel_object: real
t_can_carry: [T4, T5]
core_emotion_note: ""
core_conflict: 谢顿计划对抗帝国衰亡，各势力争夺银河霸权

# === 三层标签(主检索路径,见 TAGS.md)===
theme_primary: 科幻
theme_secondary: [太空, 政治权谋, 历史, 权力博弈]
visual_tags: [星舰内部, 空间站, 行星表面, 未来科技质感, 银河帝国]


# === 评分(v0 已有)===
acquisition_score: 4

# === 素材与玩法 ===
material_hooks: [银河帝国星舰, 川陀穹顶城, 心理史学公式, 基地孤城, 骡的突变]
existing_games: []
recommended_playstyles: []
risk_notes: [受众偏硬核, 版权成本高]

# === 元数据 ===
source: deepseek-v4-flash + v0-migrate + tags
source_run_id: 2026-05-18-tags-enrich
evidence_level: B
review_status: auto
last_updated: 2026-05-18
# v0_duplicate_count: 2
---

## 迁移备注

本样本由 v0 markdown 表格迁移而来,LLM 阶段尚未补 SDT 三需求 / T 任务态 / core_conflict / material_hooks 重判。
迁移完成后 evidence_level = C,LLM 增量打标后升至 B。
