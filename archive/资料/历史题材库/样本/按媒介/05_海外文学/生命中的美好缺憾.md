---
# === 身份 ===
work_id: WORK-LIT-00410
work_name: 生命中的美好缺憾
work_name_alt: []
media_type: LIT
year_first: 2012
year_range: [2012, 2014]
season_count: null

# === 三要素(v0 已有,迁移直翻)===
environment: 现代都市/医院
cultural_paradigm: 青春爱情+疾病抗争
narrative_core: 绝症恋爱+生命感悟

# === SDT 心理需求(TBD,等 LLM 阶段补)===
n_main: N-Rel
n_others: []
rel_object: virtual
t_can_carry: [T3]
core_emotion_note: ""
core_conflict: 绝症少年少女在有限生命中相爱与告别

# === 三层标签(主检索路径,见 TAGS.md)===
theme_primary: 恋爱
theme_secondary: [青春恋爱, 成长, 治愈, 悲剧]
visual_tags: [医院, 都市夜景, 阳光治愈, 恋爱甜蜜]


# === 评分(v0 已有)===
acquisition_score: 3

# === 素材与玩法 ===
material_hooks: [医院病床对话, 氧气面罩特写, 阿姆斯特丹旅行, 葬礼独白]
existing_games: []
recommended_playstyles: []
risk_notes: [受众窄, 付费深度低]

# === 元数据 ===
source: deepseek-v4-flash + v0-migrate + tags
source_run_id: 2026-05-18-tags-enrich
evidence_level: B
review_status: auto
last_updated: 2026-05-18
# v0_duplicate_count: 4
---

## 迁移备注

本样本由 v0 markdown 表格迁移而来,LLM 阶段尚未补 SDT 三需求 / T 任务态 / core_conflict / material_hooks 重判。
迁移完成后 evidence_level = C,LLM 增量打标后升至 B。
