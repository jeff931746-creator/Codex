---
# === 身份 ===
work_id: WORK-LIT-00515
work_name: 永恒的边缘
work_name_alt: []
media_type: LIT
year_first: 2014
year_range: [2014, 2014]
season_count: null

# === 三要素(v0 已有,迁移直翻)===
environment: 冷战时期全球变迁
cultural_paradigm: 历史写实+社会变革
narrative_core: 冷战背景下家族命运

# === SDT 心理需求(TBD,等 LLM 阶段补)===
n_main: N-Rel
n_others: []
rel_object: real
t_can_carry: [T4]
core_emotion_note: ""
core_conflict: 冷战铁幕下家族命运与意识形态对抗

# === 三层标签(主检索路径,见 TAGS.md)===
theme_primary: 历史
theme_secondary: [冷战, 战争, 政治权谋, 家庭]
visual_tags: [柏林墙, 街头抗议, 核弹阴影, 冷战氛围]


# === 评分(v0 已有)===
acquisition_score: 3

# === 素材与玩法 ===
material_hooks: [柏林墙对峙, 核弹阴影, 街头抗议, 谍战暗涌]
existing_games: []
recommended_playstyles: []
risk_notes: [受众窄, 玩法难创新]

# === 元数据 ===
source: deepseek-v4-flash + v0-migrate + tags
source_run_id: 2026-05-18-tags-enrich
evidence_level: B
review_status: auto
last_updated: 2026-05-18
---

## 迁移备注

本样本由 v0 markdown 表格迁移而来,LLM 阶段尚未补 SDT 三需求 / T 任务态 / core_conflict / material_hooks 重判。
迁移完成后 evidence_level = C,LLM 增量打标后升至 B。
