---
# === 身份 ===
work_id: WORK-LIT-00502
work_name: 然后呢？
work_name_alt: []
media_type: LIT
year_first: 2007
year_range: [2007, 2007]
season_count: null

# === 三要素(v0 已有,迁移直翻)===
environment: 现代都市/悬疑小镇
cultural_paradigm: 心理悬疑+失踪谜案
narrative_core: 悬疑解谜/真相追寻

# === SDT 心理需求(TBD,等 LLM 阶段补)===
n_main: N-Comp
n_others: []
rel_object: null
t_can_carry: [T5]
core_emotion_note: ""
core_conflict: 主角追寻失踪真相，在时间流逝中直面人性谜题

# === 三层标签(主检索路径,见 TAGS.md)===
theme_primary: 悬疑
theme_secondary: [失踪, 身份谜团, 心理悬疑, 时间轮回]
visual_tags: [雨夜追踪, 旧照片线索, 审讯室灯光, 倒计时时钟]


# === 评分(v0 已有)===
acquisition_score: 2

# === 素材与玩法 ===
material_hooks: [失踪现场, 倒计时时钟, 审讯室灯光, 旧照片线索, 雨夜追踪]
existing_games: []
recommended_playstyles: []
risk_notes: [受众窄, 题材沉重]

# === 元数据 ===
source: deepseek-v4-flash + v0-migrate + tags
source_run_id: 2026-05-18-tags-enrich
evidence_level: B
review_status: auto
last_updated: 2026-05-18
---

## 迁移备注

本样本由 v0 markdown 表格迁移而来,LLM 阶段尚未补 SDT 三需求 / T 任务态 / core_conflict / material_hooks 重判。
迁移完成后 evidence_level = C,LLM 增量打标后升至 B。
