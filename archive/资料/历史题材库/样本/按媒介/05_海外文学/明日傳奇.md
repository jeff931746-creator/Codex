---
# === 身份 ===
work_id: WORK-LIT-00544
work_name: 明日傳奇
work_name_alt: []
media_type: LIT
year_first: 2022
year_range: [2022, 2022]
season_count: null

# === 三要素(v0 已有,迁移直翻)===
environment: 现代都市+游戏行业
cultural_paradigm: 游戏开发+现实生活
narrative_core: 友谊羁绊+梦想传承

# === SDT 心理需求(TBD,等 LLM 阶段补)===
n_main: N-Rel
n_others: [N-Auto]
rel_object: mixed
t_can_carry: [T3, T1]
core_emotion_note: ""
core_conflict: 游戏开发者们在现实与梦想间维系友谊，传承创作信念

# === 三层标签(主检索路径,见 TAGS.md)===
theme_primary: 都市
theme_secondary: [职场, 友情, 成长, 励志]
visual_tags: [都市夜景, 办公室, 像素游戏画面, 游戏发布会]


# === 评分(v0 已有)===
acquisition_score: 2

# === 素材与玩法 ===
material_hooks: [团队熬夜开发, 像素游戏画面, 老办公室场景, 游戏发布会瞬间]
existing_games: []
recommended_playstyles: []
risk_notes: [受众极窄, 付费浅]

# === 元数据 ===
source: deepseek-v4-flash + v0-migrate + tags
source_run_id: 2026-05-18-tags-enrich
evidence_level: B
review_status: auto
last_updated: 2026-05-18
---

## 迁移备注

本样本由 v0 markdown 表格迁移而来,LLM 阶段尚未补 SDT 三需求 / T 任务态 / core_conflict / material_hooks 重判。
迁移完成后 evidence_level = C,LLM 增量打标后升至 B。
