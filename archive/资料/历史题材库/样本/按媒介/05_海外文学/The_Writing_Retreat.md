---
# === 身份 ===
work_id: WORK-LIT-00452
work_name: The Writing Retreat
work_name_alt: []
media_type: LIT
year_first: 2023
year_range: [2023, 2023]
season_count: null

# === 三要素(v0 已有,迁移直翻)===
environment: 现代封闭庄园
cultural_paradigm: 写作竞赛+生存博弈
narrative_core: 密室求生+人性博弈

# === SDT 心理需求(TBD,等 LLM 阶段补)===
n_main: N-Rel
n_others: [N-Comp]
rel_object: real
t_can_carry: [T4, T5]
core_emotion_note: ""
core_conflict: 作家们在封闭庄园中为生存和出版机会展开残酷博弈

# === 三层标签(主检索路径,见 TAGS.md)===
theme_primary: 悬疑
theme_secondary: [心理悬疑, 生存恐怖]
visual_tags: [古堡, 废墟, 阴森走廊, 打字机血迹, 密室门锁]


# === 评分(v0 已有)===
acquisition_score: 2

# === 素材与玩法 ===
material_hooks: [庄园阴森走廊, 打字机血迹, 密室门锁特写, 角色对峙剪影, 手稿消失反转]
existing_games: []
recommended_playstyles: []
risk_notes: [受众窄, 题材敏感]

# === 元数据 ===
source: deepseek-v4-flash + v0-migrate + tags
source_run_id: 2026-05-18-tags-enrich
evidence_level: B
review_status: auto
last_updated: 2026-05-18
---

## 迁移备注

本样本由 v0 markdown 表格迁移而来,LLM 阶段尚未补 SDT 三需求 / T 任务态 / core_conflict / material_hooks 重判。
迁移完成后 evidence_level = C,LLM 增量打标后升至 B。
