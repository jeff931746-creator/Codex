---
# === 身份 ===
work_id: WORK-LIT-00560
work_name: 怦然心动
work_name_alt: []
media_type: LIT
year_first: 2001
year_range: [2001, 2001]
season_count: null

# === 三要素(v0 已有,迁移直翻)===
environment: 现代美国小镇
cultural_paradigm: 青春校园+纯爱
narrative_core: 双向暗恋与成长

# === SDT 心理需求(TBD,等 LLM 阶段补)===
n_main: N-Rel
n_others: []
rel_object: virtual
t_can_carry: [T3]
core_emotion_note: ""
core_conflict: 男孩与女孩从误解到暗恋，在成长中彼此靠近

# === 三层标签(主检索路径,见 TAGS.md)===
theme_primary: 校园
theme_secondary: [青春恋爱, 成长, 日常喜剧]
visual_tags: [校园青春, 阳光治愈, 小镇生活, 教室]


# === 评分(v0 已有)===
acquisition_score: 3

# === 素材与玩法 ===
material_hooks: [梧桐树上的风景, 少年蓝眼睛, 午餐桌对视, 庭院里的对话]
existing_games: []
recommended_playstyles: []
risk_notes: [受众窄, 付费困难]

# === 元数据 ===
source: deepseek-v4-flash + v0-migrate + tags
source_run_id: 2026-05-18-tags-enrich
evidence_level: B
review_status: auto
last_updated: 2026-05-18
---

## 迁移备注

本样本由 v0 markdown 表格迁移而来,LLM 阶段尚未补 SDT 三需求 / T 任务态 / core_conflict / material_hooks 重判。
迁移完成后 evidence_level = C,LLM 增量打标后升至 B。
