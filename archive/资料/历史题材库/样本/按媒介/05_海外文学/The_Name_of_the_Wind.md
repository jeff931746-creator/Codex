---
# === 身份 ===
work_id: WORK-LIT-00078
work_name: The Name of the Wind
work_name_alt: []
media_type: LIT
year_first: 2007
year_range: [2007, 2007]
season_count: null

# === 三要素(v0 已有,迁移直翻)===
environment: 架空奇幻世界
cultural_paradigm: 魔法学院体系+吟游诗人
narrative_core: 天才魔法师成长自述

# === SDT 心理需求(TBD,等 LLM 阶段补)===
n_main: N-Comp
n_others: []
rel_object: null
t_can_carry: [T2, T5]
core_emotion_note: ""
core_conflict: 天才少年在魔法学院与乱世中追寻真相与力量

# === 三层标签(主检索路径,见 TAGS.md)===
theme_primary: 奇幻
theme_secondary: [魔法, 校园, 成长, 史诗奇幻]
visual_tags: [魔法学院, 古老图书馆, 神秘符文, 吟游诗人, 古风装扮]


# === 评分(v0 已有)===
acquisition_score: 4

# === 素材与玩法 ===
material_hooks: [魔法学院场景, 吟游诗人弹唱, 神秘符文闪烁, 战斗法术特效, 古老图书馆]
existing_games: []
recommended_playstyles: []
risk_notes: [版权成本高, 开发难度大]

# === 元数据 ===
source: deepseek-v4-flash + v0-migrate + tags
source_run_id: 2026-05-18-tags-enrich
evidence_level: B
review_status: auto
last_updated: 2026-05-18
---

## 迁移备注

本样本由 v0 markdown 表格迁移而来,LLM 阶段尚未补 SDT 三需求 / T 任务态 / core_conflict / material_hooks 重判。
迁移完成后 evidence_level = C,LLM 增量打标后升至 B。
