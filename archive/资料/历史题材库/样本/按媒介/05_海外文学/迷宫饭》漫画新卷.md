---
# === 身份 ===
work_id: WORK-LIT-00355
work_name: 迷宫饭》漫画新卷
work_name_alt: []
media_type: LIT
year_first: 2024
year_range: [2024, 2024]
season_count: null

# === 三要素(v0 已有,迁移直翻)===
environment: 奇幻地下迷宫
cultural_paradigm: 魔物烹饪+生存体系
narrative_core: 烹饪魔物食材求生

# === SDT 心理需求(TBD,等 LLM 阶段补)===
n_main: N-Auto
n_others: [N-Rel]
rel_object: virtual
t_can_carry: [T1, T3]
core_emotion_note: ""
core_conflict: 被困迷宫，靠烹饪魔物食材求生并寻找出口

# === 三层标签(主检索路径,见 TAGS.md)===
theme_primary: 冒险
theme_secondary: [求生, 奇幻, 美食, 小队求生]
visual_tags: [地下迷宫, 魔物, 烹饪特写, 奇幻生态]


# === 评分(v0 已有)===
acquisition_score: 4

# === 素材与玩法 ===
material_hooks: [魔物料理特写, 迷宫生态奇观, 角色烹饪互动, 食材变身过程]
existing_games: []
recommended_playstyles: []
risk_notes: [受众偏窄, 玩法创新难]

# === 元数据 ===
source: deepseek-v4-flash + v0-migrate + tags
source_run_id: 2026-05-18-tags-enrich
evidence_level: B
review_status: auto
last_updated: 2026-05-18
---

## 迁移备注

本样本由 v0 markdown 表格迁移而来,LLM 阶段尚未补 SDT 三需求 / T 任务态 / core_conflict / material_hooks 重判。
迁移完成后 evidence_level = C,LLM 增量打标后升至 B。
