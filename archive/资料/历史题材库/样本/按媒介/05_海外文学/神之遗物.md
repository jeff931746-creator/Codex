---
# === 身份 ===
work_id: WORK-LIT-00291
work_name: 神之遗物
work_name_alt: []
media_type: LIT
year_first: 2024
year_range: [2024, 2024]
season_count: null

# === 三要素(v0 已有,迁移直翻)===
environment: 全球现代都市+秘境
cultural_paradigm: 考古探险+超自然力量
narrative_core: 寻宝冒险+文明揭秘

# === SDT 心理需求(TBD,等 LLM 阶段补)===
n_main: N-Comp
n_others: []
rel_object: null
t_can_carry: [T2, T5]
core_emotion_note: ""
core_conflict: 主角在秘境中寻找神之遗物，揭开文明真相并对抗超自然威胁

# === 三层标签(主检索路径,见 TAGS.md)===
theme_primary: 冒险
theme_secondary: [寻宝, 超自然悬疑]
visual_tags: [秘境遗迹, 古文明符号, 超自然神器, 考古挖掘]


# === 评分(v0 已有)===
acquisition_score: 3

# === 素材与玩法 ===
material_hooks: [秘境遗迹, 超自然神器, 考古挖掘, 文明符号]
existing_games: []
recommended_playstyles: []
risk_notes: [红海竞争]

# === 元数据 ===
source: deepseek-v4-flash + v0-migrate + tags
source_run_id: 2026-05-18-tags-enrich
evidence_level: B
review_status: auto
last_updated: 2026-05-18
---

## 迁移备注

本样本由 v0 markdown 表格迁移而来,LLM 阶段尚未补 SDT 三需求 / T 任务态 / core_conflict / material_hooks 重判。
迁移完成后 evidence_level = C,LLM 增量打标后升至 B。
