---
# === 身份 ===
work_id: WORK-LIT-00552
work_name: 海边的卡夫卡
work_name_alt: []
media_type: LIT
year_first: 2002
year_range: [2002, 2002]
season_count: null

# === 三要素(v0 已有,迁移直翻)===
environment: 现代日本/超现实
cultural_paradigm: 魔幻现实主义+哲学
narrative_core: 少年成长与命运探索

# === SDT 心理需求(TBD,等 LLM 阶段补)===
n_main: N-Auto
n_others: []
rel_object: null
t_can_carry: [T5]
core_emotion_note: ""
core_conflict: 少年在超现实世界中寻找自我与命运真相

# === 三层标签(主检索路径,见 TAGS.md)===
theme_primary: 冒险
theme_secondary: [成长, 寻找自我, 超自然悬疑, 奇幻]
visual_tags: [森林, 图书馆, 废墟, 古风诗意, 超现实]


# === 评分(v0 已有)===
acquisition_score: 3

# === 素材与玩法 ===
material_hooks: [猫之对话, 森林小屋, 暴风雨夜, 图书馆密室, 入口石]
existing_games: []
recommended_playstyles: []
risk_notes: [受众极窄, 无商业潜力]

# === 元数据 ===
source: deepseek-v4-flash + v0-migrate + tags
source_run_id: 2026-05-18-tags-enrich
evidence_level: B
review_status: auto
last_updated: 2026-05-18
# v0_duplicate_count: 2
---

## 迁移备注

本样本由 v0 markdown 表格迁移而来,LLM 阶段尚未补 SDT 三需求 / T 任务态 / core_conflict / material_hooks 重判。
迁移完成后 evidence_level = C,LLM 增量打标后升至 B。
