---
# === 身份 ===
work_id: WORK-LIT-00439
work_name: The Love Hypothesis
work_name_alt: []
media_type: LIT
year_first: 2021
year_range: [2021, 2021]
season_count: null

# === 三要素(v0 已有,迁移直翻)===
environment: 现代美国大学实验室
cultural_paradigm: 科研学术+假恋爱契约
narrative_core: 假戏真做与科学理性

# === SDT 心理需求(TBD,等 LLM 阶段补)===
n_main: N-Rel
n_others: []
rel_object: virtual
t_can_carry: [T3]
core_emotion_note: ""
core_conflict: 假恋爱契约下理性与情感冲突，两人从伪装到真心

# === 三层标签(主检索路径,见 TAGS.md)===
theme_primary: 恋爱
theme_secondary: [职场, 都市, 校园, 喜剧]
visual_tags: [实验室, 白大褂, 咖啡馆, 校园, 都市夜景]


# === 评分(v0 已有)===
acquisition_score: 3

# === 素材与玩法 ===
material_hooks: [实验室接吻, 白大褂对视, 咖啡杯传情, 论文旁牵手]
existing_games: []
recommended_playstyles: []
risk_notes: [付费浅, 题材同质化]

# === 元数据 ===
source: deepseek-v4-flash + v0-migrate + tags
source_run_id: 2026-05-18-tags-enrich
evidence_level: B
review_status: auto
last_updated: 2026-05-18
# v0_duplicate_count: 2
---

## 迁移备注

本样本由 v0 markdown 表格迁移而来,LLM 阶段尚未补 SDT 三需求 / T 任务态 / core_conflict / material_hooks 重判。
迁移完成后 evidence_level = C,LLM 增量打标后升至 B。
