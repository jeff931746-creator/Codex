---
# === 身份 ===
work_id: WORK-LIT-00369
work_name: 飓风之舞
work_name_alt: []
media_type: LIT
year_first: 2025
year_range: [2025, 2025]
season_count: null

# === 三要素(v0 已有,迁移直翻)===
environment: 架空魔法世界
cultural_paradigm: 风暴魔法+史诗冒险
narrative_core: 英雄成长+世界危机

# === SDT 心理需求(TBD,等 LLM 阶段补)===
n_main: N-Comp
n_others: []
rel_object: null
t_can_carry: [T2]
core_emotion_note: ""
core_conflict: 少年驾驭风暴魔法对抗灭世飓风，在成长中揭开世界真相

# === 三层标签(主检索路径,见 TAGS.md)===
theme_primary: 冒险
theme_secondary: [奇幻, 成长, 少年成长, 魔法]
visual_tags: [风暴魔法特效, 巨龙翱翔, 天空城遗迹, 飓风眼战场]


# === 评分(v0 已有)===
acquisition_score: 3

# === 素材与玩法 ===
material_hooks: [风暴魔法特效, 巨龙翱翔, 天空城遗迹, 飓风眼战场]
existing_games: []
recommended_playstyles: []
risk_notes: [新IP风险高, 红海竞争]

# === 元数据 ===
source: deepseek-v4-flash + v0-migrate + tags
source_run_id: 2026-05-18-tags-enrich
evidence_level: B
review_status: auto
last_updated: 2026-05-18
# v0_duplicate_count: 2
---

## 迁移备注

本样本由 v0 markdown 表格迁移而来,LLM 阶段尚未补 SDT 三需求 / T 任务态 / core_conflict / material_hooks 重判。
迁移完成后 evidence_level = C,LLM 增量打标后升至 B。
