---
# === 身份 ===
work_id: WORK-LIT-00360
work_name: 红皇后
work_name_alt: []
media_type: LIT
year_first: 2015
year_range: [2015, 2015]
season_count: null

# === 三要素(v0 已有,迁移直翻)===
environment: 近未来阶级分裂世界
cultural_paradigm: 超能力血液等级制度
narrative_core: 底层少女挑战权贵

# === SDT 心理需求(TBD,等 LLM 阶段补)===
n_main: N-Comp
n_others: [N-Rel]
rel_object: real
t_can_carry: [T2, T4]
core_emotion_note: ""
core_conflict: 底层少女用特殊血液能力挑战阶级固化的权贵世界

# === 三层标签(主检索路径,见 TAGS.md)===
theme_primary: 科幻
theme_secondary: [阶级压迫, 异能觉醒, 成长, 逆袭, 战斗]
visual_tags: [霓虹灯, 贫民窟, 宫殿, 全息投影, 血液特效]


# === 评分(v0 已有)===
acquisition_score: 3

# === 素材与玩法 ===
material_hooks: [血液能力特效, 红皇后王冠, 阶级对立场景, 逆袭战斗]
existing_games: []
recommended_playstyles: []
risk_notes: [受众偏女性, 版权成本中等]

# === 元数据 ===
source: deepseek-v4-flash + v0-migrate + tags
source_run_id: 2026-05-18-tags-enrich
evidence_level: B
review_status: auto
last_updated: 2026-05-18
# v0_duplicate_count: 2
---

## 迁移备注

本样本由 v0 markdown 表格迁移而来,LLM 阶段尚未补 SDT 三需求 / T 任务态 / core_conflict / material_hooks 重判。
迁移完成后 evidence_level = C,LLM 增量打标后升至 B。
