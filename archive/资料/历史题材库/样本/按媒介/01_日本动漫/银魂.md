---
# === 身份 ===
work_id: WORK-ANI-00084
work_name: 银魂
work_name_alt: []
media_type: ANI
year_first: 2005
year_range: [2005, 2006]
season_count: null

# === 三要素(v0 已有,迁移直翻)===
environment: 架空江户时代+外星人入侵
cultural_paradigm: 武士道+恶搞无厘头
narrative_core: 日常搞笑+热血战斗

# === SDT 心理需求(TBD,等 LLM 阶段补)===
n_main: N-Rel
n_others: [N-Auto]
rel_object: mixed
t_can_carry: [T3, T1]
core_emotion_note: ""
core_conflict: 废柴武士在混乱江户守护同伴与尊严

# === 三层标签(主检索路径,见 TAGS.md)===
theme_primary: 喜剧
theme_secondary: [日常喜剧, 江湖, 异种入侵, 友情]
visual_tags: [古风装扮, 霓虹灯, 武器对决, 搞笑荒诞]


# === 评分(v0 已有)===
acquisition_score: 4

# === 素材与玩法 ===
material_hooks: [万事屋日常, 银时战斗, 外星人造型, 恶搞名场面]
existing_games: [银魂（PS2/PSP/手游）]
recommended_playstyles: []
risk_notes: [版权成本高, 红海]

# === 元数据 ===
source: deepseek-v4-flash + v0-migrate + tags
source_run_id: 2026-05-18-tags-enrich
evidence_level: B
review_status: auto
last_updated: 2026-05-18
# v0_duplicate_count: 4
---

## 迁移备注

本样本由 v0 markdown 表格迁移而来,LLM 阶段尚未补 SDT 三需求 / T 任务态 / core_conflict / material_hooks 重判。
迁移完成后 evidence_level = C,LLM 增量打标后升至 B。
