---
# === 身份 ===
work_id: WORK-ANI-00461
work_name: 巴克·亚罗
work_name_alt: []
media_type: ANI
year_first: 2021
year_range: [2021, 2021]
season_count: null

# === 三要素(v0 已有,迁移直翻)===
environment: 架空科幻+机甲世界
cultural_paradigm: 机甲战斗+超能力
narrative_core: 反抗命运+热血战斗

# === SDT 心理需求(TBD,等 LLM 阶段补)===
n_main: N-Comp
n_others: []
rel_object: null
t_can_carry: [T2, T5]
core_emotion_note: ""
core_conflict: 主角与伙伴反抗命运，对抗神秘机甲强敌与自身界限

# === 三层标签(主检索路径,见 TAGS.md)===
theme_primary: 机甲
theme_secondary: [冒险, 科幻]
visual_tags: [机甲, 能量光翼, 巨大机器人对决, 热血必杀技]


# === 评分(v0 已有)===
acquisition_score: 3

# === 素材与玩法 ===
material_hooks: [机甲变身战斗, 巨大机器人对决, 能量光翼特效, 热血必杀技]
existing_games: []
recommended_playstyles: []
risk_notes: [受众窄, 红海竞争]

# === 元数据 ===
source: deepseek-v4-flash + v0-migrate + tags
source_run_id: 2026-05-18-tags-enrich
evidence_level: B
review_status: auto
last_updated: 2026-05-18
---

## 迁移备注

本样本由 v0 markdown 表格迁移而来,LLM 阶段尚未补 SDT 三需求 / T 任务态 / core_conflict / material_hooks 重判。
迁移完成后 evidence_level = C,LLM 增量打标后升至 B。
