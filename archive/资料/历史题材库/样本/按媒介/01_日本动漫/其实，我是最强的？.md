---
# === 身份 ===
work_id: WORK-ANI-00345
work_name: 其实，我是最强的？
work_name_alt: []
media_type: ANI
year_first: 2023
year_range: [2023, 2023]
season_count: null

# === 三要素(v0 已有,迁移直翻)===
environment: 异世界奇幻王国
cultural_paradigm: 魔法+隐藏实力体系
narrative_core: 废柴逆袭+扮猪吃虎

# === SDT 心理需求(TBD,等 LLM 阶段补)===
n_main: N-Comp
n_others: []
rel_object: null
t_can_carry: [T2]
core_emotion_note: ""
core_conflict: 隐藏实力的主角在异世界被当作废柴，暗中解决危机并逆袭

# === 三层标签(主检索路径,见 TAGS.md)===
theme_primary: 奇幻
theme_secondary: [异世界穿越, 魔法, 喜剧, 校园]
visual_tags: [魔法阵, 魔法战斗特效, 古风装扮, 校园]


# === 评分(v0 已有)===
acquisition_score: 2

# === 素材与玩法 ===
material_hooks: [隐藏实力爆发, 魔法战斗特效, 身份反转瞬间, 废柴逆袭场面]
existing_games: []
recommended_playstyles: []
risk_notes: [同质化严重, IP热度中等]

# === 元数据 ===
source: deepseek-v4-flash + v0-migrate + tags
source_run_id: 2026-05-18-tags-enrich
evidence_level: B
review_status: auto
last_updated: 2026-05-18
---

## 迁移备注

本样本由 v0 markdown 表格迁移而来,LLM 阶段尚未补 SDT 三需求 / T 任务态 / core_conflict / material_hooks 重判。
迁移完成后 evidence_level = C,LLM 增量打标后升至 B。
