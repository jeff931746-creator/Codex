---
# === 身份 ===
work_id: WORK-ANI-00775
work_name: 女神宿舍的管理员。
work_name_alt: []
media_type: ANI
year_first: 2021
year_range: [2021, 2021]
season_count: null

# === 三要素(v0 已有,迁移直翻)===
environment: 现代日本校园/宿舍
cultural_paradigm: 后宫日常+校园喜剧
narrative_core: 少年管理女生宿舍

# === SDT 心理需求(TBD,等 LLM 阶段补)===
n_main: N-Rel
n_others: []
rel_object: virtual
t_can_carry: [T3]
core_emotion_note: ""
core_conflict: 少年管理女生宿舍，与各具特色的女舍员建立暧昧关系

# === 三层标签(主检索路径,见 TAGS.md)===
theme_primary: 都市
theme_secondary: [日常喜剧, 校园, 恋爱]
visual_tags: [校园, 宿舍, 日常温馨]


# === 评分(v0 已有)===
acquisition_score: 2

# === 素材与玩法 ===
material_hooks: [女舍员泳装, 宿舍日常互动, 福利场景, 角色表情特写]
existing_games: []
recommended_playstyles: []
risk_notes: [受众窄, 付费浅, 政策风险]

# === 元数据 ===
source: deepseek-v4-flash + v0-migrate + tags
source_run_id: 2026-05-18-tags-enrich
evidence_level: B
review_status: auto
last_updated: 2026-05-18
---

## 迁移备注

本样本由 v0 markdown 表格迁移而来,LLM 阶段尚未补 SDT 三需求 / T 任务态 / core_conflict / material_hooks 重判。
迁移完成后 evidence_level = C,LLM 增量打标后升至 B。
