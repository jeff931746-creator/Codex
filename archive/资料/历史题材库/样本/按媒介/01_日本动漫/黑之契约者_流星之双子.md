---
# === 身份 ===
work_id: WORK-ANI-00261
work_name: 黑之契约者 流星之双子
work_name_alt: []
media_type: ANI
year_first: 2008
year_range: [2008, 2009]
season_count: null

# === 三要素(v0 已有,迁移直翻)===
environment: 现代都市+超能力地下世界
cultural_paradigm: 契约者超能力体系
narrative_core: 逃亡+身份谜团

# === SDT 心理需求(TBD,等 LLM 阶段补)===
n_main: N-Comp
n_others: [N-Rel]
rel_object: mixed
t_can_carry: [T2, T5]
core_emotion_note: ""
core_conflict: 契约者与人类共存危机，兄妹逃亡中揭开身份与世界的真相

# === 三层标签(主检索路径,见 TAGS.md)===
theme_primary: 都市
theme_secondary: [异能都市, 悬疑, 求生, 家庭]
visual_tags: [都市夜景, 霓虹灯, 废墟, 超能力对决, 黑色西装]


# === 评分(v0 已有)===
acquisition_score: 3

# === 素材与玩法 ===
material_hooks: [契约者战斗, 流星碎片坠落, 黑与银的羁绊, 东京废墟夜景]
existing_games: []
recommended_playstyles: []
risk_notes: [受众窄, 版权成本中]

# === 元数据 ===
source: deepseek-v4-flash + v0-migrate + tags
source_run_id: 2026-05-18-tags-enrich
evidence_level: B
review_status: auto
last_updated: 2026-05-18
# v0_duplicate_count: 2
---

## 迁移备注

本样本由 v0 markdown 表格迁移而来,LLM 阶段尚未补 SDT 三需求 / T 任务态 / core_conflict / material_hooks 重判。
迁移完成后 evidence_level = C,LLM 增量打标后升至 B。
