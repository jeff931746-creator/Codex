---
# === 身份 ===
work_id: WORK-ANI-00357
work_name: 你的名字。
work_name_alt: []
media_type: ANI
year_first: 2016
year_range: [2016, 2016]
season_count: null

# === 三要素(v0 已有,迁移直翻)===
environment: 现代日本都市与乡村
cultural_paradigm: 灵魂互换+时空穿越
narrative_core: 跨越时空的青春羁绊

# === SDT 心理需求(TBD,等 LLM 阶段补)===
n_main: N-Rel
n_others: []
rel_object: virtual
t_can_carry: [T3]
core_emotion_note: ""
core_conflict: 少年少女跨越时空与遗忘寻找彼此

# === 三层标签(主检索路径,见 TAGS.md)===
theme_primary: 恋爱
theme_secondary: [青春恋爱, 穿越转生, 都市]
visual_tags: [都市夜景, 校园, 彗星划过夜空, 黄昏时分]


# === 评分(v0 已有)===
acquisition_score: 5

# === 素材与玩法 ===
material_hooks: [彗星划过夜空, 黄昏时分的重逢, 口嚼酒仪式, 东京与糸守的风景]
existing_games: [你的名字。：时空交错]
recommended_playstyles: []
risk_notes: [受众窄, 付费深度低]

# === 元数据 ===
source: deepseek-v4-flash + v0-migrate + tags
source_run_id: 2026-05-18-tags-enrich
evidence_level: B
review_status: auto
last_updated: 2026-05-18
---

## 迁移备注

本样本由 v0 markdown 表格迁移而来,LLM 阶段尚未补 SDT 三需求 / T 任务态 / core_conflict / material_hooks 重判。
迁移完成后 evidence_level = C,LLM 增量打标后升至 B。
