---
# === 身份 ===
work_id: WORK-ANI-00170
work_name: 欢迎来到实力至上主义的教室
work_name_alt: []
media_type: ANI
year_first: 2017
year_range: [2017, 2017]
season_count: null

# === 三要素(v0 已有,迁移直翻)===
environment: 现代封闭学园岛
cultural_paradigm: 班级竞争+暗黑智斗体系
narrative_core: 班级竞争与暗黑智斗

# === SDT 心理需求(TBD,等 LLM 阶段补)===
n_main: N-Rel
n_others: [N-Comp]
rel_object: real
t_can_carry: [T4, T5]
core_emotion_note: ""
core_conflict: 班级间通过暗黑规则竞争晋升，主角隐藏实力操控全局

# === 三层标签(主检索路径,见 TAGS.md)===
theme_primary: 校园
theme_secondary: [青春恋爱, 校园悬疑, 异能学院, 日常喜剧, 权力游戏]
visual_tags: [校园, 教室, 都市夜景, 霓虹灯, 悬疑诡异]


# === 评分(v0 已有)===
acquisition_score: 4

# === 素材与玩法 ===
material_hooks: [班级对抗场景, 智斗博弈特写, 角色眼神交锋, 规则揭露瞬间]
existing_games: []
recommended_playstyles: []
risk_notes: [红海, 版权成本中]

# === 元数据 ===
source: deepseek-v4-flash + v0-migrate + tags
source_run_id: 2026-05-18-tags-enrich
evidence_level: B
review_status: auto
last_updated: 2026-05-18
# v0_duplicate_count: 2
---

## 迁移备注

本样本由 v0 markdown 表格迁移而来,LLM 阶段尚未补 SDT 三需求 / T 任务态 / core_conflict / material_hooks 重判。
迁移完成后 evidence_level = C,LLM 增量打标后升至 B。
