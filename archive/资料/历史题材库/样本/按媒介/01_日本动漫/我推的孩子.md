---
# === 身份 ===
work_id: WORK-ANI-00114
work_name: 我推的孩子
work_name_alt: []
media_type: ANI
year_first: 2023
year_range: [2023, 2026]
season_count: null

# === 三要素(v0 已有,迁移直翻)===
environment: 现代日本娱乐圈
cultural_paradigm: 偶像文化+复仇悬疑
narrative_core: 转生复仇与演艺圈生存

# === SDT 心理需求(TBD,等 LLM 阶段补)===
n_main: N-Comp
n_others: [N-Rel]
rel_object: mixed
t_can_carry: [T2, T5]
core_emotion_note: ""
core_conflict: 转生偶像之子复仇生父，同时攀登演艺圈巅峰

# === 三层标签(主检索路径,见 TAGS.md)===
theme_primary: 都市
theme_secondary: [转生异世, 悬疑, 职场]
visual_tags: [都市夜景, 舞台光芒, 偶像服饰, 悬疑诡异]


# === 评分(v0 已有)===
acquisition_score: 4

# === 素材与玩法 ===
material_hooks: [星野爱舞台光芒, 双胞胎转生瞬间, 复仇线索暗藏, 演艺圈暗面揭露]
existing_games: []
recommended_playstyles: []
risk_notes: [题材敏感, 版权成本极高]

# === 元数据 ===
source: deepseek-v4-flash + v0-migrate + tags
source_run_id: 2026-05-18-tags-enrich
evidence_level: B
review_status: auto
last_updated: 2026-05-18
# v0_duplicate_count: 5
---

## 迁移备注

本样本由 v0 markdown 表格迁移而来,LLM 阶段尚未补 SDT 三需求 / T 任务态 / core_conflict / material_hooks 重判。
迁移完成后 evidence_level = C,LLM 增量打标后升至 B。
