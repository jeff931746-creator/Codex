---
# === 身份 ===
work_id: WORK-ANI-00493
work_name: 阿倍野桥魔法商店街
work_name_alt: []
media_type: ANI
year_first: 2002
year_range: [2002, 2002]
season_count: null

# === 三要素(v0 已有,迁移直翻)===
environment: 现代日本商店街/异世界
cultural_paradigm: 魔法+恶搞冒险
narrative_core: 少年少女在商店街的魔法冒险

# === SDT 心理需求(TBD,等 LLM 阶段补)===
n_main: N-Auto
n_others: []
rel_object: null
t_can_carry: [T1, T5]
core_emotion_note: ""
core_conflict: 少年少女在商店街遭遇魔法异变，必须找回日常

# === 三层标签(主检索路径,见 TAGS.md)===
theme_primary: 冒险
theme_secondary: [日常喜剧, 奇幻, 都市]
visual_tags: [霓虹灯, 魔法阵, 异世界传送门, 搞笑荒诞]


# === 评分(v0 已有)===
acquisition_score: 3

# === 素材与玩法 ===
material_hooks: [商店街霓虹夜景, 魔法变身场面, 恶搞角色造型, 异世界传送门]
existing_games: []
recommended_playstyles: []
risk_notes: [受众窄, 风格小众]

# === 元数据 ===
source: deepseek-v4-flash + v0-migrate + tags
source_run_id: 2026-05-18-tags-enrich
evidence_level: B
review_status: auto
last_updated: 2026-05-18
# v0_duplicate_count: 2
---

## 迁移备注

本样本由 v0 markdown 表格迁移而来,LLM 阶段尚未补 SDT 三需求 / T 任务态 / core_conflict / material_hooks 重判。
迁移完成后 evidence_level = C,LLM 增量打标后升至 B。
