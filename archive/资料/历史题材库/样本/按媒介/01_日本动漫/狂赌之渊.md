---
# === 身份 ===
work_id: WORK-ANI-00169
work_name: 狂赌之渊
work_name_alt: []
media_type: ANI
year_first: 2017
year_range: [2017, 2017]
season_count: null

# === 三要素(v0 已有,迁移直翻)===
environment: 现代私立学校地下赌场
cultural_paradigm: 赌博博弈+心理战
narrative_core: 疯狂博弈与反转

# === SDT 心理需求(TBD,等 LLM 阶段补)===
n_main: N-Comp
n_others: []
rel_object: null
t_can_carry: [T5, T2]
core_emotion_note: ""
core_conflict: 疯狂赌徒在极端博弈中争夺权力与生存

# === 三层标签(主检索路径,见 TAGS.md)===
theme_primary: 校园
theme_secondary: [青春恋爱, 校园悬疑, 社团活动, 日常喜剧]
visual_tags: [校园, 教室, 都市夜景, 霓虹灯, 赌场]


# === 评分(v0 已有)===
acquisition_score: 4

# === 素材与玩法 ===
material_hooks: [颜艺爆发, 赌局反转, 蛇喰狂气, 筹码崩塌]
existing_games: []
recommended_playstyles: []
risk_notes: [题材敏感, 红海]

# === 元数据 ===
source: deepseek-v4-flash + v0-migrate + tags
source_run_id: 2026-05-18-tags-enrich
evidence_level: B
review_status: auto
last_updated: 2026-05-18
# v0_duplicate_count: 2
---

## 迁移备注

本样本由 v0 markdown 表格迁移而来,LLM 阶段尚未补 SDT 三需求 / T 任务态 / core_conflict / material_hooks 重判。
迁移完成后 evidence_level = C,LLM 增量打标后升至 B。
