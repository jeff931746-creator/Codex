---
# === 身份 ===
work_id: WORK-ANI-00374
work_name: 枪神
work_name_alt: []
media_type: ANI
year_first: 2005
year_range: [2005, 2005]
season_count: null

# === 三要素(v0 已有,迁移直翻)===
environment: 西部+科幻架空
cultural_paradigm: 赏金猎人+枪械战斗
narrative_core: 赏金猎人的西部冒险

# === SDT 心理需求(TBD,等 LLM 阶段补)===
n_main: N-Comp
n_others: [N-Auto]
rel_object: null
t_can_carry: [T2, T1]
core_emotion_note: ""
core_conflict: 赏金猎人在西部荒野中追逐悬赏与自由

# === 三层标签(主检索路径,见 TAGS.md)===
theme_primary: 冒险
theme_secondary: [西部, 科幻, 战斗, 成长]
visual_tags: [西部小镇, 左轮双枪, 风衣牛仔, 枪火特效]


# === 评分(v0 已有)===
acquisition_score: 3

# === 素材与玩法 ===
material_hooks: [左轮双枪对决, 西部小镇场景, 枪火特效, 风衣牛仔造型]
existing_games: []
recommended_playstyles: []
risk_notes: [受众中等, 版权成本高]

# === 元数据 ===
source: deepseek-v4-flash + v0-migrate + tags
source_run_id: 2026-05-18-tags-enrich
evidence_level: B
review_status: auto
last_updated: 2026-05-18
---

## 迁移备注

本样本由 v0 markdown 表格迁移而来,LLM 阶段尚未补 SDT 三需求 / T 任务态 / core_conflict / material_hooks 重判。
迁移完成后 evidence_level = C,LLM 增量打标后升至 B。
