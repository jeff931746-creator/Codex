---
# === 身份 ===
work_id: WORK-ANI-00526
work_name: 铁马少年
work_name_alt: []
media_type: ANI
year_first: 2007
year_range: [2007, 2007]
season_count: null

# === 三要素(v0 已有,迁移直翻)===
environment: 现代日本+公路赛场
cultural_paradigm: 自行车竞技+热血成长
narrative_core: 懦弱少年通过骑行蜕变

# === SDT 心理需求(TBD,等 LLM 阶段补)===
n_main: N-Comp
n_others: [N-Rel]
rel_object: mixed
t_can_carry: [T2, T4]
core_emotion_note: ""
core_conflict: 懦弱少年通过自行车竞技突破自我，与强敌和团队羁绊对抗

# === 三层标签(主检索路径,见 TAGS.md)===
theme_primary: 体育竞技
theme_secondary: [篮球, 足球, 棒球, 网球, 田径]
visual_tags: [公路飞驰, 爬坡苦战, 终点冲刺, 团队接力]


# === 评分(v0 已有)===
acquisition_score: 3

# === 素材与玩法 ===
material_hooks: [公路飞驰, 爬坡苦战, 终点冲刺, 团队接力]
existing_games: []
recommended_playstyles: []
risk_notes: [受众窄, 玩法同质化]

# === 元数据 ===
source: deepseek-v4-flash + v0-migrate + tags
source_run_id: 2026-05-18-tags-enrich
evidence_level: B
review_status: auto
last_updated: 2026-05-18
---

## 迁移备注

本样本由 v0 markdown 表格迁移而来,LLM 阶段尚未补 SDT 三需求 / T 任务态 / core_conflict / material_hooks 重判。
迁移完成后 evidence_level = C,LLM 增量打标后升至 B。
