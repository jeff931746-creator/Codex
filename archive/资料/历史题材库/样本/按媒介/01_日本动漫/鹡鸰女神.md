---
# === 身份 ===
work_id: WORK-ANI-00248
work_name: 鹡鸰女神
work_name_alt: []
media_type: ANI
year_first: 2008
year_range: [2008, 2008]
season_count: null

# === 三要素(v0 已有,迁移直翻)===
environment: 现代东京+战斗竞技
cultural_paradigm: 契约战士+鸟形拟人
narrative_core: 少年与战士的契约战斗

# === SDT 心理需求(TBD,等 LLM 阶段补)===
n_main: N-Rel
n_others: [N-Comp]
rel_object: virtual
t_can_carry: [T3, T2]
core_emotion_note: ""
core_conflict: 少年与鹡鸰战士签订契约，在淘汰赛中争夺生存与羁绊

# === 三层标签(主检索路径,见 TAGS.md)===
theme_primary: 奇幻
theme_secondary: [异能学院, 恋爱, 都市]
visual_tags: [契约纹章发光, 萌系战士变身, 竞技场对决, 都市夜景]


# === 评分(v0 已有)===
acquisition_score: 3

# === 素材与玩法 ===
material_hooks: [鹡鸰羽翼战斗, 契约纹章发光, 萌系战士变身, 竞技场对决]
existing_games: [鹡鸰女神 游戏]
recommended_playstyles: []
risk_notes: [题材敏感, 受众窄]

# === 元数据 ===
source: deepseek-v4-flash + v0-migrate + tags
source_run_id: 2026-05-18-tags-enrich
evidence_level: B
review_status: auto
last_updated: 2026-05-18
# v0_duplicate_count: 2
---

## 迁移备注

本样本由 v0 markdown 表格迁移而来,LLM 阶段尚未补 SDT 三需求 / T 任务态 / core_conflict / material_hooks 重判。
迁移完成后 evidence_level = C,LLM 增量打标后升至 B。
