---
# === 身份 ===
work_id: WORK-ANI-00718
work_name: 白箱
work_name_alt: []
media_type: ANI
year_first: 2014
year_range: [2014, 2015]
season_count: null

# === 三要素(v0 已有,迁移直翻)===
environment: 现代日本动画业界
cultural_paradigm: 职场奋斗+创作日常
narrative_core: 梦想追逐+团队协作

# === SDT 心理需求(TBD,等 LLM 阶段补)===
n_main: N-Rel
n_others: [N-Auto]
rel_object: mixed
t_can_carry: [T3, T1]
core_emotion_note: ""
core_conflict: 五位动画新人追逐梦想，在职场压力与创作热情中协作成长

# === 三层标签(主检索路径,见 TAGS.md)===
theme_primary: 职场
theme_secondary: [日常喜剧, 友情, 青春恋爱]
visual_tags: [校园, 教室, 办公室, 动画制作现场, 日常温馨]


# === 评分(v0 已有)===
acquisition_score: 3

# === 素材与玩法 ===
material_hooks: [动画制作现场, 分镜会议, 声优录音, 原画修正, 完成放映]
existing_games: []
recommended_playstyles: []
risk_notes: [受众窄, 玩法难创新]

# === 元数据 ===
source: deepseek-v4-flash + v0-migrate + tags
source_run_id: 2026-05-18-tags-enrich
evidence_level: B
review_status: auto
last_updated: 2026-05-18
# v0_duplicate_count: 3
---

## 迁移备注

本样本由 v0 markdown 表格迁移而来,LLM 阶段尚未补 SDT 三需求 / T 任务态 / core_conflict / material_hooks 重判。
迁移完成后 evidence_level = C,LLM 增量打标后升至 B。
