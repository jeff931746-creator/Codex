---
# === 身份 ===
work_id: WORK-ANI-00259
work_name: 圣痕炼金士
work_name_alt: []
media_type: ANI
year_first: 2010
year_range: [2010, 2010]
season_count: null

# === 三要素(v0 已有,迁移直翻)===
environment: 现代学园+炼金术世界
cultural_paradigm: 炼金术+圣乳战斗
narrative_core: 战斗与秘密守护

# === SDT 心理需求(TBD,等 LLM 阶段补)===
n_main: N-Comp
n_others: [N-Rel]
rel_object: mixed
t_can_carry: [T2, T4]
core_emotion_note: ""
core_conflict: 炼金术士争夺圣乳力量，守护秘密与校园暗战

# === 三层标签(主检索路径,见 TAGS.md)===
theme_primary: 奇幻
theme_secondary: [校园, 异能学院, 魔法]
visual_tags: [校园, 魔法阵, 战斗场面, 爆衣]


# === 评分(v0 已有)===
acquisition_score: 2

# === 素材与玩法 ===
material_hooks: [圣乳战斗, 炼金术阵, 角色爆衣, 校园暗战]
existing_games: []
recommended_playstyles: []
risk_notes: [政策风险高, 受众窄]

# === 元数据 ===
source: deepseek-v4-flash + v0-migrate + tags
source_run_id: 2026-05-18-tags-enrich
evidence_level: B
review_status: auto
last_updated: 2026-05-18
---

## 迁移备注

本样本由 v0 markdown 表格迁移而来,LLM 阶段尚未补 SDT 三需求 / T 任务态 / core_conflict / material_hooks 重判。
迁移完成后 evidence_level = C,LLM 增量打标后升至 B。
