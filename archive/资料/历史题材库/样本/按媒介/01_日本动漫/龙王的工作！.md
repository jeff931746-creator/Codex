---
# === 身份 ===
work_id: WORK-ANI-00441
work_name: 龙王的工作！
work_name_alt: []
media_type: ANI
year_first: 2017
year_range: [2017, 2018]
season_count: null

# === 三要素(v0 已有,迁移直翻)===
environment: 现代日本将棋界
cultural_paradigm: 将棋竞技体系
narrative_core: 天才成长+师徒羁绊

# === SDT 心理需求(TBD,等 LLM 阶段补)===
n_main: N-Comp
n_others: [N-Rel]
rel_object: mixed
t_can_carry: [T2, T3]
core_emotion_note: ""
core_conflict: 天才将棋少年与萝莉弟子共同成长，挑战职业棋坛巅峰

# === 三层标签(主检索路径,见 TAGS.md)===
theme_primary: 体育竞技
theme_secondary: [师徒情, 校园, 日常喜剧]
visual_tags: [校园, 和服, 棋盘对决, 日常温馨]


# === 评分(v0 已有)===
acquisition_score: 3

# === 素材与玩法 ===
material_hooks: [将棋对弈特写, 萝莉认真下棋, 龙王称号对决, 师徒日常互动]
existing_games: []
recommended_playstyles: []
risk_notes: [题材小众, 版权成本中]

# === 元数据 ===
source: deepseek-v4-flash + v0-migrate + tags
source_run_id: 2026-05-18-tags-enrich
evidence_level: B
review_status: auto
last_updated: 2026-05-18
# v0_duplicate_count: 2
---

## 迁移备注

本样本由 v0 markdown 表格迁移而来,LLM 阶段尚未补 SDT 三需求 / T 任务态 / core_conflict / material_hooks 重判。
迁移完成后 evidence_level = C,LLM 增量打标后升至 B。
