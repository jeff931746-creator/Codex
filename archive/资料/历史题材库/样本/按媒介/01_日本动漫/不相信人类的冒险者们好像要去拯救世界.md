---
# === 身份 ===
work_id: WORK-ANI-00612
work_name: 不相信人类的冒险者们好像要去拯救世界
work_name_alt: []
media_type: ANI
year_first: 2023
year_range: [2023, 2023]
season_count: null

# === 三要素(v0 已有,迁移直翻)===
environment: 奇幻冒险世界
cultural_paradigm: 冒险者公会+组队战斗
narrative_core: 失意者组队重生冒险

# === SDT 心理需求(TBD,等 LLM 阶段补)===
n_main: N-Rel
n_others: [N-Comp]
rel_object: mixed
t_can_carry: [T3, T4]
core_emotion_note: ""
core_conflict: 被世界抛弃的冒险者们组队重生，对抗偏见与强敌

# === 三层标签(主检索路径,见 TAGS.md)===
theme_primary: 冒险
theme_secondary: [奇幻, 友情, 都市, 求生]
visual_tags: [奇幻瑰丽, 龙, 魔法阵, 公会大厅, 伙伴并肩]


# === 评分(v0 已有)===
acquisition_score: 2

# === 素材与玩法 ===
material_hooks: [失意者集结, 公会大厅, 龙与魔法战斗, 伙伴并肩, 逆袭瞬间]
existing_games: []
recommended_playstyles: []
risk_notes: [红海, 题材同质化]

# === 元数据 ===
source: deepseek-v4-flash + v0-migrate + tags
source_run_id: 2026-05-18-tags-enrich
evidence_level: B
review_status: auto
last_updated: 2026-05-18
---

## 迁移备注

本样本由 v0 markdown 表格迁移而来,LLM 阶段尚未补 SDT 三需求 / T 任务态 / core_conflict / material_hooks 重判。
迁移完成后 evidence_level = C,LLM 增量打标后升至 B。
