---
# === 身份 ===
work_id: WORK-ANI-00445
work_name: 五等分的新娘
work_name_alt: []
media_type: ANI
year_first: 2019
year_range: [2019, 2019]
season_count: null

# === 三要素(v0 已有,迁移直翻)===
environment: 现代日本高中
cultural_paradigm: 校园恋爱+五胞胎谜题
narrative_core: 恋爱推理与选择

# === SDT 心理需求(TBD,等 LLM 阶段补)===
n_main: N-Rel
n_others: []
rel_object: virtual
t_can_carry: [T3]
core_emotion_note: ""
core_conflict: 五胞胎姐妹与男主角的恋爱谜题，最终选择谁？

# === 三层标签(主检索路径,见 TAGS.md)===
theme_primary: 恋爱
theme_secondary: [校园恋爱, 日常喜剧, 青春恋爱]
visual_tags: [校园, 教室, 日常温馨, 恋爱甜蜜]


# === 评分(v0 已有)===
acquisition_score: 4

# === 素材与玩法 ===
material_hooks: [五胞胎姐妹同框, 约会场景特写, 婚礼悬念, 校园日常]
existing_games: [五等分的新娘 ～与你度过的五个回忆～, 五等分的新娘 五胞胎无法将五等分]
recommended_playstyles: []
risk_notes: [付费深度低, 受众偏窄]

# === 元数据 ===
source: deepseek-v4-flash + v0-migrate + tags
source_run_id: 2026-05-18-tags-enrich
evidence_level: B
review_status: auto
last_updated: 2026-05-18
# v0_duplicate_count: 2
---

## 迁移备注

本样本由 v0 markdown 表格迁移而来,LLM 阶段尚未补 SDT 三需求 / T 任务态 / core_conflict / material_hooks 重判。
迁移完成后 evidence_level = C,LLM 增量打标后升至 B。
