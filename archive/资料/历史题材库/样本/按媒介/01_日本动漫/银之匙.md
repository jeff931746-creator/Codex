---
# === 身份 ===
work_id: WORK-ANI-00711
work_name: 银之匙
work_name_alt: []
media_type: ANI
year_first: 2013
year_range: [2013, 2013]
season_count: null

# === 三要素(v0 已有,迁移直翻)===
environment: 现代日本农业学校
cultural_paradigm: 农业实践+青春成长
narrative_core: 青春成长+生活感悟

# === SDT 心理需求(TBD,等 LLM 阶段补)===
n_main: N-Rel
n_others: [N-Auto]
rel_object: mixed
t_can_carry: [T3, T1]
core_emotion_note: ""
core_conflict: 城市少年在农业学校中寻找自我与归属

# === 三层标签(主检索路径,见 TAGS.md)===
theme_primary: 田园
theme_secondary: [校园, 青春恋爱, 日常喜剧, 家庭]
visual_tags: [田园清新, 校园青春, 牧场晨光, 丰收祭典]


# === 评分(v0 已有)===
acquisition_score: 3

# === 素材与玩法 ===
material_hooks: [牧场晨光, 挤牛奶场景, 马匹奔跑, 丰收祭典, 校园日常]
existing_games: []
recommended_playstyles: []
risk_notes: [受众窄, 付费浅]

# === 元数据 ===
source: deepseek-v4-flash + v0-migrate + tags
source_run_id: 2026-05-18-tags-enrich
evidence_level: B
review_status: auto
last_updated: 2026-05-18
# v0_duplicate_count: 2
---

## 迁移备注

本样本由 v0 markdown 表格迁移而来,LLM 阶段尚未补 SDT 三需求 / T 任务态 / core_conflict / material_hooks 重判。
迁移完成后 evidence_level = C,LLM 增量打标后升至 B。
