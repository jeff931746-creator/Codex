---
# === 身份 ===
work_id: WORK-ANI-00193
work_name: 怪兽8号
work_name_alt: []
media_type: ANI
year_first: 2024
year_range: [2024, 2026]
season_count: null

# === 三要素(v0 已有,迁移直翻)===
environment: 近未来日本，怪兽频发都市
cultural_paradigm: 防卫队体制+怪兽变身能力
narrative_core: 变身怪兽逆袭守护

# === SDT 心理需求(TBD,等 LLM 阶段补)===
n_main: N-Comp
n_others: [N-Rel]
rel_object: mixed
t_can_carry: [T2, T4]
core_emotion_note: ""
core_conflict: 主角变身怪兽对抗怪兽，在防卫队中隐藏身份守护同伴

# === 三层标签(主检索路径,见 TAGS.md)===
theme_primary: 科幻
theme_secondary: [异种入侵, 求生, 职场]
visual_tags: [怪兽, 都市废墟, 防卫队装备, 战斗场面]


# === 评分(v0 已有)===
acquisition_score: 4

# === 素材与玩法 ===
material_hooks: [怪兽变身战斗, 防卫队装备, 巨大怪兽破坏, 身份隐藏悬念]
existing_games: []
recommended_playstyles: []
risk_notes: [版权成本高, 题材同质化]

# === 元数据 ===
source: deepseek-v4-flash + v0-migrate + tags
source_run_id: 2026-05-18-tags-enrich
evidence_level: B
review_status: auto
last_updated: 2026-05-18
# v0_duplicate_count: 2
---

## 迁移备注

本样本由 v0 markdown 表格迁移而来,LLM 阶段尚未补 SDT 三需求 / T 任务态 / core_conflict / material_hooks 重判。
迁移完成后 evidence_level = C,LLM 增量打标后升至 B。
