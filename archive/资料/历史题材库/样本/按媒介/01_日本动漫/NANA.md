---
# === 身份 ===
work_id: WORK-ANI-00806
work_name: NANA
work_name_alt: []
media_type: ANI
year_first: 2006
year_range: [2006, 2006]
season_count: null

# === 三要素(v0 已有,迁移直翻)===
environment: 现代日本都市
cultural_paradigm: 音乐摇滚+青春友情
narrative_core: 两个同名女孩的成长故事

# === SDT 心理需求(TBD,等 LLM 阶段补)===
n_main: N-Rel
n_others: []
rel_object: mixed
t_can_carry: [T3]
core_emotion_note: ""
core_conflict: 两个同名女孩在友情与爱情中相互依赖又彼此伤害

# === 三层标签(主检索路径,见 TAGS.md)===
theme_primary: 都市
theme_secondary: [友情, 恋爱, 职场, 日常喜剧]
visual_tags: [都市夜景, 霓虹灯, 公寓, 咖啡馆, 校园]


# === 评分(v0 已有)===
acquisition_score: 4

# === 素材与玩法 ===
material_hooks: [朋克乐队演出, 雪地牵手镜头, 破碎的玻璃杯, 东京夜景天台]
existing_games: []
recommended_playstyles: []
risk_notes: [受众窄, 付费浅, 无战斗]

# === 元数据 ===
source: deepseek-v4-flash + v0-migrate + tags
source_run_id: 2026-05-18-tags-enrich
evidence_level: B
review_status: auto
last_updated: 2026-05-18
# v0_duplicate_count: 2
---

## 迁移备注

本样本由 v0 markdown 表格迁移而来,LLM 阶段尚未补 SDT 三需求 / T 任务态 / core_conflict / material_hooks 重判。
迁移完成后 evidence_level = C,LLM 增量打标后升至 B。
