---
# === 身份 ===
work_id: WORK-ANI-00846
work_name: KURAU Phantom Memory
work_name_alt: []
media_type: ANI
year_first: 2004
year_range: [2004, 2004]
season_count: null

# === 三要素(v0 已有,迁移直翻)===
environment: 近未来科幻都市
cultural_paradigm: 超能力+能量体共生
narrative_core: 少女成长与身份认同

# === SDT 心理需求(TBD,等 LLM 阶段补)===
n_main: N-Rel
n_others: [N-Comp]
rel_object: virtual
t_can_carry: [T3, T2]
core_emotion_note: ""
core_conflict: 少女与能量体共生，在守护与自我认同间挣扎

# === 三层标签(主检索路径,见 TAGS.md)===
theme_primary: 科幻
theme_secondary: [近未来, 都市, 友情, 冒险]
visual_tags: [都市夜景, 能量体光效, 双人战斗, 近未来质感]


# === 评分(v0 已有)===
acquisition_score: 2

# === 素材与玩法 ===
material_hooks: [能量体光效, 双人战斗, 都市夜景, 记忆碎片]
existing_games: []
recommended_playstyles: []
risk_notes: [受众极窄, 无改编基础]

# === 元数据 ===
source: deepseek-v4-flash + v0-migrate + tags
source_run_id: 2026-05-18-tags-enrich
evidence_level: B
review_status: auto
last_updated: 2026-05-18
---

## 迁移备注

本样本由 v0 markdown 表格迁移而来,LLM 阶段尚未补 SDT 三需求 / T 任务态 / core_conflict / material_hooks 重判。
迁移完成后 evidence_level = C,LLM 增量打标后升至 B。
