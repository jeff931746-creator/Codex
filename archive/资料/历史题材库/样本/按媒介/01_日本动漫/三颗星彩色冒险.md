---
# === 身份 ===
work_id: WORK-ANI-00861
work_name: 三颗星彩色冒险
work_name_alt: []
media_type: ANI
year_first: 2018
year_range: [2018, 2018]
season_count: null

# === 三要素(v0 已有,迁移直翻)===
environment: 现代日本地方城市
cultural_paradigm: 小学生日常+城市守护
narrative_core: 日常冒险+友情喜剧

# === SDT 心理需求(TBD,等 LLM 阶段补)===
n_main: N-Rel
n_others: [N-Auto]
rel_object: virtual
t_can_carry: [T3, T1]
core_emotion_note: ""
core_conflict: 三个小学生守护城市和平的日常冒险与友情羁绊

# === 三层标签(主检索路径,见 TAGS.md)===
theme_primary: 都市
theme_secondary: [日常喜剧, 友情, 校园]
visual_tags: [校园青春, 都市繁华, 日常温馨]


# === 评分(v0 已有)===
acquisition_score: 2

# === 素材与玩法 ===
material_hooks: [三色队服奔跑, 彩色星星标志, 小镇街景巡逻, 三人搞怪表情]
existing_games: []
recommended_playstyles: []
risk_notes: [受众极窄, 付费浅, 低龄化]

# === 元数据 ===
source: deepseek-v4-flash + v0-migrate + tags
source_run_id: 2026-05-18-tags-enrich
evidence_level: B
review_status: auto
last_updated: 2026-05-18
---

## 迁移备注

本样本由 v0 markdown 表格迁移而来,LLM 阶段尚未补 SDT 三需求 / T 任务态 / core_conflict / material_hooks 重判。
迁移完成后 evidence_level = C,LLM 增量打标后升至 B。
