---
# === 身份 ===
work_id: WORK-ANI-00327
work_name: 寒蝉鸣泣之时 卒
work_name_alt: []
media_type: ANI
year_first: 2021
year_range: [2021, 2021]
season_count: null

# === 三要素(v0 已有,迁移直翻)===
environment: 昭和乡村+轮回世界
cultural_paradigm: 恐怖轮回+解谜推理
narrative_core: 轮回惨剧+真相揭露

# === SDT 心理需求(TBD,等 LLM 阶段补)===
n_main: N-Comp
n_others: [N-Rel]
rel_object: mixed
t_can_carry: [T5, T2]
core_emotion_note: ""
core_conflict: 主角团在轮回中对抗惨剧，揭露幕后黑手与真相

# === 三层标签(主检索路径,见 TAGS.md)===
theme_primary: 悬疑
theme_secondary: [恐怖, 推理, 校园, 都市]
visual_tags: [昭和乡村, 柴刀血溅, 轮回崩坏, 恐怖压抑]


# === 评分(v0 已有)===
acquisition_score: 4

# === 素材与玩法 ===
material_hooks: [柴刀血溅, 雏见泽村落, 轮回崩坏, 御社神作祟]
existing_games: [《寒蝉鸣泣之时》系列多部游戏]
recommended_playstyles: []
risk_notes: [版权成本高, 受众偏核心]

# === 元数据 ===
source: deepseek-v4-flash + v0-migrate + tags
source_run_id: 2026-05-18-tags-enrich
evidence_level: B
review_status: auto
last_updated: 2026-05-18
---

## 迁移备注

本样本由 v0 markdown 表格迁移而来,LLM 阶段尚未补 SDT 三需求 / T 任务态 / core_conflict / material_hooks 重判。
迁移完成后 evidence_level = C,LLM 增量打标后升至 B。
