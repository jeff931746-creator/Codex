---
# === 身份 ===
work_id: WORK-ANI-00810
work_name: 白虎
work_name_alt: []
media_type: ANI
year_first: 2008
year_range: [2008, 2008]
season_count: null

# === 三要素(v0 已有,迁移直翻)===
environment: 现代都市动物咖啡店
cultural_paradigm: 动物拟人+日常经营
narrative_core: 温馨日常+经营成长

# === SDT 心理需求(TBD,等 LLM 阶段补)===
n_main: N-Rel
n_others: [N-Auto]
rel_object: virtual
t_can_carry: [T3, T1]
core_emotion_note: ""
core_conflict: 动物店员与顾客在咖啡店中经营日常、建立羁绊

# === 三层标签(主检索路径,见 TAGS.md)===
theme_primary: 都市
theme_secondary: [日常喜剧, 职场日常, 友情]
visual_tags: [咖啡馆, 都市繁华, 阳光治愈, 日常温馨]


# === 评分(v0 已有)===
acquisition_score: 2

# === 素材与玩法 ===
material_hooks: [毛茸茸动物店员, 温馨咖啡店场景, 萌系互动瞬间, 治愈系日常画面]
existing_games: []
recommended_playstyles: []
risk_notes: [受众极窄, 付费天花板低]

# === 元数据 ===
source: deepseek-v4-flash + v0-migrate + tags
source_run_id: 2026-05-18-tags-enrich
evidence_level: B
review_status: auto
last_updated: 2026-05-18
---

## 迁移备注

本样本由 v0 markdown 表格迁移而来,LLM 阶段尚未补 SDT 三需求 / T 任务态 / core_conflict / material_hooks 重判。
迁移完成后 evidence_level = C,LLM 增量打标后升至 B。
