---
# === 身份 ===
work_id: WORK-ANI-00373
work_name: 极道鲜师
work_name_alt: []
media_type: ANI
year_first: 2004
year_range: [2004, 2004]
season_count: null

# === 三要素(v0 已有,迁移直翻)===
environment: 现代日本高中
cultural_paradigm: 热血教师+不良少年
narrative_core: 教师收服不良学生

# === SDT 心理需求(TBD,等 LLM 阶段补)===
n_main: N-Rel
n_others: [N-Comp]
rel_object: mixed
t_can_carry: [T3, T2]
core_emotion_note: ""
core_conflict: 热血女教师用真心与行动收服不良学生，对抗校园黑暗

# === 三层标签(主检索路径,见 TAGS.md)===
theme_primary: 校园
theme_secondary: [青春恋爱, 校园暴力, 日常喜剧, 师生关系]
visual_tags: [校园, 教室, 操场, 老街巷, 校园青春]


# === 评分(v0 已有)===
acquisition_score: 3

# === 素材与玩法 ===
material_hooks: [女教师热血训话, 不良少年群架, 教室对峙场面, 师生并肩作战]
existing_games: []
recommended_playstyles: []
risk_notes: [版权成本中, 玩法同质化]

# === 元数据 ===
source: deepseek-v4-flash + v0-migrate + tags
source_run_id: 2026-05-18-tags-enrich
evidence_level: B
review_status: auto
last_updated: 2026-05-18
# v0_duplicate_count: 2
---

## 迁移备注

本样本由 v0 markdown 表格迁移而来,LLM 阶段尚未补 SDT 三需求 / T 任务态 / core_conflict / material_hooks 重判。
迁移完成后 evidence_level = C,LLM 增量打标后升至 B。
