---
# === 身份 ===
work_id: WORK-ANI-00244
work_name: 英雄时代
work_name_alt: []
media_type: ANI
year_first: 2007
year_range: [2007, 2007]
season_count: null

# === 三要素(v0 已有,迁移直翻)===
environment: 星际战争时代
cultural_paradigm: 黄金种族+机甲战斗
narrative_core: 人类与神族共存抗争

# === SDT 心理需求(TBD,等 LLM 阶段补)===
n_main: N-Comp
n_others: []
rel_object: null
t_can_carry: [T2, T5]
core_emotion_note: ""
core_conflict: 人类与黄金神族在星际战争中争夺生存与共存之路

# === 三层标签(主检索路径,见 TAGS.md)===
theme_primary: 科幻
theme_secondary: [星际战争, 机甲, 太空, 战争]
visual_tags: [星舰内部, 机甲, 爆炸场面, 太空孤寂]


# === 评分(v0 已有)===
acquisition_score: 3

# === 素材与玩法 ===
material_hooks: [机甲巨炮对决, 神族光翼战舰, 星际战场爆炸, 黄金种族造型]
existing_games: []
recommended_playstyles: []
risk_notes: [红海竞争, 版权成本中等]

# === 元数据 ===
source: deepseek-v4-flash + v0-migrate + tags
source_run_id: 2026-05-18-tags-enrich
evidence_level: B
review_status: auto
last_updated: 2026-05-18
---

## 迁移备注

本样本由 v0 markdown 表格迁移而来,LLM 阶段尚未补 SDT 三需求 / T 任务态 / core_conflict / material_hooks 重判。
迁移完成后 evidence_level = C,LLM 增量打标后升至 B。
