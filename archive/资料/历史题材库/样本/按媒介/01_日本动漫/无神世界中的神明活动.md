---
# === 身份 ===
work_id: WORK-ANI-00342
work_name: 无神世界中的神明活动
work_name_alt: []
media_type: ANI
year_first: 2023
year_range: [2023, 2023]
season_count: null

# === 三要素(v0 已有,迁移直翻)===
environment: 无神异世界
cultural_paradigm: 邪教传教+神明体系
narrative_core: 传教扩张+信仰战争

# === SDT 心理需求(TBD,等 LLM 阶段补)===
n_main: N-Rel
n_others: [N-Comp]
rel_object: virtual
t_can_carry: [T4, T2]
core_emotion_note: ""
core_conflict: 主角在无神世界传教扩张，与敌对信仰和势力争夺信徒与统治权

# === 三层标签(主检索路径,见 TAGS.md)===
theme_primary: 奇幻
theme_secondary: [异世界穿越, 喜剧, 政治权谋]
visual_tags: [异世神秘, 古风装扮, 魔法阵, 搞笑荒诞, 信仰战争]


# === 评分(v0 已有)===
acquisition_score: 3

# === 素材与玩法 ===
material_hooks: [邪神祭祀仪式, 荒诞传教场面, 信仰战争冲突, 反套路搞笑演出]
existing_games: []
recommended_playstyles: []
risk_notes: [题材敏感, 受众窄]

# === 元数据 ===
source: deepseek-v4-flash + v0-migrate + tags
source_run_id: 2026-05-18-tags-enrich
evidence_level: B
review_status: auto
last_updated: 2026-05-18
---

## 迁移备注

本样本由 v0 markdown 表格迁移而来,LLM 阶段尚未补 SDT 三需求 / T 任务态 / core_conflict / material_hooks 重判。
迁移完成后 evidence_level = C,LLM 增量打标后升至 B。
