---
# === 身份 ===
work_id: WORK-ANI-00668
work_name: 再见！绝望先生
work_name_alt: []
media_type: ANI
year_first: 2007
year_range: [2007, 2007]
season_count: null

# === 三要素(v0 已有,迁移直翻)===
environment: 现代日本校园
cultural_paradigm: 黑色幽默+社会讽刺
narrative_core: 绝望教师吐槽社会

# === SDT 心理需求(TBD,等 LLM 阶段补)===
n_main: N-Auto
n_others: []
rel_object: null
t_can_carry: [T1]
core_emotion_note: ""
core_conflict: 绝望教师用黑色幽默对抗荒诞社会

# === 三层标签(主检索路径,见 TAGS.md)===
theme_primary: 喜剧
theme_secondary: [讽刺喜剧, 校园, 日常喜剧]
visual_tags: [校园, 教室, 搞笑荒诞, 黑色幽默]


# === 评分(v0 已有)===
acquisition_score: 3

# === 素材与玩法 ===
material_hooks: [夸张绝望表情, 文字吐槽弹幕, 社会讽刺画面]
existing_games: []
recommended_playstyles: []
risk_notes: [受众窄, 难做素材]

# === 元数据 ===
source: deepseek-v4-flash + v0-migrate + tags
source_run_id: 2026-05-18-tags-enrich
evidence_level: B
review_status: auto
last_updated: 2026-05-18
---

## 迁移备注

本样本由 v0 markdown 表格迁移而来,LLM 阶段尚未补 SDT 三需求 / T 任务态 / core_conflict / material_hooks 重判。
迁移完成后 evidence_level = C,LLM 增量打标后升至 B。
