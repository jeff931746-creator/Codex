---
# === 身份 ===
work_id: WORK-ANI-00581
work_name: 摇曳露营△
work_name_alt: []
media_type: ANI
year_first: 2018
year_range: [2018, 2018]
season_count: null

# === 三要素(v0 已有,迁移直翻)===
environment: 现代日本户外营地
cultural_paradigm: 露营文化+慢生活
narrative_core: 治愈日常+户外体验

# === SDT 心理需求(TBD,等 LLM 阶段补)===
n_main: N-Auto
n_others: [N-Rel]
rel_object: mixed
t_can_carry: [T1, T3]
core_emotion_note: ""
core_conflict: 少女们在户外露营中寻找自我节奏与同伴温暖

# === 三层标签(主检索路径,见 TAGS.md)===
theme_primary: 田园
theme_secondary: [日常喜剧, 友情, 美食]
visual_tags: [森林, 山脉, 篝火, 星空夜景, 阳光治愈]


# === 评分(v0 已有)===
acquisition_score: 3

# === 素材与玩法 ===
material_hooks: [富士山远景, 篝火暖光, 露营装备细节, 美食特写, 星空夜景]
existing_games: [《摇曳露营△ VIRTUAL CAMP》]
recommended_playstyles: []
risk_notes: [付费浅, 受众窄]

# === 元数据 ===
source: deepseek-v4-flash + v0-migrate + tags
source_run_id: 2026-05-18-tags-enrich
evidence_level: B
review_status: auto
last_updated: 2026-05-18
---

## 迁移备注

本样本由 v0 markdown 表格迁移而来,LLM 阶段尚未补 SDT 三需求 / T 任务态 / core_conflict / material_hooks 重判。
迁移完成后 evidence_level = C,LLM 增量打标后升至 B。
