---
# === 身份 ===
work_id: WORK-ANI-00414
work_name: 物语系列
work_name_alt: []
media_type: ANI
year_first: 2013
year_range: [2013, 2013]
season_count: null

# === 三要素(v0 已有,迁移直翻)===
environment: 现代日本都市/怪异世界
cultural_paradigm: 怪异现象+对话战斗体系
narrative_core: 怪异事件解谜与自我救赎

# === SDT 心理需求(TBD,等 LLM 阶段补)===
n_main: N-Rel
n_others: [N-Comp]
rel_object: virtual
t_can_carry: [T5, T3]
core_emotion_note: ""
core_conflict: 怪异事件背后的人性纠葛与自我救赎

# === 三层标签(主检索路径,见 TAGS.md)===
theme_primary: 都市
theme_secondary: [怪谈都市, 悬疑, 友情, 校园]
visual_tags: [都市夜景, 校园, 怪异战斗场面, 古风装扮]


# === 评分(v0 已有)===
acquisition_score: 4

# === 素材与玩法 ===
material_hooks: [怪异少女对话, 战场原黑仪, 阿良良木历, 怪异战斗场面]
existing_games: []
recommended_playstyles: []
risk_notes: [受众窄, 剧情改编难度高]

# === 元数据 ===
source: deepseek-v4-flash + v0-migrate + tags
source_run_id: 2026-05-18-tags-enrich
evidence_level: B
review_status: auto
last_updated: 2026-05-18
# v0_duplicate_count: 2
---

## 迁移备注

本样本由 v0 markdown 表格迁移而来,LLM 阶段尚未补 SDT 三需求 / T 任务态 / core_conflict / material_hooks 重判。
迁移完成后 evidence_level = C,LLM 增量打标后升至 B。
