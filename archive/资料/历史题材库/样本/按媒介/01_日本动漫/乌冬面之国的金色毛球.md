---
# === 身份 ===
work_id: WORK-ANI-00737
work_name: 乌冬面之国的金色毛球
work_name_alt: []
media_type: ANI
year_first: 2016
year_range: [2016, 2016]
season_count: null

# === 三要素(v0 已有,迁移直翻)===
environment: 现代日本乡村
cultural_paradigm: 日常治愈+狸猫传说
narrative_core: 回乡治愈与亲情

# === SDT 心理需求(TBD,等 LLM 阶段补)===
n_main: N-Auto
n_others: [N-Rel]
rel_object: virtual
t_can_carry: [T1, T3]
core_emotion_note: ""
core_conflict: 都市青年回乡寻找自我，与狸猫化身的亲情羁绊

# === 三层标签(主检索路径,见 TAGS.md)===
theme_primary: 田园
theme_secondary: [家庭, 亲情, 日常喜剧, 都市]
visual_tags: [田园清新, 阳光治愈, 日常温馨, 乡村田园]


# === 评分(v0 已有)===
acquisition_score: 2

# === 素材与玩法 ===
material_hooks: [乡村田园风光, 狸猫变身小孩, 日常温馨互动, 四季景色变换]
existing_games: []
recommended_playstyles: []
risk_notes: [受众窄, 付费浅]

# === 元数据 ===
source: deepseek-v4-flash + v0-migrate + tags
source_run_id: 2026-05-18-tags-enrich
evidence_level: B
review_status: auto
last_updated: 2026-05-18
# v0_duplicate_count: 2
---

## 迁移备注

本样本由 v0 markdown 表格迁移而来,LLM 阶段尚未补 SDT 三需求 / T 任务态 / core_conflict / material_hooks 重判。
迁移完成后 evidence_level = C,LLM 增量打标后升至 B。
