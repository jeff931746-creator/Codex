---
# === 身份 ===
work_id: WORK-ANI-00730
work_name: 三月的狮子
work_name_alt: []
media_type: ANI
year_first: 2016
year_range: [2016, 2017]
season_count: null

# === 三要素(v0 已有,迁移直翻)===
environment: 现代日本都市
cultural_paradigm: 将棋竞技文化
narrative_core: 孤独治愈与成长

# === SDT 心理需求(TBD,等 LLM 阶段补)===
n_main: N-Rel
n_others: [N-Auto]
rel_object: virtual
t_can_carry: [T3, T1]
core_emotion_note: ""
core_conflict: 孤独将棋少年在家庭温暖与竞技压力中寻找归属

# === 三层标签(主检索路径,见 TAGS.md)===
theme_primary: 都市
theme_secondary: [体育竞技, 家庭, 友情, 日常喜剧]
visual_tags: [都市夜景, 公寓, 校园, 咖啡馆, 阳光治愈]


# === 评分(v0 已有)===
acquisition_score: 3

# === 素材与玩法 ===
material_hooks: [将棋对弈特写, 川本家日常场景, 猫与暖色调画面, 孤独少年独白]
existing_games: []
recommended_playstyles: []
risk_notes: [受众窄, 付费浅]

# === 元数据 ===
source: deepseek-v4-flash + v0-migrate + tags
source_run_id: 2026-05-18-tags-enrich
evidence_level: B
review_status: auto
last_updated: 2026-05-18
# v0_duplicate_count: 3
---

## 迁移备注

本样本由 v0 markdown 表格迁移而来,LLM 阶段尚未补 SDT 三需求 / T 任务态 / core_conflict / material_hooks 重判。
迁移完成后 evidence_level = C,LLM 增量打标后升至 B。
